...
namespace Bars.GkhGji.Report
{
    ...
    public class ProtocolGjiReport : GjiBaseReport
    {
        ...
        public override void PrepareReport(ReportParams reportParams)
        {
            ...
            try
            {
                ...
                var protocolArticleLaws = protocolArticleLawDomain.GetAll()
                    ...
                    .Select(x => new { x.ArticleLaw.Name, x.ArticleLaw.NameOMS })
                    .ToList();

                reportParams.SimpleReportParams["СтатьяЗакона"] = protocolArticleLaws.Select(x => x.Name)
                             .AggregateWithSeparator(", ");

                reportParams.SimpleReportParams["Описание"] = protocolArticleLaws.Select(x => x.NameOMS)
                             .AggregateWithSeparator(", ");
                ...
                if (protocol.MinuteOfProceedings < 10)
                {
                    reportParams.SimpleReportParams["ДатаИВремяРассмотренияДела"] = string.Format("{0} в {1} час. 0{2} мин.",
