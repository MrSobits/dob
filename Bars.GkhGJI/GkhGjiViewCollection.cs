...
FROM gji_inspection insp
    ...
    LEFT JOIN b4_state state ON insp.state_id = state.id";
        ...
        {
            return @"
CREATE OR REPLACE VIEW view_gji_ins_statement AS 
SELECT 
    insp.id, 
...
FROM gji_inspection insp
    ...
    LEFT JOIN GJI_BASESTAT_APPCIT base on insp.id=base.INSPECTION_ID
    LEFT JOIN GJI_APPEAL_CITIZENS appeal on appeal.id=base.GJI_APPCIT_ID";
        }
