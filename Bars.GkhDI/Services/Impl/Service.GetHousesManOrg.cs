                ...
                if (period != null)
                {
                    var houses =
                        Container.Resolve<IDomainService<ManOrgContractRealityObject>>().GetAll()
                                 .Where(x => x.ManOrgContract.ManagingOrganization.Id == idManOrg && (!x.ManOrgContract.StartDate.HasValue || x.ManOrgContract.StartDate < period.DateEnd))
                                 .Select(
                                     x =>
                                     new House
                                         {
                                             ...
                                             AreaRooms = x.RealityObject.AreaMkd.HasValue ? x.RealityObject.AreaMkd.Value.RoundDecimal(2).ToString(numberformat) : string.Empty
                                         })
                                .ToArray();
