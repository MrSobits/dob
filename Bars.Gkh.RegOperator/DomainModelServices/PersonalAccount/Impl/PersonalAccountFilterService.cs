namespace Bars.Gkh.RegOperator.DomainModelServices.Impl
{
    ...
    public class PersonalAccountFilterService : IPersonalAccountFilterService
    {
        ...
        public IQueryable<TModel> FilterByRegFondSetting<TModel>(IQueryable<TModel> data) where TModel : PersonalAccountDto
        {
            ...
            var crFundFormationTypes = new List<CrFundFormationType>();

            if (includeSpecialCalcAccount)
            {
                crFundFormationTypes.Add(CrFundFormationType.SpecialAccount);
            }

            if (includeRegopCalcAccount)
            {
                crFundFormationTypes.Add(CrFundFormationType.RegOpAccount);
            }

            if (includeRegopSpecialCalcAccount)
            {
                crFundFormationTypes.Add(CrFundFormationType.SpecialRegOpAccount);
            }

            if (includeUnknown)
            {
                crFundFormationTypes.Add(CrFundFormationType.Unknown);
                crFundFormationTypes.Add(CrFundFormationType.NotSelected);
            }

            return data.Where(x => crFundFormationTypes.Contains(x.AccountFormationVariant));
        }
