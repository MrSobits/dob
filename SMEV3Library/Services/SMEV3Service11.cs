...
namespace SMEV3Library.Services
{
    public class SMEV3Service11 : ISMEV3Service
    {
        ...
        public async Task<SendRequestResponse> SendRequestAsyncSGIO(XElement request, string messageId, List<FileAttachment> attachments = null, bool saveLog = false)
        {
            ...
            var senderProvidedRequestDataXElement = new XElement(SMEVNamespaces11.TypesNamespace + "SenderProvidedRequestData",
                    new XAttribute("Id", "SIGNED_BY_CONSUMER")
                    , new XElement(SMEVNamespaces11.TypesNamespace + "MessageID", messageId)
                    , new XElement(SMEVNamespaces11.BasicNamespace + "MessagePrimaryContent", request)
                );
            ...
            //-----------Применяем namespace-----------
            sendRequestRequestXElement.SetAttributeValue(XNamespace.Xmlns + "ns", SMEVNamespaces11.TypesNamespace);
            sendRequestRequestXElement.SetAttributeValue(XNamespace.Xmlns + "ns1", SMEVNamespaces11.BasicNamespace);
