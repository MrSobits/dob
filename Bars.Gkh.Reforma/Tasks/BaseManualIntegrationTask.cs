        ...
        public override void Execute(DynamicDictionary @params)
        {
            ...
            try
            {
                ExplicitSessionScope.CallInNewScope(() =>
                {
                    try
                    {
                        this.SyncProvider = this.Container.Resolve<ISyncProvider>(new { typeIntegration = TypeIntegration.Selection });
                        this.Execute();
                    }
            ...
            {
                this.Container.UsingForResolved<ILogManager>((container, manager) => manager.Fatal(exception.ToString(), exception));
            }
        }
