...
create temp table temp_temle_to_actual as 
   ...
   srt_el.id AS STRUCT_EL_ID, srt_el.name as STRUCT_EL_NAME, state.id as state_id, state.name as state_name
     ...
     update ovrhl_stage1_version  set struct_el_id = ss.RO_EL from 
		...
		JOIN ovrhl_version_rec rec ON stage2.st3_version_id = rec.id
...
year = data.plan_year
  ...
  (
    ...
    JOIN ovhl_dpkr_correct_st2 corr ON st2.id = corr.st2_version_id
    WHERE st3.is_manually_correct IS TRUE AND st3.is_changed_year IS FALSE
  ) data
