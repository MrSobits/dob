...
CREATE OR REPLACE VIEW public.view_gji_protocol AS 
 SELECT 
    pr.date_violation,
    doc.id,
    doc.state_id,
    ...
    doc.type_document AS type_doc,
        CASE
            WHEN insp.control_type IS NULL THEN 30
            ELSE insp.control_type
        END AS control_type,
...
ALTER TABLE public.view_gji_protocol
  OWNER TO bars;";
		}
		...
		{
			return @"
DROP VIEW public.view_gji_protocol197;
CREATE OR REPLACE VIEW public.view_gji_protocol197 AS 
 SELECT doc.id,
    doc.state_id,
    doc.id AS document_id,
    doc.document_date,
    ...
    gjigetdocarticlelaw(pr.id) AS art_law,
        CASE
            WHEN pr.control_type IS NULL AND insp.control_type IS NOT NULL THEN insp.control_type
            WHEN pr.control_type IS NULL AND insp.control_type IS NULL THEN 30
            ELSE pr.control_type
        END AS control_type,
    pr.fio,
    pr.place_residence,
    pr.actually_residence,
    pr.birth_place,
    pr.job,
    pr.date_birth,
    pr.passport_number,
...
ALTER TABLE public.view_gji_protocol197
  OWNER TO bars;";
		}
