namespace Bars.Gkh.Reforma.Impl
{
    ...
    public class SyncProvider : ISyncProvider
    {
        ...
        private ApiSoapPortClient GetClient()
        {
            this.ThrowIfClosed();
                ...
                lock (lockObject)
                {
                    if (this.client == null || this.client.State == CommunicationState.Faulted)
                    {
                        this.client = !string.IsNullOrEmpty(this.remoteAddress)
                                          ? new ApiSoapPortClient(new BasicHttpBinding("ApiSoapBinding"), new EndpointAddress(this.remoteAddress))
                        ...
                        this.client.Endpoint.Behaviors.Add(new AuthHeaderBehavior(this.login, this.password));
                        this.client.Endpoint.Behaviors.Add(new NamespaceFixingBehavior(this.remoteAddress));
                        if (this.Logger != null)
                        {
                            this.client.Endpoint.Behaviors.Add(new LoggerBehavior(this.Logger));
                        }
                        ...
                        if (vs != null)
                        {
                            this.client.Endpoint.Behaviors.Remove(vs);
                        }
                    }
                }
            }
        ...
        private ISyncActionPerformer GetPerformer()
        {
            this.ThrowIfClosed();
            if (this.performer == null)
            {
