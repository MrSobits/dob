namespace Bars.Gkh.Map.Contragent
{
    ...
    public class ContragentContactMap : BaseImportableEntityMap<ContragentContact>
    {
        ...
        protected override void Map()
        {
            ...
            Reference(x => x.IndividualPerson, "Физическое лицо").Column("INDIVIDUAL_PERSON_ID").Fetch();
            Property(x => x.Snils, "СНИЛС").Column("SNILS").Length(50);
            Property(x => x.PlaceResidence, "Место жительства").Column("PLACE_RESIDENCE");
            Property(x => x.Birthplace, "Место рождения").Column("BIRTHPLACE");
            Property(x => x.PassportID, "Номер паспорта").Column("PASSPORT_ID");
            Property(x => x.PassportSeries, "Серия паспорта").Column("PASSPORT_SERIES");
