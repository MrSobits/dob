	...
	public class ActCheckNotificationStimulReport : GjiBaseStimulReport
    {
		...
		public override void PrepareReport(ReportParams reportParams)
		{
			...
			if (contragent != null)
			{
				...
				Report["ЮрАдрес"] = contragent.FiasJuridicalAddress != null
			...
			if (_protocol != null)
			{
				...
				using (Container.Using(protocolDefinitionDomain))
				{
					...
					{
						Report["ДатаНазнач"] = protocolDefinition.DocumentDate.HasValue
							? protocolDefinition.DocumentDate.Value.ToString("«dd» MMMM yyyy", new CultureInfo("ru-RU"))
							: string.Empty;

						Report["ВремНазнач"] = protocolDefinition.TimeDefinition.HasValue
