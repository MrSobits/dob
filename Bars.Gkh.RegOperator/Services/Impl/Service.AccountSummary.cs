namespace Bars.Gkh.RegOperator.Services.Impl
{
    ...
    public partial class Service
    {
            ...
            using (this.Container.Using(accountDomain, individualOwnerDomain, legalOwnerDomain))
            {
                ...
                else
                {
                    ...
                    foreach (var account in accountList)
                    {
                        var owner = ownerDict.Get(account.AccountOwner.Id);
                        if (owner == null ||
                            !(account.Room.RoomNum == flatNum && (!string.IsNullOrEmpty(lastName) ||
                                string.Equals(owner.Surname, lastName, StringComparison.CurrentCultureIgnoreCase))))
                        {
                            continue;
                        }
        ...
        public GetReportLsResponse GetReportLsResponse(string numLs)
        {
            ...
            return new GetReportLsResponse
                { Base64File = Convert.ToBase64String(bytes), FileName = codedReport.OutputFileName, Result = new Result { Code = "0" } };
        }
            ...
            else
            {
                ...
                try
                {
                    ...
                    var persAccList = summaryDomain.GetAll()
                        ...
                        .Where(x => x.PersonalAccount.Room.RealityObject.Id == mkdId)
