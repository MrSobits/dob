namespace Bars.Gkh.Reforma.Impl.Logger
{
    ...
    public class SyncLogger : ISyncLogger
    {
        ...
        {
            ExplicitSessionScope.CallInNewScope(() => this.FlushItems(force));
        }
                        ...
                        () =>
                            {
                                foreach (var item in items)
                                {
                                    ...
                                    {
                                        using (var file = new ZipFile(Encoding.UTF8) { AlternateEncoding = Encoding.Default, CompressionLevel = CompressionLevel.BestCompression })
                                        {
                                            var i = 0;
