Ext.define('B4.controller.realityobj.DecisionProtocol', {
    ...
    onChangeFundFormatDecision: function (field, newValue) {
        ...
        if (newValue == B4.enums.CrFundFormationDecisionType.SpecialAccount) {
            accOwnerDecForm.down('checkbox').setValue(true);
            accOwnerDecForm.down('checkbox').enable();
        } else {
            accOwnerDecForm.down('checkbox').setValue(false);
        ...
        if (!protocolId || protocolId == 0) {
            ...
            B4.Ajax.request({
                ...
                if (id > 0) {
                    model.load(id, {
                        success: function (newRec) {
                            win = Ext.ComponentQuery.query('nskdecisionaddconfirm')[0];
                            if (!win) {
    ...
    onAccountOwnerDecisionTypeSelect: function (selField, val) {
        var win = selField.up('nskdecisionedit'),
            ...
            isCreditOrgCheckbox = win.down('form[entity=CreditOrgDecision] checkbox[name=IsChecked]');
        ...
        if (isCreditOrgCheckbox) {
            ...
            isCreditOrgCheckbox.setDisabled(val === B4.enums.AccountOwnerDecisionType.RegOp);
        }
    },
