    ...
    public class SyncLogService : ISyncLogService
    {
        ...
        public IDataResult ReplayAction(BaseParams baseParams)
        {
            ...
            try
            {
                ...
                ExplicitSessionScope.CallInNewScope(() => result = this.ReplayActionInternal(action.Name, Encoding.UTF8.GetString(action.Parameters), action.Session.SessionId));
                if (result.Success)
                {
        ...
        {
            var provider = this.Container.Resolve<ISyncProvider>(new { sessionId = Guid.Parse(sessionId) });
