Ext.define('B4.controller.dict.ViolationGji', {
    ...
    views: [
        ...
        'dict.violationgji.ViolationGjiMunicipalityGrid'
    ],
    mixins: {
    ...
    aspects: [
        ...
        },
        {
            ...
            otherActions: function (actions) {
                actions['#violationGjiEditWindow #cbTypeMunicipality'] = { 'change': { fn: this.onChangeTypeMunicipality, scope: this } };
            },
            onChangeTypeMunicipality: function (field, newValue) {
                var form = this.getForm(),
                    violationGjiMunicipality = form.down('#violationGjiMunicipalityGrid');
                if (newValue == 50) {
                    violationGjiMunicipality.setDisabled(true);
                } else {
                    violationGjiMunicipality.setDisabled(false);
                }
                debugger;
            },
            ...
            onSaveSuccess: function (asp, record) {
                ...
                //var docMunGrid = form.down('#violationGjiMunicipalityGrid');
                //var docMunStore = docMunGrid.getStore();
                //var docMun = new Array();
               // for (var index = 0; index < docM)
            ...
            listeners: {
                aftersetformdata: function (asp, record) {
                    asp.controller.setViolationId(record.getId());
                    asp.controller.getStore('dict.ViolationGjiMunicipality').load();
                    var violationId = record.getId();
                    asp.controller.violationId = violationId;
                    ...
                    //var docMunGridBtn = Ext.ComponentQuery.query('#violationGjiMunicipalityGrid b4addbutton')[0];
                    //docMunGridBtn.setDisabled(!record.getId());

                    //if (violationId > 0) {
                    //    asp.controller.mask('Загрузка', asp.controller.getMainComponent());
                    //    B4.Ajax.request({
                    //        method: 'POST',
                    //        url: B4.Url.action('ListMunicipality', 'ViolationGji'),
                    //        params: {
                    //            violationId: asp.controller.violationId
                    //        }
        ...
        {
            xtype: 'gkhgridmultiselectwindowaspect',
            name: 'violationActionsMunicipalityAspect',
            gridSelector: '#violationGjiMunicipalityGrid',
            storeName: 'dict.ViolationGjiMunicipality',
            modelName: 'dict.ViolationGjiMunicipality',
            multiSelectWindow: 'SelectWindow.MultiSelectWindow',
            multiSelectWindowSelector: '#violationGjiMunicipalityMultiSelectWindow',
            ...
            listeners: {
                getdata: function (asp, records) {
                    ...
                    if (recordIds[0] > 0) {
                        asp.controller.mask('Сохранение', asp.controller.getMainComponent());
                        B4.Ajax.request(B4.Url.action('AddMunicipalites', 'ViolationGjiMunicipality', {
                            objectIds: recordIds,
                            violationId: asp.controller.violationId
                        })).next(function () {
                            asp.controller.unmask();
    ...
    init: function () {
        ...
        me.getStore('dict.ViolationGjiMunicipality').on('beforeload', me.onBeforeLoad, me);
        ...
        me.control({
            ...
            "#violationGjiMunicipalityGrid b4updatebutton": {
                click: function (btn) {
                    var store = btn.up('violationGjiMunicipalityGrid').getStore();
                    store.load();
                }
            }
        });
    ...
    setViolationId: function (id) {
        ...
        var storeViolMun = this.getStore('dict.ViolationGjiMunicipality');
