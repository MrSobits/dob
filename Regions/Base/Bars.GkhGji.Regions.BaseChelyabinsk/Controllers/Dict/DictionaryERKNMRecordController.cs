namespace Bars.GkhGji.Regions.BaseChelyabinsk.Controllers.Protocol197
{
    ...
    public class DictionaryERKNMRecordController : B4.Alt.DataController<DictionaryERKNMRecord>
	{
		...
		{
			var result = this.Container.Resolve<IDictionaryERKNMRecordService>().GetInfo(documentId);
			return result.Success ? new JsonNetResult(result.Data) : JsonNetResult.Failure(result.Message);
		}

		public ActionResult AddRequirements(BaseParams baseParams)
		{
			var result = this.Container.Resolve<IDictionaryERKNMRecordService>().AddRequirements(baseParams);
			return result.Success ? new JsonNetResult(result.Data) : JsonNetResult.Failure(result.Message);
		}

		public ActionResult AddDirections(BaseParams baseParams)
		{
			var result = this.Container.Resolve<IDictionaryERKNMRecordService>().AddDirections(baseParams);
			return result.Success ? new JsonNetResult(result.Data) : JsonNetResult.Failure(result.Message);
		}
