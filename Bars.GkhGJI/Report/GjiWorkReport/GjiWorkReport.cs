namespace Bars.GkhGji.Report
{
    ...
    public class GjiWorkReport : BasePrintForm
    {
        ...
        {
            get { return "Инспекторская деятельнсость"; }
        }
                ...
                foreach (var group in dataDic)
                {
                    ...
                    foreach (var municip in group.Value.OrderBy(x => x.Key.municipalityName))
                    {
                        ...
                        foreach (var realtyObjectDisposalProxy in municip.Value.OrderBy(x => x.disposalId).ThenBy(x => x.roAddress))
                        {
                            ...
                            // Вид обследования
                            var planned = (disposal.kindCheckCode == TypeCheck.PlannedExit)
                            ...
                            if (protocolProxyDict.ContainsKey(disposalId) && protocolProxyDict[disposalId].ContainsKey(homeId))
                            {
                                ...
                                disposalSection["param101"] = resolutionsCount > 0 ? resolutionsCount.ToStr() : string.Empty;
                                ...
                                foreach (var resolution in resolutions)
                                {
                                    if (planned)
                                    {
                                        resDic["102"] += resolution.CourtAdmPenalty;
                                        resDic["103"] += resolution.CourtFinished;
                                        resDic["104"] += resolution.GjiAdmPenalty;
                                        resDic["105"] += resolution.GjiFinished;

                                        penaltyDic["110"] += resolution.CourtSumPenalty;
                                        penaltyDic["111"] += resolution.GjiSumPenalty;
                                        penaltyDic["112"] += resolution.CourtSumPenaltyPaid;
                                        penaltyDic["113"] += resolution.GjiSumPenaltyPaid;
                                    }
                                    else
                                    {
                                        resDic["106"] += resolution.CourtAdmPenalty;
                                        resDic["107"] += resolution.CourtFinished;
                                        resDic["108"] += resolution.GjiAdmPenalty;
                                        resDic["109"] += resolution.GjiFinished;

                                        penaltyDic["114"] += resolution.CourtSumPenalty;
                                        penaltyDic["115"] += resolution.GjiSumPenalty;
                                        penaltyDic["116"] += resolution.CourtSumPenaltyPaid;
                                        penaltyDic["117"] += resolution.GjiSumPenaltyPaid;
                                    }
                                }
        ...
        private Dictionary<long, Dictionary<long, ProtocolProxy>> GetProtocolData(List<long> realtyObjIds, out IQueryable<long> protocolIds)
        {
            ...
            try
            {
                ...
                var protocolArticleLawDict =
                    ...
                    .Select(x => new { x.Protocol.Id, x.ArticleLaw.OMS })
                    .AsEnumerable()
                    .GroupBy(x => x.Id)
                    .ToDictionary(x => x.Key, x => x.Select(y => y.OMS).ToList());
