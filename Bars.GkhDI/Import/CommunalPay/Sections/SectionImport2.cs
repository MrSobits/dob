namespace Bars.GkhDi.Import.Sections
{
    ...
    public class SectionImport2 : ISectionImport
    {
        public string Name
        {
            get
            {
                return "Импорт из комплат секция #2";
            }
        }
        ...
        public void ImportSection(ImportParams importParams)
        {
            ...
            foreach (var section2Record in sectionsData.Section2)
            {
                ...
                {
                    logImport.Warn(this.Name, string.Format("Для дома с кодом ЕРЦ {0} и упр. организацией с инн {1} нет договора управления в данном периоде", section2Record.CodeErc, inn));
                    continue;
                }
                ...
                {
                    logImport.Error(Name, string.Format("Для дома с кодом ЕРЦ {0} и упр. организацией с инн {1} нет договора управления в данном периоде", section2Record.CodeErc, inn));
                    continue;
                }
                ...
                {
                    logImport.Info(this.Name, string.Format("Добавлен дом {0} в договор {1}", realityObject.FiasAddressName, manorgBaseContract.DocumentNumber));
