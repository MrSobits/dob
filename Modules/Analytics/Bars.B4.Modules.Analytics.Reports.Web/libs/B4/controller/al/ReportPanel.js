Ext.define('B4.controller.al.ReportPanel', {
    ...
    buildParamsView: function (id, select, historyId) {
        ...
        B4.Ajax.request({
            ...
            panel.add({
                ...
                name: 'format',
                value: B4.enums.al.ReportPrintFormat.xlsx,
        ...
        if (validateMessage) {
            ...
            B4.Ajax.request({
                ...
                if (result.taskedReport) {
                    me.taskResult(me, result.taskId, tab, button);
                } else {
                    linkContainer.add(
                    [
                        {
    ...
    taskResult: function (controller, taskId, tab, button) {
        var linkContainer = tab.down('#downloadLink'),
            pb = tab.down('progressbar');
                ...
                items: [
                    {
                        ...
                        items: [
                            {
                                xtype: 'label',
                                html: '<h1 class="descTitle">Задача успешно поставлена в очередь</h1>'
                            }
                        ]
                    },
                    {
                        xtype: 'container',
                        html: '<div class="warningXlsxReport">Проследуйте в раздел задач для отслеживания процесса формирования отчёта</div>'
                    }
                ]
            }
        ]);
