namespace Bars.GkhCr.Report
{
	...
	public class BuildContractCrReport : GkhBaseStimulReport
    {
		...
		public override void PrepareReport(ReportParams reportParams)
		{
			...
			Report["АдресДома"] = string.Format("{0}, {1}, {2}, {3}, {4}, {5}, {6}",
			...
			Report["НомерДома"] = string.Format("{0}, {1}, {2}, {3}",
				objectCr.RealityObject.FiasAddress.House,
				objectCr.RealityObject.FiasAddress.Letter,
				objectCr.RealityObject.FiasAddress.Housing,
				objectCr.RealityObject.FiasAddress.Building);

			Report["НомерДоговора"] = buildContract.DocumentNum;
			Report["ДатаДоговора"] = buildContract.DocumentDateFrom.ToDateString();
			...
			if (regop != null)
			{
				Report["РегОператор"] = regop.Contragent.Name;
				...
				if (contact != null)
				{
					Report["Руководитель"] = contact.FullName;
                }
			}

			if (buildContract.Builder != null)
			{
				Report["Подрядчик"] = buildContract.Builder.Contragent.Name;
            }

			Report["ПротоколКвалОтбора"] = string.Format("№ {0} от {1}", 
			...
			Report["Работы"] = works.AggregateWithSeparator(", ");
			Report["СредстваСобственников"] = buildContract.OwnerMeans.ToStr();
			Report["СредстваБюджетов"] = (buildContract.BudgetMo + buildContract.BudgetSubject + buildContract.FundMeans).ToString();
		    ...
		    if (winnerByLot != null)
		    {
                Report ["ДатаПротокола"] = winnerByLot.Lot.ContractDate.ToDateString();
            }
            Report ["СтоимостьРаботЦифрами"] = buildContract.Sum.ToString();
            Report ["ДатаНачалаРаботПоДоговору"] = buildContract.DateStartWork.ToDateString();
            Report ["ДатаОкончанияРаботПоДоговору"] = buildContract.DateEndWork.ToDateString();
