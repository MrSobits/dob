    ...
    public class UpdateConditionHouseAndPersAccsTask : BaseTask
    {
        ...
        public override void Execute(DynamicDictionary paramDictionary)
        {
            ExplicitSessionScope.CallInNewScope(() =>
            {
                ...
                try
                {
                    logManager.Info("Запуск задания обработки состояния и типа дома");
                    ...
                    try
                    {
                        this.Container.InTransaction(() =>
                        {
                            ...
                            logManager.Info("Закрытие ЛС в связи со сменой состояния дома завершено");
                        });
                    }
                    catch (Exception e)
                    {
                        logManager.Error("Ошибка при закрытии лицевых счетов в связи со сменой состояния дома", e);
                    }
                    ...
                    foreach (var realityObject in realityObjects)
                    {
                        ...
                        {
                            logManager.Error($"Ошибка во время смены статусов ЛС во время обработки состояния дома ({realityObject.Address})", exception);
                        }
                    }

                    logManager.Info("Завершение задания обработки состояния и типа дома");
                }
                catch (Exception exception)
                {
