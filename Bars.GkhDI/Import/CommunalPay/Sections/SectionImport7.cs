namespace Bars.GkhDi.Import.Sections
{
    ...
    public class SectionImport7 : ISectionImport
    {
        public string Name
        {
            get
            {
                return "Импорт из комплат секция #7";
            }
        }
        ...
        public void ImportSection(ImportParams importParams)
        {
            ...
            var periodDi = this.Container.Resolve<IDomainService<PeriodDi>>().Get(importParams.PeriodDiId);
            if (periodDi == null)
            {
            ...
            var disclosureInfo =
                this.Container.Resolve<IDomainService<DisclosureInfo>>()
                    .GetAll()
                ...
                foreach (var section7Record in sectionsData.Section7)
                {
                    switch (section7Record.DocumentType)
                    {
                        ...
                        case "Базовый перечень показателей качества содержания, эксплуатации и технического обслуживания жилых зданий":
                            {
                                document.FileServiceApartment = FileCreater.Create(importParams.zipName, section7Record.File, fileinfoService, fileManager);
                        ...
                        case "Приложение к бухгалтерскому балансу":
                            {
                                finDoc.BookkepingBalanceAnnex = FileCreater.Create(importParams.zipName, section7Record.File, fileinfoService, fileManager);
                ...
                foreach (var realityObjId in realityObjIds)
                {
                    switch (section7Record.DocumentType)
                    {
                        ...
                        case "Протокол собрания № 4":
                            {
                                var documentsRo = documentsRoServiceDict.ContainsKey(realityObjId) ? documentsRoServiceDict[realityObjId].FirstOrDefault(x => x.Year == currentYear - 1) : null;
                        ...
                        case "Протокол собрания № 8":
                            {
                                var documentsRo = documentsRoServiceDict.ContainsKey(realityObjId) ? documentsRoServiceDict[realityObjId].FirstOrDefault(x => x.Year == currentYear) : null;
