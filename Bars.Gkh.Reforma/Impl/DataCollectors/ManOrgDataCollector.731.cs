    ...
    public partial class ManOrgDataCollector
    {
        ...
        private void FillProtocolDocs(List<ICollectedFile<CompanyProfileData>> collectedFiles, DisclosureInfo disclosureInfo, ReportingPeriodDict period)
        {
            ...
            try
            {
                ...
                var robjectFilter =
                    manOrgContractService.GetAll()
                                         .Where(x => x.ManOrgContract.ManagingOrganization.Id == disclosureInfo.ManagingOrganization.Id)
                                         .Select(x => new { x.RealityObject.Id, x.ManOrgContract.StartDate, x.ManOrgContract.EndDate })
                                         .Where(x => x.StartDate <= periodEnd)
                                         .Where(x => !x.EndDate.HasValue || x.EndDate >= periodStart)
                                         .Select(x => x.Id);

                var protocols =
                    roDocsService.GetAll()
                                 .Where(x => robjectFilter.Contains(x.RealityObject.Id) && x.Year >= periodStart.Value.Year - 1 && x.Year <= periodEnd.Value.Year && x.File != null && x.File.Size > 0)
                                 .Select(x => x.File)
                                 .ToArray();
