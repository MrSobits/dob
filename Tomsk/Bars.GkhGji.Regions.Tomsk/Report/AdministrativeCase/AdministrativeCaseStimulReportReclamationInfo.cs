     ...
     
    {
        ...
        public override void PrepareReport(ReportParams reportParams)
        {
            ...
            try
            {
                ...
                {
                    this.Report["ДолжностьИнспектораДела"] = adminCase.Inspector.Position;
                    this.Report["ИнспекторДела"] = adminCase.Inspector.Fio;
                    ...
                    this.Report["ИнициалыИнспекторДела"] = string.Format("{0}.{1} {2}", name[0], patronymic[0], surname);
                }

                this.Report["ОбъектНедвижимости"] =
                    adminCase.RealityObject != null
                        ? adminCase.RealityObject.Address
                        : string.Empty;

                this.Report["Вопрос"] = adminCase.DescriptionQuestion;
                this.Report["Установил"] = this.DescriptionDomain.GetAll().FirstOrDefault(x => x.AdministrativeCase.Id == adminCase.Id).Return(x => Encoding.UTF8.GetString(x.DescriptionSet))
                                      ?? adminCase.DescriptionSet;
                this.Report["НомерДела"] = adminCase.DocumentNumber;
                ...
                this.Report["СтатьяЗакона"] = articleLawName;

                this.Report["ИдентификаторДокументаГЖИ"] = this.AdminCaseId.ToString();
                ...
                this.Report.RegData("ПредоставляемыеДокументы", providedDocuments);

                this.Report["Основание"] = adminCase.TypeAdminCaseBase.GetDisplayName();

                this.Report["Документ"] = this.NameDocument(adminCase);
