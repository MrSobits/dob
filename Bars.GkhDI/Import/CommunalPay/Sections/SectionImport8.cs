namespace Bars.GkhDi.Import.Sections
{
    ...
    public class SectionImport8 : ISectionImport
    {
        public string Name
        {
            get
            {
                return "Импорт из комплат секция #8";
            }
        }
        ...
        public void ImportSection(ImportParams importParams)
        {
            ...
            var positionList = this.Container.Resolve<IDomainService<Position>>().GetAll().ToList();
            ...
            foreach (var section8Record in sectionsData.Section8)
            {
                ...
                var position = positionList.FirstOrDefault(x => x.Code == positionCode);
                if (position == null)
                {
                ...
                var fullName = string.Format(
                    "{0} {1} {2}", section8Record.Surname, section8Record.Name, section8Record.Patronomyc);
