    ...
    public class ObjectCrService : IObjectCrService
    {
        ...
        public IDataResult SendFKRToGZHIMail(BaseParams baseParams, Int64 taskId)
        {
            ...
            var files = RealityObjectImageDomain.GetAll()
                .Where(x=> x.RealityObject == realityObject && x.ImagesGroup == Gkh.Enums.ImagesGroup.MOPWasted && x.ObjectCreateDate > DateTime.Now.AddMonths(-1))
        ...
        private void SendToGZHI(StiReport repo, FileInfo archive, RealityObject entity)
        {
            ...
            try
            {
                ...
                m.Body = $"<h3>Фонд капитального ремонта сообщает, что обнаружено захламление мест общего пользования по адресу {entity.Address}. Обращение и подтверждающие материалы по вложениях.</h3>";
                m.IsBodyHtml = true;
