    ...
    public class SuggestionService : ISuggestionService
    {
        ...
        public Rubric[] GetRubrics()
        {
            return
                Container.Resolve<IDomainService<Entities.Suggestion.Rubric>>().GetAll()
                    .Select(x => new Rubric { Id = x.Id, Name = x.Name }).OrderByDescending(x=> x.Id)
                    .ToArray();
        }
        ...
        public long CreateSuggestion(Suggestion suggestion)
        {
            ...
            var logOperation = new LogOperation
            {
                ...
                OperationType = LogOperationType.AppealReg,
                User = user,
                EndDate = DateTime.Now,
                Comment = suggestion != null? $"{suggestion.ApplicantFio} {suggestion.ApplicantEmail}": "Обращение пустое"
            };
            ...
            if (message.Length > 0)
            {
                ...
                var ppDomain = Container.ResolveDomain<Entities.Suggestion.ProblemPlace>();
                var tpDomain = Container.ResolveDomain<Entities.Suggestion.SugTypeProblem>();

            try
            {
                Entities.Suggestion.ProblemPlace problemPlace;
                Entities.Suggestion.SugTypeProblem typeProblem;
                if (suggestion.ProblemPlace.Id > 0 && ppDomain.GetAll().FirstOrDefault(x=> x.Id == suggestion.ProblemPlace.Id) != null )
                {
                    problemPlace =
                ...
                else
                {
                    ...
                    ppDomain.Save(problemPlace);
                }
                if (suggestion.ProblemPlace.Id > 0 && tpDomain.GetAll().FirstOrDefault(x => x.Id == suggestion.ProblemPlace.Id) != null)
                {
                    typeProblem =
                        tpDomain.Load(suggestion.ProblemPlace.Id);
                }
                else
                {
                    typeProblem = null;
                ...
                if (suggestion.Files != null && suggestion.Files.Length > 0)
                {
        ...
        public Suggestion GetSuggestion(long id)
        {
            ...
            try
            {
                ...
                return new Suggestion
                {
                    ...
                    ApplicantPhone = citizenSuggestion.ApplicantPhone,
                    ApplicantEmail = citizenSuggestion.ApplicantEmail,
                    ProblemPlace = citizenSuggestion.ProblemPlace != null? new ProblemPlace
                    {
                        Id = citizenSuggestion.ProblemPlace.Id,
                        Name = citizenSuggestion.ProblemPlace.Name
                    }:null,
