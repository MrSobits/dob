        ...
        public override IDataResult AfterUpdateAction(IDomainService<RealityObject> service, RealityObject entity)
        {
            try
            {
                if (entity.ConditionHouse == ConditionHouse.Emergency || entity.ConditionHouse == ConditionHouse.Razed)
                {
                    ...
                    bool res = this.PersAccOperationService.MassClosingAccounts(
                        new[] { entity.Id },
                    ...
                    if (!res)
                    {
                        return Failure("Ошибка массового закрытия лицевых счетов");
                    }
                    else
                    {
                        var dataPersAcc = BasePersonalAccountDomain.GetAll()
                            .Where(x => x.Room.RealityObject.Id == entity.Id)
                            .Where(x => x.State.Name == "Открыт").Count();
                        if (dataPersAcc > 0)
                        {
                            return Failure("Ошибка массового закрытия лицевых счетов! В системе осталось " + dataPersAcc + " лицевых счетов");
                        }
                    }
                }
                else
