namespace Bars.GkhGji.Entities
{
    ...
    public class Prescription : DocumentGji
    {
        ...
        [JsonIgnore]
        private readonly ICollection<PrescriptionCloseDoc> _closeDocs;

        public Prescription()
        {
            _closeDocs = new List<PrescriptionCloseDoc>();
        }
        ...
        [JsonIgnore]
        public virtual IEnumerable<PrescriptionCloseDoc> CloseDocs
        {
            get
            {
                return _closeDocs;
            ...
            {
                _closeDocs.Clear();
                foreach (var closeDoc in value)
                {
                    _closeDocs.Add(closeDoc);
                }
            }
        }
        ...
        public virtual bool ContainsCloseAct()
        {
            return _closeDocs.Any(x => x.DocType == PrescriptionDocType.Act);
        }

        public virtual bool IsClosed()
        {
            return Closed == YesNoNotSet.Yes;
        }

        public virtual bool IsRemindersObsolete()
        {
            return this.IsClosed() &&
                   (this.ContainsCloseAct() ||
                    (this.CloseReason == PrescriptionCloseReason.ManOrgChanged ||
                     this.CloseReason == PrescriptionCloseReason.OrganizationLiquidated));
        }
    }
}