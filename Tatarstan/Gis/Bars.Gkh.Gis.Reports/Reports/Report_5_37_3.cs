namespace Bars.Gkh.Gis.Reports.Reports
{
    ...
    public class Report_5_37_3 : StimulReportDynamicExcel
    {
        ...
        {
            const string sqlMaster = @"select headbank, a.nzp_headbank, sum(count_ls) as count_ls,
                sum(sum_oplat) as sum_oplat
                from t1 a, public.s_head_bank s
                where a.nzp_headbank=s.nzp_headbank
                group by 1,2
                order by 1";

            const string sqlDetail = @"select nzp_headbank, (case when (paysource < 0) or (paysource > 7) then 99
...
//            session.CreateSQLQuery("SELECT public.report_5_37(month, year, month, year);").ExecuteUpdate();
            ...
            using (var transaction = session.BeginTransaction())
            {
                ...
                Report.RegBusinessObject("ПараметрыОтчета", new { StartDate = _startDateReport, FinishDate = _endDateReport, Rajon = "Нижнекамск" });
                ...
                var relationDetail = new StiDataRelation("masterdetail", "masterdetail", "masterdetail", Report.Dictionary.DataSources["master"],
                    Report.Dictionary.DataSources["detail"], new[] { "nzp_headbank" }, new[] { "nzp_headbank" });
        ...
        {
            get { return "Статистика по платежам за период (по банкам)"; }
        }
        ...
        {
            get { return "B4.controller.report.Report_5_37_3"; }
        }
        ...
        {
            get { return Resources.Report_5_37_3; }
        }

        public override Stream GetTemplate()
        {
            return new MemoryStream(Resources.Report_5_37_3);
        }
        ...
        public override void SetUserParams(B4.BaseParams baseParams)
        {
            ...
            _regionId = baseParams.Params["regionId"].ToInt();
        }
    }
}