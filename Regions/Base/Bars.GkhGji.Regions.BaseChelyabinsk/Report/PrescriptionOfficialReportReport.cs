namespace Bars.GkhGji.Regions.BaseChelyabinsk.Report
{
    ...
    public class PrescriptionOfficialReportReport : GkhBaseStimulReport
    {
        ...
        public override void PrepareReport(ReportParams reportParams)
        {
            ...
            try
            {
                Report["СЗ_Номер"] = data.DocumentNumber;
                Report["СЗ_Дата"] = data.DocumentDate.HasValue ? data.DocumentDate.Value.ToString("dd.MM.yyyy") : "";
                Report["Предп_Номер"] = data.Prescription.DocumentNumber;
                Report["Предп_Дата"] = data.Prescription.DocumentDate.HasValue ? data.Prescription.DocumentDate.Value.ToString("dd.MM.yyyy") : "";
                Report["ИнспекторРод"] = data.Inspector!= null? data.Inspector.FioAblative:"";
                Report["Инспектор"] = data.Inspector != null ? data.Inspector.Fio : "";
                if (executants != null)
                {
                    //Report["Начальник"] = executant.PositionDative + " " + executant.FioDative;
                    //Report["НачальникФИО"] = executant.Fio;
                    Report["Начальник"] = executants.Executant.PositionDative + " " + executants.Executant.FioDative;
                    Report["НачальникФИО"] = executants.Executant.Fio;
                }
                ...
                Report["ОтИнспектора"] = data.Inspector != null ? data.Inspector.PositionAccusative + " " + data.Inspector.FioAblative : "";
                Report["Контрагент_Наименование"] = data.Prescription.Contragent != null? data.Prescription.Contragent.Name:"";
                Report["Контрагент_ИНН"] = data.Prescription.Contragent != null ? data.Prescription.Contragent.Inn : "";
                Report["Контрагент_ОГРН"] = data.Prescription.Contragent != null ? data.Prescription.Contragent.Ogrn : "";
                Report["Выполнение"] = data.YesNo == Gkh.Enums.YesNo.Yes? "Нарушения устранены": "Нарушения не устранены";
                Report["Адрес"] = realityAddresses;
                Report["Площадь"] = Decimal.Round(realityArea,2).ToString();
