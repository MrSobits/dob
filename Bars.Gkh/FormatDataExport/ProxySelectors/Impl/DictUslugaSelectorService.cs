                ...
                templateServiceRepository))
            {
                ...
                var comContractServices = comContractServiceRepository.GetAll()
                    .Select(x => new DictUslugaProxy
                    {
                        ...
                        IsNotSortOrder = 1,
                        CommunalServiceType = this.GetCommunalServiceType(x.CommunalResource),
                ...
                var templateServices = templateServiceRepository.GetAll()
                    ...
                    .Select(x =>
                    {
                        var communalService = x.Type == TypeGroupServiceDi.Communal
                            ? this.GetCommunalServiceType(x.CommunalResourceType)
                            : null;

                        return new DictUslugaProxy
                        {
                            Id = x.Id,
                            Name = x.Name,
                            Type = this.GetTypeService(x.Type),
                            ServiceType = 1,
                            CommunalServiceType = communalService,
