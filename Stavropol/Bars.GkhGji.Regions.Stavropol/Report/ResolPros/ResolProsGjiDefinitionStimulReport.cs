namespace Bars.GkhGji.Regions.Stavropol.Report.ResolPros
{
    ...
    public class ResolProsGjiDefinitionStimulReport : GjiBaseStimulReport
    {
        ...
        private void FillCommonInfo()
        {
            ...
            Report["ДатаПостановления"] = _resolPros.DocumentDate.HasValue
               ? _resolPros.DocumentDate.Value.ToShortDateString()
               : string.Empty;

            Report["НомерПостановления"] = _resolPros.DocumentNumber;

            if (_resolPros.Contragent != null)
            {
                Report["Контрагент"] = _resolPros.Contragent.Name;
                Report["АдресКонтрагента"] = _resolPros.Contragent.FiasJuridicalAddress != null
		...
		private void FillPostponeCaselInfo()
        {
            ...
            {
				Report["ОбластьПравонарушения"] = "в области охраны собственности";
            }
            else if (articles.Any(p => industryActicles.Any(n => p == n)))
            {
				Report["ОбластьПравонарушения"] = "в промышленности, строительстве и энергетике";
            }
            else if (articles.Any(p => againstOrderArticles.Any(n => p == n)))
            {
				Report["ОбластьПравонарушения"] = "против порядка управления";
            }
            else if (articles.Any(p => againstPublicOrderArticles.Any(n => p == n)))
            {
				Report["ОбластьПравонарушения"] = "посягающее на общественный порядок и общественную безопасность";
            }
            ...
            Report["ДатаИВремяРассмотренияДела"] = string.Format("{0:dd MMMM yyyy} в {1:HH} час. {1:mm} мин.",
				date, time);
        }
