namespace Bars.Gkh.Gis.DomainService.Analysis.Impl
{
    ...
    public class MultipleAnalysisService : IMultipleAnalysisService
    {
        ...
        public Stream GetExcelReport(MultipleAnalysisTemplate template, DateTime date)
        {
            ...
            report.Load(Resources.MultipleAnalysis);
            report.Dictionary.DataSources.Clear();
            report.RegData("Data", dataSet);
            report.Dictionary.Synchronize();
            report.RegBusinessObject("ПараметрыОтчета", new { totalHouseCount });
            report.Dictionary.Relations.Clear();
            var relationDetail = new StiDataRelation("DataRow", "DataRow", "DataRow", report.Dictionary.DataSources["Rows"],
                    report.Dictionary.DataSources["Data"], new[] { "Id" }, new[] { "RowId" });
        ...
        {
            const int sleep = 1000 * 60; // минута
            Thread.Sleep(10000);
            while (true)
            {
                    ...
                    () =>
                        {
                            ...
                            var templates = MultipleAnalysisTemplateRepository
                                .GetAll()
                                .ToList()
                                .Where(
                                    x =>
                                        x.LastFormDate.HasValue &&
                                        x.LastFormDate.Value < date &&
                                        date.Day == x.FormDay
                                        || !x.LastFormDate.HasValue && x.FormDay <= date.Day)
                                .ToList();

                            templates.ForEach(x =>
                            {
                                var stream = GetExcelReport(x, prevMonth);
                                var send = SendEmail(x, stream, prevMonth);
                                if (!send)
                                {
                                    return;
                                }
                                x.LastFormDate = new DateTime(date.Year, date.Month, x.FormDay);
                                MultipleAnalysisTemplateRepository.Update(x);
                            });
