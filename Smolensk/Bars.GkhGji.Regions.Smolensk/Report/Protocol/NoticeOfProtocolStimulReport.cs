namespace Bars.GkhGji.Regions.Smolensk.Report.ProtocolGji
{
    ...
    public class NoticeOfProtocolStimulReport : GjiBaseStimulReport
    {
        ...
        protected void FillProtocolData()
        {
            ...
            Report["ДатаУведомления"] = Protocol.NoticeDocDate.HasValue ? Protocol.NoticeDocDate.Value.ToShortDateString() : string.Empty;
            Report["НомерУведомления"] = Protocol.NoticeDocNumber;
            ...
            if (Protocol.Executant != null)
            {
                ...
                if (firstExecutantCodeList.Contains(executant.Return(x => x.Code)))
                {
                    ...
                    Report["АдресКонтрагентаФакт"] = contragent.Return(x => x.FactAddress);
                }

                else if (secondExecutantCodeList.Contains(executant.Return(x => x.Code)))
                {
                    Report["УправОрг"] = Protocol.PhysicalPerson;
                    Report["АдресКонтрагентаФакт"] = Protocol.PhysicalPersonInfo;
                }
            }
        ...
        protected void FillProtocolInspectors()
        {
            ...
            using (Container.Using(docinspDomain))
            {
                ...
                if (inspectors.Any())
                {
                    ...
                    Report["КодИнспектора"] = firstInspector.Position;
