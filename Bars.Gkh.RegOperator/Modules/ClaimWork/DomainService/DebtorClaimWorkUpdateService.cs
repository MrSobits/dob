...
*/
        ...
        private class Cache
        {
            ...
            private Dictionary<long, List<TransferDto>> GetTransfers(IQueryable<ClaimWorkAccountDetail> accountDetails)
            {
                ...
                Dictionary<long, long> claimworkAccDict;
                try
                {
                    claimworkAccDict = accountDetails
                        .Select(x => new
                        {
                            x.PersonalAccount.Id,
                            ClaimWorkId = x.ClaimWork.Id
                        })
                        .ToDictionary(x => x.Id, x => x.ClaimWorkId);
                }
                catch (ArgumentException ex)
                {
                    var errorRecords = accountDetails
                        .Select(x => new
                        {
                            x.PersonalAccount.Id,
                        ...
                        .GroupBy(x => x.Id)
                        .Where(x => x.Count() > 1)
                        .SelectMany(x => x.Select(y => $"ЛС '{y.PersonalAccountNum}' - Id ПИР '{y.ClaimWorkId}'"))
                        .AggregateWithSeparator(Environment.NewLine);
                    var errorMessage = $"По одному ЛС ведется несколько ПИР:{Environment.NewLine}{errorRecords}";
