namespace Bars.GkhDi.Import.Sections
{
    ...
    public class SectionImport4 : ISectionImport
    {
        public string Name
        {
            get
            {
                return "Импорт из комплат секция #4";
            }
        }
        ...
        public void ImportSection(ImportParams importParams)
        {
            ...
            var infoReductPayService = this.Container.Resolve<IDomainService<InfoAboutReductionPayment>>();
            var infoReductPayServiceDict = infoReductPayService
                .GetAll()
                .Where(x => x.DisclosureInfoRealityObj.PeriodDi.Id == importParams.PeriodDiId)
                .GroupBy(x => x.DisclosureInfoRealityObj.Id)
                .ToDictionary(x => x.Key);

            var baseServiceDict = this.Container.Resolve<IDomainService<BaseService>>().GetAll()
                .Where(x => x.DisclosureInfoRealityObj.PeriodDi.Id == importParams.PeriodDiId)
                .Select(
                    x =>
                        new
            ...
            foreach (var section4Record in sectionsData.Section4)
            {
                ...
                {
                    logImport.Warn(this.Name, string.Format("Для дома с кодом ЕРЦ {0} и упр. организацией с инн {1} нет договора управления в данном периоде", section4Record.CodeErc, inn));
                    continue;
                }
                ...
                var disclosureInfoRealityObj = disclosureInfoRealityObjServiceDict.ContainsKey(realityObject.Id)
                    ? disclosureInfoRealityObjServiceDict[realityObject.Id] : null;
                ...
                var baseService = baseServiceDict.ContainsKey(disclosureInfoRealityObj.Id) ? baseServiceDict[disclosureInfoRealityObj.Id].FirstOrDefault(x => x.TemplateServiceId == templateService.Id) : null;
                if (baseService == null)
                {
                    logImport.Warn(this.Name, string.Format("Не удалось получить сведение об услуге для услуги с кодом {0}", section4Record.CodeCommunalPay));
                    continue;
                }

                var infoReductPay = infoReductPayServiceDict.ContainsKey(disclosureInfoRealityObj.Id) ? infoReductPayServiceDict[disclosureInfoRealityObj.Id].FirstOrDefault(x => x.BaseService.Id == baseService.Id) : null;

                if (infoReductPay == null)
                {
                    logImport.Info(this.Name, string.Format("Добавлена запись в раздел 'Сведения о снижения плат' для услуги {0}", baseService.TemplateServiceName));
            ...
            this.InTransaction(resultList, infoReductPayService);
        }
