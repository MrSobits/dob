    ...
    public class RealityObjectViewModel : BaseViewModel<Entities.RealityObject>
    {
        ...
        public override IDataResult List(IDomainService<Entities.RealityObject> domain, BaseParams baseParams)
        {
            ...
            /* baseParams метода:
              ...
              Id - пришла строка с идентификаторами домов которые необходимо получить
        ...
        public override IDataResult Get(IDomainService<Entities.RealityObject> domainService, BaseParams baseParams)
        {
            ...
            try
            {
                var typeIds = typeRoDomain.GetAll().Where(y => y.RealityObject.Id == id).Select(x => x.RealEstateType.Id);
                ...
                if (realObjOverhaulDataService != null && rec != null)
                {
                    publishDate = realObjOverhaulDataService.GetPublishDateByRo(rec);
                }
