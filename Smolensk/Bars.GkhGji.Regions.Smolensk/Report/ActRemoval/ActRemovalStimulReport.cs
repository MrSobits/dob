    ...
    public class ActRemovalStimulReport : GjiBaseStimulReport
    {
        ...
        private void PrepareReport()
        {
            ...
            try
            {
                ...
                Report["ДатаАкта"] = actRemoval.DocumentDate.ToDateString("d MMMM yyyy");

                Report["НомерАкта"] = actRemoval.DocumentNumber;
                ...
                Report["РеквизитыПредписаний"] = String.Format("№{0} от {1} г.", prescriptionDocNum, prescriptionDocDate);

                Report["Описание"] = actRemoval.Description;
                ...
                Report["НаселенныйПункт"] = violData
                ...
                if (house != null)
                {
                    Report["ТипДома"] = house.TypeHouse.GetEnumMeta().Display;

                    Report["АдресДома"] = string.Empty;
                    Report["НомерДома"] = string.Empty;

                    
                    if (house.FiasAddress != null)
                    {
                        Report["АдресДома"] = house.FiasAddress.PlaceName + ", " + house.FiasAddress.StreetName;
                        ...
                        Report["НомерДома"] = houseNumber;
                    }

                    Report["ФормаСобственности"] = house.TypeOwnership != null ? house.TypeOwnership.Name : string.Empty;
                }
                ...
                Report["НомераНевыполненныхПунктов"] = linksViolation;

                var inspectorsQuery = docInspDomain.GetAll().Where(x => x.DocumentGji.Id == DocumentId);

                Report["ИнспекторыИКоды"] = inspectorsQuery
                ...
                Report["ИнспекторыАкта"] = inspectorsQuery
                ...
                Report["Отдел"] = zonalInspDomain.GetAll()
                ...
                if (disposal != null)
                {
                    ...
                    {
                        Report["ДатаИНомерРешенияПрокурора"] = "Не требуется";
                    }

                    var kindCheckName = disposal.KindCheck.Return(x => x.Name);
                    Report["ВидПроверки"] = kindCheckName;

                    if (disposal.IssuedDisposal != null)
                    {
                        Report["ДЛВынесшееРаспоряжение"] = disposal.IssuedDisposal.Position;
                        Report["ФИОВынесшееПриказ"] = disposal.IssuedDisposal.Fio;
                        Report["РуководительРП"] = (!string.IsNullOrEmpty(disposal.IssuedDisposal.PositionGenitive)
                    ...
                    Report["ВидОбследованияРП"] = GetTypeCheckAblative(disposal.KindCheck);
                    if (disposal.DateStart.HasValue && disposal.DateEnd.HasValue)
                    {
                        ...
                        Report["ПродолжительностьПроверки"] = countDays;
                    }
                ...
                if (act != null)
                {
                    ...
                    {
                        Report["УправОрг"] = act.Inspection.Contragent.Name;
                        Report["ИННУправОрг"] = act.Inspection.Contragent.Inn;
                        Report["УправОргРП"] = act.Inspection.Contragent.Name;
                        Report["СокрУправОрг"] = act.Inspection.Contragent.ShortName;
                        ...
                        {
                            Report["РуководительЮЛДолжность"] = string.Format("{0} {1} {2} {3}",
                                headContragent.Position.Return(x => x.Name),
                                headContragent.Surname,
                                headContragent.Name,
                                headContragent.Patronymic);

                            Report["РуководительЮЛ"] = string.Format("{0} {1} {2}",
                                headContragent.Surname,
                                headContragent.Name,
                                headContragent.Patronymic);
                        }
                        ...
                        {
                            Report["ЮрАдресКонтрагента"] = act.Inspection.Contragent.JuridicalAddress;
                        }

                        Report["АдресКонтрагентаФакт"] =
                    ...
                    Report["Ознакомлен"] = !string.IsNullOrEmpty(witnessIsFamiliar)
                        ? witnessIsFamiliar.TrimEnd(new[] {',', ' '})
                        : string.Empty;

                    //Собираем дату и время проведения проверки

                    Report["ДатаВремяПроверки"] = actCheckPeriodDomain.GetAll()
                        .Where(x => x.ActCheck.Id == act.Id)
                ...
                Report["ПродолжительностьПроверкиДЧ"] = allTimesResult;
        ...
        private void PrepareJurPersonReport()
        {
            ...
            try
            {
                ...
                Report["НомерАкта"] = string.Format("{0} от {1}",
                    actRemoval.Return(x => x.DocumentNumber),
                    actRemoval.DocumentDate.ToDateString());

                Report["ПроверПлощадь"] = act.Area.HasValue ? act.Area.Value.ToStr() : string.Empty;
                ...
                Report["РеквизитыПредписаний"] = docPrescriptionNames.AggregateWithSeparator(", ");
                Report["РеквизитыПротоколов"] = docProtocolNames.Count > 0
                ...
                Report["ВремяСоставленияАкта"] = lastCheckDate != null
                    ? lastCheckDate.DateEnd.ToDateString("HH-mm")
                    : string.Empty;

                Report["ДатаВремяПроверки"] = actCheckPeriods
                    .Select(x =>
                    {
                ...
                Report["ПродолжительностьПроверкиДЧ"] = string.Format("{0} д., {1} ч.", daysCount,
                    Math.Round(periodTimeSum.TotalHours));

                Report["ИнспекторыИКоды"] = docInspDomain.GetAll()
                ...
                Report["ИнспекторыАкта"] = docInspDomain.GetAll()
                    .Where(x => x.DocumentGji.Id == act.Id)
                    .Select(x => new {x.Inspector.Fio, x.Inspector.Position})
                    .ToList()
                    .Aggregate(string.Empty, (x, y) =>
                ...
                if (act.Inspection.Contragent != null)
                {
                    ...
                    Report["УправОргРП"] = contragent.Name;
                    ...
                    if (headContragent != null)
                    {
                        Report["РуководительЮЛДолжность"] = string.Format(
                            "{0} {1} {2} {3}",
                        ...
                        Report["РуководительЮЛ"] = string.Format(
                            "{0} {1} {2}",
                            headContragent.Surname,
                            headContragent.Name,
                            headContragent.Patronymic);
                ...
                if (actRoList.Count > 0)
                {
                    ...
                    {
                        Report["НомерДома"] = realtyObject.FiasAddress.House;
                        Report["АдресДома"] = realtyObject.FiasAddress.PlaceName
                                              + ", "
                    ...
                    if (actRoList.Count == 1)
                    {
                        ...
                        Report["УлицаДом"] = string.Format("{0}, {1}, д.{2}{3} ", realtyObject.FiasAddress.PlaceName,
                            realtyObject.FiasAddress.StreetName, realtyObject.FiasAddress.House, housing);
                        Report["ДомаИАдреса"] = realtyObject.FiasAddress.AddressName;
                        Report["Описание"] = descriptions.Get(firstRecord.Id) ?? firstRecord.Description;
                    }
                    else
                    {
                        ...
                        Report["ДомаИАдреса"] = string.Format("{0}, {1}. ",
                ...
                if (sum.HasValue)
                {
                    Report["ОбщаяПлощадьСумма"] = sum.Value.ToString();
                }
                ...
                if (disposal != null)
                {
                    ...
                    Report["ВидПроверки"] = kindCheckName;
                    Report["ВидОбследования"] = kindCheckName;
                    ...
                    if (disposal.IssuedDisposal != null)
                    {
                        if (!string.IsNullOrEmpty(disposal.IssuedDisposal.PositionGenitive))
                        {
                            ...
                            Report["КодИнспектораТП"] = disposal.IssuedDisposal.PositionAblative.ToLower();
                        }

                        Report["РуководительРП"] = disposal.IssuedDisposal.FioGenitive;
                        Report["РуководительТП"] = disposal.IssuedDisposal.FioAblative;
                    }
                    ...
                    Report["Эксперты"] = disposalExpertDomain.GetAll()
                        .Where(x => x.Disposal.Id == disposal.Id)
                        .Select(x => x.Expert)
                    ...
                    if (disposal.IssuedDisposal != null)
                    {
                        Report["КодРуководителяФИО"] = string.Format(
                            "{0} - {1}",
                    ...
                    Report.RegData("ПредоставляемыеДокументы", listDisposalProvDocs);
                ...
                Report["СсылкиНаПунктыНормативныхАктов"] = linksViolation;
        ...
        private void RegViolationsDataSource(Prescription prescription)
        {
            ...
            using (Container.Using(prescriptionViolDomain, prescrViolDescrDomain))
            {
                if (prescription != null)
                {
                    ...
                    Report.RegData("Нарушения", prescriptionViols.Select(x => new
                    {
                        НомерПП = ++i,
        ...
        private void RegViolationDescriptions(Prescription prescription)
        {
            ...
            using (Container.Using(violGroupDomain, violPiontDomain))
            {
                ...
                Report.RegData("ОписаниеНарушений", data.Select(x => new
                {
                    Пункт = x.PointCodes,
