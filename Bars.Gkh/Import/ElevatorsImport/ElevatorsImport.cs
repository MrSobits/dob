namespace Bars.Gkh.Import.ElevatorsImport
{
    ...
    public class ElevatorsImport : GkhImportBase
    {
        ...
        private ILogImportManager _logImportManager;
        ...
        public override ImportResult Import(BaseParams baseParams)
        {
            ...
            try
            {
                ...
                if (roIds.Count > 0)
                {
                    ...
                    {
                        Logger.Error("Ошибка импорта лифтов", ex);
                    }
                }
            ...
            _logImportManager.Add(file, _logImport);
            _logImportManager.Save();

            message = string.IsNullOrEmpty(message) ? _logImportManager.GetInfo() : message;

            var status = _logImportManager.CountError > 0
                             ? StatusImport.CompletedWithError
                             : StatusImport.CompletedWithoutError;

            var logField = _logImportManager.LogFileId;

            _logImport = null;
            _logImportManager = null;
        ...
        private void InitLog(string fileName)
        {
            ...
            _logImportManager = Container.Resolve<ILogImportManager>();
            _logImportManager.FileNameWithoutExtention = fileName;
            _logImportManager.UploadDate = DateTime.Now;

            _logImport = Container.ResolveAll<ILogImport>().FirstOrDefault(x => x.Key == MainLogImportInfo.Key);
            if (_logImport == null)
            {
        ...
        private void AddLog(int rowNum, string message, bool success = true, Exception ex = null)
        {
            ...
            if (ex.IsNotNull())
            {
                Logger.Error("Ошибка в импорте лифтов", ex);
            }
        }
        ...
        private void WriteLogs()
        {
            foreach (var log in _logDict.OrderBy(x => x.Key))
            {
                ...
                foreach (var rowLog in log.Value)
                {
                    if (rowLog.Key)
                    {
                        _logImport.Info(rowNumber, string.Join(". ", rowLog.Value));
                        _logImport.CountAddedRows++;
                    }
                    else
                    {
                        _logImport.Warn(rowNumber, string.Join(". ", rowLog.Value));
                    }
                }
            }
        }
