namespace Bars.GkhDi.Import.Sections
{
    ...
    public class SectionImport12 : ISectionImport
    {
        public string Name
        {
            get { return "Импорт из комплат секция #12"; }
        }
        ...
        public void ImportSection(ImportParams importParams)
        {
            ...
            var templateServiceDict = this.Container.Resolve<IDomainService<TemplateService>>()
                .GetAll()
                .Where(x => x.Code != null)
                .GroupBy(x => x.Code)
            ...
            foreach (var section12Record in sectionsData.Section12)
            {
                ...
                {
                    logImport.Warn(this.Name, string.Format("Для дома с кодом ЕРЦ {0} и упр. организацией с инн {1} нет договора управления в данном периоде", section12Record.CodeErc, inn));
                    continue;
                }

                // Получаем раскрытие в доме (или если нету создаем его)
                var disclosureInfoRealityObj = disclosureInfoRealityObjServiceDict.ContainsKey(realityObject.Id) ? disclosureInfoRealityObjServiceDict[realityObject.Id] : null;
                ...
                {
                    var communal = communalServiceDict.ContainsKey(disclosureInfoRealityObj.Id) ? communalServiceDict[disclosureInfoRealityObj.Id].FirstOrDefault(x => x.TemplateService.Id == templateService.Id) : null;
                    ...
                    var tariffForRso = tariffForRsoDict.ContainsKey(communal.Id) ? tariffForRsoDict[communal.Id].FirstOrDefault(x => x.Cost == section12Record.Tariff) : null;
