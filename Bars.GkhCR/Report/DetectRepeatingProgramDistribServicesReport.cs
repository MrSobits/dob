                                           ...
                                           .ToDictionary(
                                               ...
                                               y => y.GroupBy(z => z.ProgramCrId)
                                                     .ToDictionary(
                                                        ...
                                                        w => 
                                                        { 
                                                            var sumWork = w.Where(z => z.TypeWork == TypeWork.Work).Sum(z => z.Sum).ToDecimal();
                                                            var sumService = w.Where(z => z.TypeWork == TypeWork.Service).Sum(z => z.Sum).ToDecimal();

                                                            var tmpDict = w.GroupBy(u => u.WorkCode)
                                                                    .ToDictionary(
                                                                        u => u.Key,
                                                            ...
                                                            if (sumService > 0)
                                                            {
                                                                var dopSum = dictSumWork.ToDictionary(k => k.Key, v => v.Value * (sumService / sumWork));
                                                                var tmpSum = dopSum.Sum(v => v.Value).ToDecimal();
                                                                var err = Math.Round(sumService - tmpSum, 2);
            ...
            foreach (var municipality in municipalityDict.OrderBy(x => x.Value))
            {
                ...
                foreach (var realtyObject in municipalityData.OrderBy(x => x.Key))
                {
                    ...
                    foreach (var work in listWorkDistinctRo)
                    {
                        ...
                        if (cr > 1)
                        {
                            foreach (var progCr in workDataByCodeByProgramDict)
                            {
