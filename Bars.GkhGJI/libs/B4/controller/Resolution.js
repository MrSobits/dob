    ...
    aspects: [
        ...
        },
        {
            ...
            */
            ...
            otherActions: function (actions) {
                ...
                actions[this.editPanelSelector + ' #cbPaided'] = { 'change': { fn: this.onChangecbPaided, scope: this } };
                actions['#resolutionEditPanel #sfConcederationResult'] = { 'change': { fn: this.onChangesfConcederationResult, scope: this } }; 
                //actions[this.editPanelSelector + ' #dfIndividualPerson'] = { 'beforeload': { fn: this.onBeforeLoadIndividualPerson, scope: this } };
            },

            onChangesfConcederationResult: function (field, newValue) {
                debugger;
                var me = this,
                panel = me.getPanel(),
                    taAdditionalInfo = panel.down('#taAdditionalInfo'),
                    taComment = panel.down('#taComment');
                if (newValue && newValue.Name.includes('о прекращении')) {
                  //  taAdditionalInfo.show();
                    taComment.show();
                }
                else {
                   // taAdditionalInfo.hide();
                    taComment.hide();
                }
            },

            onChangecbPaided: function (field, value) {

                var dfProtocol205Date = field.up('panel').down('#dfProtocol205Date');
                var dfPaymentDate = field.up('panel').down('#dfPaymentDate');

                if (value === 10) {
                    dfPaymentDate.allowBlank = false;
                    dfPaymentDate.show();
                    dfProtocol205Date.allowBlank = true;
                    dfProtocol205Date.hide();
                }
                else if (value === 20) {
                    dfProtocol205Date.allowBlank = false;
                    dfProtocol205Date.show();
                    dfPaymentDate.allowBlank = true;
                    dfPaymentDate.hide();
                }
                else {
                    dfProtocol205Date.allowBlank = true;
            ...
            onBeforeLoadIndividualPerson: function (field, options, store) {
                options = options || {};
                options.params = options.params || {};
                options.params.documentId = this.controller.params.documentId;
            ...
            onAfterSetPanelData: function (asp, rec, panel) {
              ...
              //  this.setTypeExecutantPermission(rec.get('Executant'));
                ...
                me.controller.getAspect('resolutionBlobTextAspect').doInjection();
        ...
        },
        {
            ...
            getParentPanel: function () {
                return this.componentQuery('#resolutionEditPanel');//resolutionEditPanel
            },
