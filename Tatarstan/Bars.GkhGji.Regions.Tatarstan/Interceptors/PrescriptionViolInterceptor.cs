        ...
        protected override IDataResult CheckDatePlanRemoval(PrescriptionViol entity, DateTime? actCheckDate)
        {
            ...
            try
            {
                ...
                if (configIsLimitDate)
                {
                    ...
                    if (entity.DatePlanRemoval.HasValue && actCheckDate.HasValue)
                    {
                        if (configPeriodKind == PeriodKind.Day)
                        {
                            ...
                            {
                                return this.Failure($"Срок устранения нарушения не должен превышать {0} д.", configPeriod);
                            }
                        }
                        ...
                        if (configPeriodKind == PeriodKind.Month)
                        {
                            ...
                            {
                                return this.Failure($"Срок устранения нарушения не должен превышать {0} м.", configPeriod);
                            }
                        }
                    }
                }
                else
                {
                    ...
                    {
                        return this.Failure($"Срок устранения нарушения не должен превышать {0} м.", PrescriptionViolInterceptor.DefaultPlanRemoval);
                    }
                }
