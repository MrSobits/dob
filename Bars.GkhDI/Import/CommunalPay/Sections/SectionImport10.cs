namespace Bars.GkhDi.Import.Sections
{
    ...
    public class SectionImport10 : ISectionImport
    {
        public string Name
        {
            get
            {
                return "Импорт из комплат секция #10";
            }
        }
        ...
        public void ImportSection(ImportParams importParams)
        {
            ...
            else
            {
                ...
                foreach (var section10Record in sectionsData.Section10)
                {
                    ...
                    {
                        logImport.Warn(this.Name, string.Format("Для дома с кодом ЕРЦ {0} и упр. организацией с инн {1} нет договора управления в данном периоде", section10Record.CodeErc, inn));
                        continue;
                    }

                    var infoContract = infoContractServiceList.FirstOrDefault(x => x.RealityObject.Id == realityObject.Id && x.Number == section10Record.Number);
