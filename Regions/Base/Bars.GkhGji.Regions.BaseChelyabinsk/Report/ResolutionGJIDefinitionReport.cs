...
namespace Bars.GkhGji.Regions.BaseChelyabinsk.Report
{
    ...
    public class ResolutionGjiDefinitionReport : GjiBaseReport, IComissionMeetingCodedReport
    {
        ...
        public string ReportId => "ResolutionDefinition";
        public string OutputFileName { get; set; } = "Определение постановления";

        public void GenerateMassReport()
        {

            var DocumentGjiDomain = this.Container.Resolve<IDomainService<ResolutionDefinition>>();
            try
            {
                if (this.DocId.IsEmpty())
                {
                    throw new Exception("Не найдено определение");
                }
                ...
                this.OutputFileName =
                    $"Определение постановления {ownerInfo.Resolution.DocumentNumber}  ({ownerInfo.DocumentNum} - {ownerInfo.DocumentDate.Value.ToString("dd.MM.yyyy")}).xls";
        ...
        public override void SetUserParams(UserParamsValues userParamsValues)
        {
            ...
            if (DefinitionId == 0)
            {
                DefinitionId = userParamsValues.GetValue<object>("DocumentId").ToLong();
            }
        }
