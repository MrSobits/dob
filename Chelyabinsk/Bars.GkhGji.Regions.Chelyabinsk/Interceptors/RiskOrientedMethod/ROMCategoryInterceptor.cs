    ...
    class ROMCategoryInterceptor : EmptyDomainInterceptor<ROMCategory>
    {
        ...
        public override IDataResult AfterCreateAction(IDomainService<ROMCategory> service, ROMCategory entity)
        {
            try
            {
                ...
                {
                    var servOgr = ServiceOrganizationDomain.GetAll()
                    ...
                    //смотрим договора на обслуживание
                    if (servOgr != null)
                    {
                        manOrgRO.AddRange(
                            ServiceOrgRealityObjectContractDomain.GetAll()
                            .Where(x=> x.ServOrgContract.ServOrg.Contragent == entity.Contragent)
                            .Where(x=> !roids.Contains(x.RealityObject.Id))
                            .Where(x=> x.ServOrgContract.DateStart <= entity.CalcDate && (!x.ServOrgContract.DateEnd.HasValue || x.ServOrgContract.DateEnd.Value>entity.CalcDate))
                            .Select(x=> new
                            {
                                x.Id,
                                x.RealityObject,
                                StartDate = x.ServOrgContract.DateStart
                            }).ToList()
                            );
                    }
              ...
              .Select(x => x.Protocol197.Id).Count();
                ...
                if (areaTotal > 0)
                {
                    ...
                    if (entity.KindKND == Enums.KindKND.HousingSupervision)
                    {
                        if (result <= 0.3m)
                        {
                            entity.RiskCategory = Enums.RiskCategory.Low;
                        }
                        if (result > 0.3m && result <= 1)
                        {
                            entity.RiskCategory = Enums.RiskCategory.Moderate;
                        }
                        if (result > 1 && result <= 3.5m)
                        {
                            entity.RiskCategory = Enums.RiskCategory.Average;
                        }
                        if (result > 3.5m)
                        {
                            entity.RiskCategory = Enums.RiskCategory.High;
                        }
                    }
