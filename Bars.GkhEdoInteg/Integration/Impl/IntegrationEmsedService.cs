    ...
    public class IntegrationEmsedService : IEmsedService
    {
        ...
        public bool Send(BaseParams baseParams, out string msg)
        {
            ...
            using (var client = this.GetClient())
            {
                ...
                if (appealCitsId == 0)
                {
                    this.LogManager.Error("Отправка данных в ЭДО. Идентификатор обращения граждан отсутствует", new Exception());
                    return false;
                }
                ...
                if (appealCits == null)
                {
                    this.LogManager.Error("Отправка данных в ЭДО. Обращение граждан не найдено", new Exception());
                    return false;
                }
                ...
                {
                    this.LogManager.Error("Отправка данных в ЭДО. Не удалось отправить данные", exp);
        ...
        public bool LoadData(DynamicDictionary dict, out string message)
        {
            ...
            {
                this.LogManager.Error($"Интеграция Эдо. {message}");
                return false;
            }
            ...
            catch (Exception exp)
            {
                ...
                this.LogManager.Error($"Интеграция Эдо. {message}", exp);
                return false;
            }
                ...
                try
                {
                    for (var curStartDay = dateToStartLoad; curStartDay < DateTime.Now; curStartDay = curStartDay.AddDays(IntegrationEmsedService.DaysToLoad))
                    {
                        ...
                        catch (Exception exp)
                        {
                            ...
                            this.LogManager.Error(errorMsg, exp);
        ...
        private AnswerEdo RequestEdo(DateTime actualDate, ref string message, DateTime? untilDate = null)
        {
            ...
            using (var client = this.GetClient())
            {
                ...
                {
                    this.LogManager.Error("Интеграция с ЭДО. Не удалось получить данные", exp);
            ...
            {
                this.LogManager.Error("Интеграция с ЭДО. Не удалось сохранить JSON для лога", exp);
            }
            ...
            IFileManager fileManager)
        {
            ...
            catch (OutOfMemoryException exp)
            {
                ...
                this.LogManager.Error(message, exp);
            }
            catch (Exception exp)
            {
                message += $"Ошибка загрузки документа для {appealCits.NumberGji}. {exp.Message};";
                this.LogManager.Error($"Интеграция с ЭДО. {message}", exp);
            }
        }
        ...
        private string GetDnsid(ref string msg, WebClient client, string key)
        {
            ...
            try
            {
                var uriDnsid = this.GetUri(key);
                var data = client.UploadData(new Uri(uriDnsid, UriKind.RelativeOrAbsolute), new byte[0]);
                var dnsidJson = client.Encoding.GetString(data);
                var obj = JsonConvert.DeserializeObject<DnsidResponse>(dnsidJson);
                if (obj.Errors.IsNotEmpty())
                {
                    var edoMsg = obj.Errors.Length > 0 ? string.Join(", ", obj.Errors) : string.Empty;
            ...
            {
                this.LogManager.Error("Отправка данных в ЭДО. Не удалось получить DNSID", exp);
        ...
        private string GetUri(string key)
        {
            ...
            switch (key)
            {
                case "EdoDnsid":
                case "SendEdoDnsid":
                    return $"https://doc.tatar.ru/login.php?api=1&user_id=1703454&password={Convert.ToBase64String(Encoding.ASCII.GetBytes("54321"))}";
        ...
        private AnswerEdo SerealizeData(ref string message, byte[] bytes)
        {
            ...
            {
                this.LogManager.Error("Интеграция с ЭДО. Ошибка сериализации данных", exp);
