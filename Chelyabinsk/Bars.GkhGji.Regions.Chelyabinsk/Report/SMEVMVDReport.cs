                ...
                if (file != null)
                {
                    ...
                    if (messageElement != null)
                    {
                        ...
                        if (persSig != null)
                        {
                            ...
                            if (Certificate != null)
                            {
                                Report["PeriodSP"] = $"Действителен с {Certificate.NotBefore.ToShortDateString()} по {Certificate.NotAfter.ToShortDateString()}";
                                Report["TumpSP"] = Certificate.Thumbprint;
                                Report["УЦЭПСП"] = GetCName(Certificate.Issuer);
                                Report["ПолучательЭПСП"] = GetCName(Certificate.Subject);
                            }
                        }
                        ...
                        if (smevSig != null)
                        {
                            ...
                            if (Certificate != null)
                            {
                                Report["PeriodOV"] = $"Действителен с {Certificate.NotBefore.ToShortDateString()} по {Certificate.NotAfter.ToShortDateString()}";
                                Report["TumpOV"] = Certificate.Thumbprint;
                                Report["УЦЭПОВ"] = GetCName(Certificate.Issuer);
                                Report["ПолучательЭПОВ"] = GetCName(Certificate.Subject);
                            }
                        }
                    }
                }
