    ...
    public class TechPassportService : ITechPassportService
    {
        ...
        public IDataResult GetForm(BaseParams baseParams)
        {
            try
            {
                ...
                {
                    comp.Rows = comp.Rows.OrderBy(x => x.Code).ToList();
                    comp.Elements = comp.Elements.OrderBy(x => x.Code).ToList();
                    comp.Columns = comp.Columns.OrderBy(x => x.Order).ThenBy(x => x.Code).ToList();
                }
