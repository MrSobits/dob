        ...
        private PersonalAccountOperationResult DistributeSource(PerformedWorkChargeSource source)
        {
            ...
            foreach (var charge in charges)
            {
                foreach (var walletType in source.GetDistributableWalletTypes())
                {
                    ...
                    if (howMuch != 0)
                    {
                        this.LogManager.DebugFormat("Сумма распределения ({0}): {1}", walletType.GetDisplayName(), howMuch);
            ...
            this.LogManager.DebugFormat("Количество трансферов: {0}", result.Transfers.Count);
