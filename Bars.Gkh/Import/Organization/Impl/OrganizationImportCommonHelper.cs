namespace Bars.Gkh.Import.Organization.Impl
{
    ...
    public class OrganizationImportCommonHelper : IOrganizationImportCommonHelper
    {
        ...
        private Dictionary<long, KeyValuePair<string, string>> realityObjectManOrgDict;
        ...
        public void UpdateRealtyObjectManOrg(long roId, DateTime contractStartDate, string manOrgName, string typeContract)
        {
            ...
            if (this.realityObjectManOrgDict.ContainsKey(roId))
            {
                ...
                realtyObject.ManOrgs = manorgData.Key;
                realtyObject.TypesContract = manorgData.Value;
            }

            if (string.IsNullOrWhiteSpace(realtyObject.ManOrgs))
            {
                realtyObject.ManOrgs = manOrgName;
            }
            else
            {
                if (!realtyObject.ManOrgs.Contains(manOrgName))
                {
                    realtyObject.ManOrgs += ", " + manOrgName;
                }
            }

            if (string.IsNullOrWhiteSpace(realtyObject.TypesContract))
            {
                realtyObject.TypesContract = typeContract;
            }
            else
            {
                if (!realtyObject.TypesContract.Contains(typeContract))
                {
                    realtyObject.TypesContract += ", " + typeContract;
                }
            }
