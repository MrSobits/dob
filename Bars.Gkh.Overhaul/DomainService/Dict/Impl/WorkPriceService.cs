        ...
        where T: WorkPrice, new ()
    {
        ...
        public IDataResult YearList(BaseParams baseParams)
        {
			...
			var years = this.DomainService.GetAll()
				.Where(x => x.Municipality.Id == municipalityId || x.Municipality.ParentMo.Id == municipalityId)
				.AsEnumerable()
				.WhereIf(workPriceMoLevel != WorkpriceMoLevel.Both,
					x => x.Municipality.Level.ToWorkpriceMoLevel(Container) == workPriceMoLevel || (x.Municipality.ParentMo != null && x.Municipality.ParentMo.Level.ToWorkpriceMoLevel(Container) == workPriceMoLevel))
				.Select(x => x.Year)
                .Distinct()
                .AsEnumerable()
                .OrderBy(x => x)
