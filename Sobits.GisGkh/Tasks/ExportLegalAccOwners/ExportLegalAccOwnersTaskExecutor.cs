        ...
        public IDataResult Execute(BaseParams @params, Bars.B4.Modules.Tasks.Common.Contracts.ExecutionContext ctx, IProgressIndicator indicator, CancellationToken ct)
        {
            try
            {
                var legalContragents = LegalAccountOwnerDomain.GetAll()
                    ...
                    .Where(x => x.Contragent.GisGkhVersionGUID == null || x.Contragent.GisGkhVersionGUID == "")
                    .Where(x => x.Contragent.Ogrn != null && x.Contragent.Ogrn != "" && x.Contragent.Ogrn.Trim().Length == 13
                        && x.Contragent.Kpp != null && x.Contragent.Kpp != "" && x.Contragent.Kpp.Trim().Length == 9)
