        ...
        public IDataResult ActualizeFromShortCr(BaseParams baseParams)
        {
            ...
            try
            {
                ...
                var services = workDomain.GetAll()
                    .Where(x => x.TypeWork == TypeWork.Service && (x.Code == serviceTehnadzor || x.Code == servicePsdExpertiza || x.Code == servicePsdRazrabotka))
                    .AsEnumerable()
                    .GroupBy(x => x.Code)
                ...
                var addHistoryQuery =
                    twpHistoryDomain.GetAll().Where(x => x.TypeWorkCr.ObjectCr.ProgramCr.Id == programCrId && x.TypeAction == TypeWorkCrHistoryAction.Creation);
                ...
                // получаем все записи 1 этапа сгруппированные по КЭ нужно тчобы понимат ьпо каким записям бежат ьесли нужно удалить все элементы
                var st1dictByStrEl =
                    st1query.Select(x => new { x.Id, strElId = x.StrElement.Id, roId = x.RealityObject.Id })
                            .AsEnumerable()
                ...
                var st2Query = versSt2Domain.GetAll().Where(x => x.Stage3Version.ProgramVersion.Id == versionId && x.Stage3Version.RealityObject.Municipality.Id == municipalityId);
                ...
                var typeRefDictBySt1 = typeWorkCrRefDomain.GetAll()
                                        .Where(x => refQuery.Any(y => y.Stage1Version.Id == x.Stage1Version.Id))
                                        .Select(x => new {x.Id, st1Id = x.Stage1Version.Id})
                                        .AsEnumerable()
                                        .GroupBy(x => x.st1Id)
                                        .ToDictionary(x => x.Key, y => y.Select(z => z.Id).ToList());

                var shortPrgRecbySt1 = shortPrgRecordDomain.GetAll()
                                        .Where(x => refQuery.Any(y => y.Stage1Version.Id == x.Stage1.Id))
                                        .Select(x => new { x.Id, st1Id = x.Stage1.Id })
                                        .AsEnumerable()
                                        .GroupBy(x => x.st1Id)
                                        .ToDictionary(x => x.Key, y => y.Select(z => z.Id).ToList());
                ...
                foreach (var kvp in dataWorks)
                {
                    ...
                    foreach (var workData in kvp.Value)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                return new BaseDataResult(false, string.Format("Для вида работ {0} нет ни одной работы, связанной с конструктивным элементом. В ДПКР попадают только виды работ, связанные с КЭ и ООИ.", workData.WorkName));
                            }
                            ...
                            foreach (var strEl in StrEls.list)
                            {
                                ...
                                if (roStrElDict.ContainsKey(roStrLeKey))
                                {
                                    ...
                                    CreateStage1(out st1, out twRef, workData.Id, roId, roStrEl.strElId, roStrEl.Id, workData.ShortYearStart, roStrEl.Volume);
                    ...
                    foreach (var workData in kvp.Value)
                    {
                        if (workData.Stage1List != null && workData.Stage1List.Count == 1)
                        {
                            ...
                            if (st1.Id > 0)
                            {
                                ...
                                if (st1.Sum != oldSum || st1.SumService != oldService || st1.Volume != oldVolume || (newYear > 0 && st1.Year != newYear)|| ( !string.IsNullOrEmpty(st3.CommonEstateObjects) && st3.CommonEstateObjects.Contains("Grouping")))
                                {
                                    // добавляем в список на сохранение
                                    if (newYear > 0)
                                    {
                        ...
                        else if (workData.Stage1List != null && workData.Stage1List.Count > 1)
                        {
                            ...
                            foreach (var st1 in workData.Stage1List)
                            {
                                ...
                                if (st1.Id > 0)
                                {
                                    ...
                                    if (st3.CorrectYear != workData.ShortYearStart || st1DictHistoryCreation.ContainsKey(st1.Id) || ( !string.IsNullOrEmpty(st3.CommonEstateObjects) && st3.CommonEstateObjects.Contains("Grouping")))
                                    {
                                        newYear = workData.ShortYearStart;
                                    ...
                                    if (st1.Sum != oldSum || st1.SumService != oldService || st1.Volume != oldVolume || (newYear > 0 && st1.Year != newYear))
                                    {
                                        // добавляем в список на сохранение
                                        if (newYear > 0)
                                        {
                            ...
                            case TypeWorkCrReason.NewYear:
                                {
                                    if (typeWorkCrRefDpkr.ContainsKey(work.Id))
                                    {
                                        ...
                                        foreach (var refSt1 in refData.Stage1List)
                                        {
                                            ...
                                            var key = roId + "_" + refSt1.StrElement.Id;
                ...
                var st3NewData = st2List
                                    ...
                                    .ToDictionary(x => x.Key, y => new
                                    {
                                        ...
                                        ceoNames = y.Select(z => z.CommonEstateObject.Name).ToList().Any() ?
                                                        y.Select(z => z.CommonEstateObject.Name).ToList().Distinct().Aggregate((str, res) => !string.IsNullOrEmpty(res) ? res + ", " + str : res) : string.Empty,
                                        volume = y.Sum(z => z.Volume)
                                    });
                ...
                if (!stage3ToSave.Any() && !stage2ToSave.Any() && !stage1ToSave.Any() && !typeWorkStage1ToSave.Any() && !stage1ToDelete.Any() && !stage2ToDelete.Any() && !stage3ToDelete.Any())
                {
                ...
                if (!logRecords.Any())
                {
                    return new BaseDataResult(false, "Логи отсутствуют");
                }
                        ...
                        typeWorkStage1ToSave.Values.ForEach(x =>
                        {
                            typeWorkCrRefDomain.Save(x);
                        });

                        stage1ToDelete.ForEach(id =>
                            {

                                // если есть связи, то удаляем и их
                                if (typeRefDictBySt1.ContainsKey(id))
                                {
                                    foreach (var refId in typeRefDictBySt1[id])
                                    {
                                        typeWorkCrRefDomain.Delete(refId);
                                    }
                                }

                                if (shortPrgRecbySt1.ContainsKey(id))
                                {
                                    foreach (var refId in shortPrgRecbySt1[id])
                                    {
                                        shortPrgRecordDomain.Delete(refId);
                                    }
                                }
                                
                                versSt1Domain.Delete(id);
                        ...
                        stage3ToDelete.ForEach(id =>
                            {
                                versSt3Domain.Delete(id);
                            });
...
// формируем логи актуализации
        ...
        private IDataResult GetNewRecords(BaseParams baseParams, out List<VersionRecord> ver3S, out List<VersionRecordStage2> ver2S, out List<VersionRecordStage1> ver1S, Dictionary<long, RoStrElAfterRepair> afterRepaired = null, List<long> RoStrElList = null, List<string> existRoStrInYear = null)
        {
            ...
            var stagesService = Container.Resolve<IStage2Service>();
                    ...
                    versSt3Domain))
            {
                ...
                var newStructQuery =
                    roStructElService.GetUsedInLongProgram()
                                     .Where(
                                         x =>
                                         x.RealityObject.Municipality.Id == municipalityId
                                         || x.RealityObject.MoSettlement.Id == municipalityId)
                                     .WhereIf(RoStrElList != null, x => RoStrElList.Contains(x.Id) || !versSt1Query.Any(y => y.StructuralElement.Id == x.Id))
                                     .WhereIf(RoStrElList == null, x => !versSt1Query.Any(y => y.StructuralElement.Id == x.Id));
                                         /*
                                     .WhereIf(changedRoStrEls != null, x => changedRoStrEls.Contains(x.Id) || !versSt1Query.Any(y => y.StructuralElement.Id == x.Id))
                                     .WhereIf(changedRoStrEls == null, x => !versSt1Query.Any(y => y.StructuralElement.Id == x.Id));
                                          * */
                ...
                var stage1ToSave = stagesService.GetStage1(
                    actualizeStart, periodEnd, version.Municipality.Id, newStructQuery);
                ...
                IList<RealityObjectStructuralElementInProgrammStage2> stage2ToSave;
                IList<RealityObjectStructuralElementInProgrammStage3> stage3ToSave;
                ...
                stagesService.GetStage2And3(stage1ToSave, out stage2ToSave, out stage3ToSave);
                                ...
                                st2Version = new VersionRecordStage2
                                                 {
                                                     ...
                                                     ObjectCreateDate = DateTime.Now,
                                                     TypeDpkrRecord =
                                                         TypeDpkrRecord.ActualizeAddRecord
                                                 };
                            }
                            else
                            {
                                        ...
                                        new VersionRecordStage1
                                            {
                                                ...
                                                ObjectCreateDate = DateTime.Now,
                                                StrElement =
                                                    stage1.StructuralElement.StructuralElement,
            ...
            using (Container.Using(programVersionDomain, versSt1Domain, versSt2Domain, versSt3Domain, workPriceDomain, structElementWorkDomain))
            {
                ...
                foreach (var st1Rec in versSt1Recs)
                {
                    ...
                    else
                    {
                        ...
                        if (!allPricesExist)
                        {
                            var text = "Отсутсвуют расченки по работам:" + Environment.NewLine;
