namespace Bars.Gkh.Domain.Cache
{
        ...
        where T : class
    {
        ...
        ///     Логгер
        /// </summary>
        protected ILogManager Log
        {
            get
            {
                return this.log ?? (this.log = ApplicationContext.Current.Container.Resolve<ILogManager>());
            }
            set
            {
                this.log = value;
        ...
        public IEntityCache<T> SetLivingTime(TimeSpan timespan)
        {
            ...
            {
                this.Log.DebugFormat("Время жизни кэша {0} установлено в {1}", this.EntityName, timespan);
            }
