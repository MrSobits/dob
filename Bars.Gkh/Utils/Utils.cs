namespace Bars.Gkh.Utils
{
    ...
    public static class Utils
    {
        ...
        public static Municipality GetMunicipality(IWindsorContainer container, FiasAddress address)
        {
            ...
            if (result == null)
            {
                var mcp = service.GetAll().FirstOrDefault(x => x.FiasId == address.PlaceGuidId);
                if (mcp != null)
                {
                    result = mcp;
                }
        ...
        public static Municipality GetMoSettlement(IWindsorContainer container, FiasAddress address)
        {
            ...
            var fias = container.Resolve<IDomainService<Fias>>().FirstOrDefault(x => x.AOGuid == address.PlaceGuidId);
        ...
        /// <returns> Возврат ссылки на муниципальное образование </returns>
        public static Municipality GetSettlementForConstructionObject(IWindsorContainer container, FiasAddress fiasAddress)
        {
            if (fiasAddress == null)
            {
                return null;
            ...
            var fias =
                container.Resolve<IDomainService<Fias>>().FirstOrDefault(x => x.AOGuid == fiasAddress.PlaceGuidId);
        ...
        public static FiassHouseProxy GetFiasProxy(this FiasAddress fiasAddress, IWindsorContainer container)
        {
            ...
            if (fiasAddress.HouseGuid != null)
            {
                ...
                using (container.Using(fiasHouseDomain))
                {
                    ...
                    {
                        parts.Add($"{EnumHelper.GetPrefix(fiasHouse.EstimateStatus)} {fiasHouse.HouseNum}");
                    }
        ...
        public class FiassHouseProxy
        {
            ...
            /// Признак владения
            /// </summary>
            public FiasEstimateStatusEnum EstimateStatus { get; set; }
        }
    }
}