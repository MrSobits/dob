namespace Bars.Gkh.Overhaul.Hmao.DomainService.Impl
{
    ...
    public partial class ActualizeVersionService : IActualizeVersionService
    {
        ...
        public IDataResult AddNewRecords(BaseParams baseParams)
        {
            ...
            var versionActualizeLogDomain = Container.ResolveDomain<VersionActualizeLog>();
        ...
        public IDataResult ActualizeSum(BaseParams baseParams)
        {
            ...
            using (this.Container.Using(programVersionDomain, versSt1Domain, versSt2Domain, versSt3Domain, sessionProvider, 
                workPriceDomain, structElementWorkDomain, realEstService))
            {
                var version = programVersionDomain.Get(versionId);
                ...
                var roQuery = realObjDomain.GetAll()
                    .Where(x => versSt1Domain.GetAll()
                            .Where(y => y.VersionRecordState != VersionRecordState.NonActual)
                ...
                log.Municipality = version.Municipality;
                log.ProgramVersion = version;
                        ...
                        try
                        {
                            ...
                            var st2ForDelete =
                                versSt2Domain.GetAll()
                                    .Where(
                                        x =>
                                            !versSt1Domain.GetAll().Where(y => y.VersionRecordState != VersionRecordState.NonActual).Any(y => y.Stage2Version.Id == x.Id) &&
                                            x.Stage3Version.ProgramVersion.Id == versionId);
            ...
            try
            {
                ...
                var publishYearsList = publishProgRecDomain.GetAll()
                    ...
                    .Join(versSt1Domain.GetAll(),
                        ...
                        (x, y) => new
                        {
                            ...
                            y.Year,
                            y.VersionRecordState
                        })
                    .Where(x => x.VersId == versionId && x.Year > crEndYear)
                    .Where(x => x.VersionRecordState != VersionRecordState.NonActual)
                    .ToList();
                ...
                var realObjInfo = realObjDomain.GetAll()
                            ...
                            .Select(x => new
                            {
                                ...
                                CommissioningYear = x.BuildYear.HasValue ? x.BuildYear.Value : 0
                            })
                            .AsEnumerable()
                            .ToDictionary(x => x.Id);
                ...
                var st1Query = versSt1Domain.GetAll().Where(x => x.VersionRecordState != VersionRecordState.NonActual).Where(y => y.Stage2Version.Stage3Version.ProgramVersion.Id == versionId);
                ...
                if (changedRoStrEls.Any())
                {
                        ...
                        try
                        {
                            allStage1VerRecs.Where(x => changedRoStrEls.Contains(x.Key))
                                .ForEach(x => x.Value.ForEach(y =>
                                {
                                    ...
                                    versSt1Domain.Delete(y.St1Id);
                                    var st2 = stage2Dict[y.St2Id];
                                    st2.Sum = st2.Sum - y.St1Sum;
                                    versSt2Domain.Update(st2);

                                    var st3 = stage3Dict[y.St3Id];

                                    st3.Sum = st3.Sum - y.St1Sum;
                                    versSt3Domain.Update(st3);
                    ...
                    baseParams.Params.Add("FromActualizeYear", true);
                    var dataResult = this.GetNewRecordsForStavropol(baseParams, out ver3S, out ver2S, out ver1S, st1Repaired, forDeleting, publishYears, dictSt2PublishYears);
        ...
        public IQueryable<ActualizeSumEntriesDto> GetActualizeSumEntriesQueryable(BaseParams baseParams)
        {
            ...
            using (this.Container.Using(programVersionDomain, versSt1Domain, versSt2Domain, versSt3Domain, sessionProvider, workPriceDomain, structElementWorkDomain, realEstService))
            {
                ...
                var roQuery = realObjDomain.GetAll()
                    .Where(x => versSt1Domain.GetAll().Where(y => y.VersionRecordState != VersionRecordState.NonActual)
                            .Where(y => y.Stage2Version.Stage3Version.ProgramVersion.Id == versionId)
            ...
            try
            {
                ...
                */
                ...
                foreach (var item in priorityParams.OrderBy(x => x.Value))
                {
                    ...
                    var eval = this.Container.Resolve<IProgrammPriorityParam>(key, new object());
                    if (eval != null)
                    {
                        proxyList2 = eval.Asc
                ...
                var logRecords = dictStage3
                    .Join(
                        ...
                        (y, x) =>
                        {
                            ...
                            {
                                return new ActualizeVersionLogRecord
                                {
        ...
        public IQueryable<VersionRecordStage1> GetDeletedEntriesQueryable(BaseParams baseParams)
        {
            ...
            try
            {
                ...
                return versStage1Domain.GetAll()
                        .Where(x => x.VersionRecordState!= VersionRecordState.NonActual)
                        .Where(x => x.Stage2Version.Stage3Version.ProgramVersion.Id == versionId)
                        .Where(x => x.Stage2Version.Stage3Version.Year >= actualizeStart)
                        ...
                        using (var transaction = session.BeginTransaction())
                        {
                            try
                            {
                                ...
                                foreach (var id in stage2IdsForDelete)
                                {
                                    ...
                                    // удаляем ссылку на второй этап у записи опубликованной программы
                                    session.CreateSQLQuery(string.Format(@"update ovrhl_publish_prg_rec set stage2_id = null where stage2_id = {0} ", id)).ExecuteUpdate();
                        ...
                        foreach (var delId in notExistSt3Ids)
                        {
                            ...
                            if (old.NewYear == 0)
                            {
                                var logRecord = new ActualizeVersionLogRecord
                                {
                                    TypeAction = VersionActualizeType.ActualizeFromShortCr,
                                    Action = "Удаление",
                                    Description = string.Format("Удаление при Актуализации из КПКР"),
                                    Address = old.Address,
                                    Ceo = old.CeoNames,
                                    PlanYear = old.Year,
                                    Volume = old.Volume,
                            ...
                            else
                            {
                                ...
                                //Change TypeAction
                                var duplicate = logRecords.FirstOrDefault(x => x.Ceo == old.CeoNames && x.Address == old.Address && x.PlanYear == old.NewYear);
            ...
            List<string> existRoStrInYear = null)
        {
                    ...
                    roStructElService))
            {
                ...
                var stage1ToSave = longProgramService.GetStage1(actualizeStart,
                    version.Municipality.Id, newStructQuery, null, true);
                    ...
                    if (stage2Dict.ContainsKey(stage3))
                    {
                        ...
                        foreach (var stage2 in st2RecList)
                        {
                            ...
                            else if (stage3Period == 0)
                            {
                                ...
                                ver3.Sum += stage3.Sum;
                                ver3.ObjectEditDate = DateTime.Now;
                                ver3.Changes = string.Format("Добавлено: {0}", DateTime.Now.ToString("dd.MM.yyyy HH:mm"));
        ...
        private class ShortRecordProxy
        {
