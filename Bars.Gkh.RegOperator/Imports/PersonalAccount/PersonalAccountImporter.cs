namespace Bars.Gkh.RegOperator.Imports
{
    ...
    public class PersonalAccountImporter : BaseBillingImporter<BasePersonalAccount>
    {
        ...
        private Dictionary<long, Dictionary<string, Gkh.Entities.Room>> roomDictByRoDict;
        ...
        protected override ValidateResult Validate(StreamReader streamReader)
        {
            ...
            var check = this.CheckHash(origin, controlCode);

            return new ValidateResult(check) { Message = check ? null : "Некорректная контрольная строка" };
        }
        ...
        protected override void SaveRecords(List<BasePersonalAccount> records)
        {
            ...
            this.LogManager.InfoFormat("{0}. SubPersonalAccountService", this.sw.Elapsed);

            this.SubPersonalAccountService?.AddSubPersonalAccount();

            this.LogManager.InfoFormat("{0}. SubPersonalAccountService end", this.sw.Elapsed);

            this.LogManager.InfoFormat("{0}. IVersionedEntityService", this.sw.Elapsed);

            TransactionHelper.InsertInManyTransactions(this.Container, this.logLights, 1000, true, true);

            this.LogManager.InfoFormat("{0}. IVersionedEntityService end", this.sw.Elapsed);
            ...
            this.LogManager.InfoFormat("{0}. this.SessionProvider.CloseCurrentSession()();", this.sw.Elapsed);
        }
            ...
            List<DateTime> periodDates)
        {
            ...
            this.persAccsByRoomDict = this.AccountService.GetAll()
                .Where(x => roomsIds.Contains(x.Room.Id))
           ...
           var realityObjectId = this.RoomService.GetAll()
                .Where(x => roomsIds.Contains(x.Id))
                .Select(x => x.RealityObject.Id).Distinct().ToList();


            this.roomDictByRoDict = this.RoomService.GetAll()
                .Where(x => realityObjectId.Contains(x.RealityObject.Id))
        ...
        {
            this.LogManager.InfoFormat("{0}. Обработка строки: {1}", this.sw.Elapsed, ++this.rowNumber);
            ...
            var ro = this.GetRealityObject(splits[2], splits[1]);
            ...
            var room = this.GetOrCreateRoom(
                ...
                // Appartment (RoomNum)
                splits[7].ToUncheckedInt(),

                // roomsCount 
                splits[9] == "true" ? RoomType.NonLiving : RoomType.Living,
            ...
            switch (acc.ServiceType)
            {
                case PersAccServiceType.NotSelected:
                    if (splits[22] == PersonalAccountImporter.OverhaulCode269 || splits[22] == PersonalAccountImporter.OverhaulCode356)
                    {
            ...
            if (!period.IsClosed || this.closedPeriodImportRight)
            {
                ...
                if (splits[22] == PersonalAccountImporter.OverhaulPenaltyCode || splits[22] == PersonalAccountImporter.RecruitmentPenaltyCode)
                {
            ...
            /* period summary */
            this.LogManager.InfoFormat("{0}. Обработка строки завершена: {1}", this.sw.Elapsed, this.rowNumber);
        }
            ...
            decimal livingArea)
        {
            ...
            Dictionary<string, Gkh.Entities.Room> roRooms = null;

            if (!this.roomDictByRoDict.TryGetValue(ro.Id, out roRooms))
            {
                roRooms = this.roomDictByRoDict[ro.Id] = new Dictionary<string, Gkh.Entities.Room>();
            }

            var room = roRooms.Get(roomNum);
            if (room == null)
            {
        ...
        {
            this.LogManager.InfoFormat("{0}. Сохранение помещений", this.sw.Elapsed);
            ...
            this.LogManager.InfoFormat("{0}. Сохранение помещений завершено", this.sw.Elapsed);
        }
        ...
        {
            this.LogManager.InfoFormat("{0}. Сохранение ЛС", this.sw.Elapsed);
            ...
            this.LogManager.InfoFormat("{0}. Сохранение ЛС завершено", this.sw.Elapsed);
        }
        ...
        protected void SaveUpdateChargePeriods()
        {
            ...
            foreach (var sql in this.GetSqlCommandsNeedToExecute())
            {
        ...
        {
            this.LogManager.InfoFormat("{0}. Сохранение информации по ЛС за период", this.sw.Elapsed);
            ...
            this.LogManager.InfoFormat("{0}. Сохранение информации по ЛС за период завершено", this.sw.Elapsed);
        }
        ...
        {
            this.LogManager.InfoFormat("{0}. Сохранение информации информации по счетам начислений", this.sw.Elapsed);
            this.LogManager.InfoFormat("{0}. Сохранение информации информации по счетам начислений заврешено", this.sw.Elapsed);
            ...
            this.LogManager.InfoFormat("{0}. Их счета начислений", this.sw.Elapsed);
            ...
            this.LogManager.InfoFormat("{0}. Операции счета начислений", this.sw.Elapsed);

            // операции лс по дому, периоду
            var periodSummaries = this.PeriodSummaryService.GetAll()
                .Where(x => municipalitiesId.Contains(x.PersonalAccount.Room.RealityObject.Municipality.Id) && periods.Contains(x.Period.Id))
            ...
            this.LogManager.InfoFormat("{0}. операции лс по дому, периоду", this.sw.Elapsed);

            // берем счет начислений
            foreach (var charAcc in chargeAccounts)
            {
                this.LogManager.InfoFormat("{0}. Счет начислений {1}", this.sw.Elapsed, charAcc.RealityObject.Address);
                ...
                foreach (var chargePeriod in periodSummariesByRo.Keys)
                {
                    ...
                    chargeAccOper.ChargedTotal = summariesPeriod.SafeSum(x => x.ChargedByBaseTariff + x.RecalcByBaseTariff);
        ...
        private IEnumerable<string> GetSqlCommandsNeedToExecute()
        {
            ...
            return this.SessionProvider.GetCurrentSession().CreateSQLQuery(SqlGetNeedToExecute).List<string>();
        }
