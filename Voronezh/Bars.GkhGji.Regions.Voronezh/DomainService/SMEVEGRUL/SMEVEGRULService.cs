...
namespace Bars.GkhGji.Regions.Voronezh.DomainService
{
    ...
    public class SMEVEGRULService : ISMEVEGRULService
    {
        ...
        public bool TryProcessResponse(SMEVEGRUL requestData, GetResponseResponse response, IProgressIndicator indicator = null)
        {
            try
            {
                ...
                else
                {
                    ...
                    //xmlDocument.LoadXml(FNSresponse.);
                    //var ms = new MemoryStream();
                    //xmlDocument.Save(ms);
                    //requestData.XmlFile = _fileManager.SaveFile("ExtractXml", ".xml", ms.ReadAllBytes());
        ...
        private string MakeAddress(XElement address)
        {
            ...
            string cityname = address.Element(fnstNamespace + "Город")?.Attribute("НаимГород")?.Value;
            string citytype = address.Element(fnstNamespace + "Город")?.Attribute("ТипГород")?.Value;
            ...
            string np = address.Element(fnstNamespace + "НаселПункт")?.Attribute("НаимНаселПункт")?.Value;
            ...
            string kvart = address.Attribute("Кварт")?.Value;
            string zpt = ", ";
            if (!string.IsNullOrEmpty(np))
            {
                if (!string.IsNullOrEmpty(kvart))
                {
                    return $"{regiontype} {regionname}, {nptype} {np}, {streettype} {streetname}, {house}, {korpus}, {kvart}";
                }
                return $"{regiontype} {regionname}, {nptype} {np}, {streettype} {streetname}, {house}, {korpus}";
            }
            if (!string.IsNullOrEmpty(cityname))
            {
                if (!string.IsNullOrEmpty(kvart))
                {
                    return $"{regiontype} {regionname}, {citytype} {cityname}, {streettype} {streetname}, {house}, {korpus}, {kvart}";
                }
                return $"{regiontype} {regionname}, {citytype} {cityname}, {streettype} {streetname}, {house}, {korpus}";
            }
            if (!string.IsNullOrEmpty(kvart))
            {
                return $"{regiontype} {regionname}, {citytype} {cityname}, {house}, {korpus}, {kvart}";
            }
            return $"{regiontype} {regionname}, {streettype} {streetname}, {house}, {korpus}";
        }
