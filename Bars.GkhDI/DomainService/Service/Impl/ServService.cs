...
#warning людям со слабой психикой не смотреть, переписать
        ...
        private string[] RequiredCommunalService988 = { "17", "18", "19", "20", "21", "22" };
        private DateTime Period988StartDate = new DateTime(2015, 1, 1);
        private DateTime Period988GjiStartDate = new DateTime(2016, 1, 1);
        ...
        public IDataResult CopyService(BaseParams baseParams)
        {
            ...
            try
            {
                ...
                this.InitCollections(new List<long> { baseService.DisclosureInfoRealityObj.PeriodDi.Id }, initParams, listRealityObjIds);
                ...
                foreach (var roId in realityObjIds)
                {
                    ...
                    switch (copingServiceKind)
                    {
                        case KindServiceDi.Communal:
                            {
                                this.CopyCommunalService(baseService, roId);
                            }
        ...
        public IDataResult GetCountMandatory(BaseParams baseParams)
        {
            try
            {
                ...
                var mandatoryTemplateServiceIdList = this.Container.Resolve<IDomainService<TemplateService>>()
                    .GetAll()
        ...
        public IDataResult CopyServPeriod(BaseParams baseParams)
        {
            ...
            try
            {
                ...
                .Where(x =>
                    ...
                    || ((x.ManOrgContract.StartDate.HasValue && periodFrom.DateStart.HasValue && (periodFrom.DateStart.Value >= x.ManOrgContract.StartDate.Value) || !x.ManOrgContract.StartDate.HasValue)
                        && (x.ManOrgContract.StartDate.HasValue && periodFrom.DateEnd.HasValue
                ...
                this.InitCollections(new List<long> { periodCurrentId, periodFromId }, initParams, realityObjIds);
                ...
                {                                       
                    ...
                    */
                    ...
                    var currentDisclosureInfoRealObj = DictDisclosureInfoRealObj.ContainsKey(realityObjId) ? DictDisclosureInfoRealObj[realityObjId].FirstOrDefault(x => x.PeriodDi.Id == periodFromId) : null;
                    ...
                    if (communalServices != null)
                    {
                        foreach (var communalService in communalServices)
                        {
                            ...
                            if(!existKeys.ContainsKey(key))
                            {
                                if(!this.CopyCommunalService(communalService, realityObjId, periodCurrentId))
                                {
                ...
                finalStr.AppendFormat("Копирование услуг из отчетного периода {0} в отчетный период {1} по УО = {2}\n",
                    periodFrom.Name, periodCurrent.Name, manOrg.Contragent.Name);
        ...
        public IList<string> GetUnfilledMandatoryServsNameList(long disclosureInfoRealityObjId)
        {
            ...
            try
            {
                ...
                var result = templateServiceDomain.GetAll()
                    .Where(x => x.IsMandatory && !fillMandatoryServs.Contains(x.Id))
                    .WhereIf(realObjDi.PeriodDi.DateStart >= this.Period988StartDate, x => x.KindServiceDi != KindServiceDi.Communal)
                    .WhereIf(realObjDi.PeriodDi.DateStart >= this.Period988GjiStartDate, x => x.KindServiceDi != KindServiceDi.CapitalRepair)
                    .WhereIf(realObj != null && realObj.NumberLifts == 0, x => !(x.Code == "27" || x.Code == "28"))
                    .WhereIf(isNotChute, x => x.Code != "7")
                    .Select(x => x.Name)
                    .ToList();

                if (realObjDi.PeriodDi.DateStart >= this.Period988StartDate)
                {
                    var requiredCommunal = templateServiceDomain.GetAll()
                        .Where(x => !fillMandatoryServs.Contains(x.Id))
                        .WhereContains(x => x.Code, this.RequiredCommunalService988)
                        .Select(x => x.Name)
                        .ToList();

                    result.AddRange(requiredCommunal);
                }
            ...
            finally
            {
                ...
                this.Container.Release(templateServiceDomain);
        ...
        public IDictionary<long, string[]> GetUnfilledMandatoryServsNameList(IQueryable<DisclosureInfoRealityObj> diRoQuery)
        {
            ...
            using (this.Container.Using(templateServiceDomain, tehPassportDomain, baseServiceDomain))
            {
                var mandatoryTemplateServices = templateServiceDomain.GetAll()
                    .Where(x => x.IsMandatory || this.RequiredCommunalService988.Contains(x.Code))
                    .ToArray();
                ...
                return diRoQuery
                    ...
                    .ToDictionary(
                        ...
                        y => mandatoryTemplateServices
                                ...
                                .WhereIf(y.DateStart >= this.Period988StartDate, x => x.KindServiceDi != KindServiceDi.Communal || this.RequiredCommunalService988.Contains(x.Code))
                                .WhereIf(y.DateStart >= this.Period988GjiStartDate, x => x.KindServiceDi != KindServiceDi.CapitalRepair)
        ...
        protected virtual void InitCollections(List<long> periodIds, InitParams initParams, List<long> realityObjIds = null)
        {
            ...
            InitWorkRepairList(periodIds, initParams, realityObjIds);
            InitWorkRepairDetail(periodIds, initParams, realityObjIds);
        ...
        protected virtual void InitWorkRepairDetail(List<long> periodIds, InitParams initParams, List<long> realityObjIds = null)
        {
        ...
        protected virtual void InitWorkRepairList(List<long> periodIds, InitParams initParams, List<long> realityObjIds = null)
        {
        ...
        protected virtual bool CopyCommunalService(BaseService oldService, long realObjId = 0, long periodId = 0)
        {
