    ...
    public class ActCheckGjiStimulReport : GjiBaseStimulReport
    {
        ...
        public override void PrepareReport(ReportParams reportParams)
        {
            ...
            this.Report["ДатаАкта"] = act.DocumentDate.HasValue
                                          ...
                                          : string.Empty;
            this.Report["ВремяСоставления"] = actCheckTime != null
            ...
            if (act.Inspection.Contragent != null)
            {
                ...
                {
                    this.Report["Руководитель"] = string.Format(
                        "{0} {1} {2} {3}",
                ...
                this.Report["АдресКонтрагента"] = contragent.FiasFactAddress.Return(x => x.AddressName);
            }
            ...
            if (actCheckRealtyObjects.Length > 0)
            {
                ...
                if (realtyObject != null)
                {
                    ...
                    {
                        this.Report["НаселенныйПункт"] = realtyObject.FiasAddress.PlaceName;
                    }
                }

                if (actCheckRealtyObjects.Count(x => x.RealityObject != null) == 1)
                {
                    this.Report["МатериалСтен"] = realtyObject.WallMaterial != null
                                                      ? realtyObject.WallMaterial.Name
                                                      : string.Empty;
                    this.Report["ОбщаяПлощадь"] = realtyObject.AreaMkd.HasValue
                                                      ? realtyObject.AreaMkd.Value.ToStr()
                                                      : string.Empty;
                    this.Report["Этажей"] = realtyObject.Floors.HasValue
                                                ? realtyObject.Floors.Value.ToStr()
                                                : string.Empty;
                    this.Report["ГодСдачи"] = realtyObject.DateCommissioning.HasValue
                                                  ? realtyObject.DateCommissioning.Value.ToShortDateString()
                                                  : string.Empty;
                    this.Report["ФизИзнос"] = realtyObject.PhysicalWear.HasValue
                                                  ? realtyObject.PhysicalWear.Value.ToStr()
                                                  : string.Empty;
                    this.Report["Подъезд"] = realtyObject.NumberEntrances.HasValue
                                                  ? realtyObject.NumberEntrances.Value.ToStr()
                                                  : string.Empty;
                    this.Report["Кровля"] = realtyObject.RoofingMaterial != null
                                                ? realtyObject.RoofingMaterial.Name
                                                : string.Empty;
                    this.Report["ЧислоКвартир"] = realtyObject.NumberApartments.HasValue
                                                  ? realtyObject.NumberApartments.Value.ToStr()
                                                  : string.Empty;
                    this.Report["ДомАдрес"] = realtyObject.FiasAddress.AddressName;
                    this.Report["Описание"] = actCheckRealtyObjects[0].Description;
                    ...
                    this.Report["ОбщедомовыхХВС"] = tehPassValue == null ? string.Empty : tehPassValue.Value;

                    tehPassValue = queryTehPassValue
                        .Where(x => x.TehPassport.RealityObject.Id == realtyObject.Id)
                        .FirstOrDefault(x => x.FormCode == "Form_3_2_2" && x.CellCode == "1:4");
                    this.Report["ОбщедомовыхГВС"] = tehPassValue == null ? string.Empty : tehPassValue.Value;
                }
                ...
                {
                    this.Report["ТекстНарушения"] = string.Join(". ", violationsNames);
                }
            }
            ...
            if (disposal != null)
            {
                ...
                this.Report["ВидОбследования"] = kindCheckName;
                this.Report["НачалоПериода"] = disposal.DateStart.HasValue
                                                   ? disposal.DateStart.Value.ToString("d MMMM yyyy")
                                                   : string.Empty;
                this.Report["ОкончаниеПериода"] = disposal.DateEnd.HasValue
                                                      ? disposal.DateEnd.Value.ToString("d MMMM yyyy")
                                                      : string.Empty;
                this.Report["СогласСПрокурат"] = disposal.TypeAgreementProsecutor.GetEnumMeta().Display;
            }
            ...
            this.Report["ИнспекторыИКоды"] = inspectors.Aggregate(
                string.Empty,
            ...
            if (actCheckRo != null)
            {
                this.Report["Описание"] = actCheckRo.Description;
            }
            ...
            if (act.Inspection.TypeBase == TypeBase.ProsecutorsClaim)
            {
                ...
                {
                    this.Report["ДлВынесшееТребование"] = baseProsClaim.IssuedClaim;
                }
            }
        ...
        protected virtual void FillCheckPeriods(ReportParams reportParams, DocumentGji doc)
        {
            ...
            this.Report["ДатаВремяПроверки"] = dc1.ToString();
