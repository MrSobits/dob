namespace Bars.Gkh.Overhaul.Tat.DomainService.Impl
{
    ...
    public class DpkrCorrectionService : IDpkrCorrectionService
    {
        ...
        public IDataResult ChangeIndexNumber(BaseParams baseParams)
        {
            ...
            try
            {
                var versionRec = this.VersionRecordDomain.GetAll().FirstOrDefault(x => x.Id == stage3Id);
                var dpkrCorrectionStage2 = this.domain.GetAll().FirstOrDefault(x => x.Id == dpkrCorrectionId);

                if (versionRec == null || dpkrCorrectionStage2 == null)
                {
