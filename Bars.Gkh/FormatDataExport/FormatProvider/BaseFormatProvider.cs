namespace Bars.Gkh.FormatDataExport.FormatProvider
{
    ...
    public abstract class BaseFormatProvider : IExportFormatProvider, IDisposable
    {
        ...
        public event EventHandler<IList<string>> OnAfterExport = delegate { };
        ...
        protected IList<string> ExportedEntityCodeList { get; } = new List<string>();
        ...
        /// Экспорт данных
        /// </summary>
        protected abstract void ExportData(string pathToSave);
        ...
        public void Export(string pathToSave)
        {
            ...
            try
            {
                ...
                formatDataExportFilterService.Init(this.FormatDataExportRoleService.GetProviderType(this.Operator.Role),
        ...
        {
            var config = this.Container.GetGkhConfig<AdministrationConfig>().FormatDataExport;

            if (config.FormatDataExportGeneral.UseIncrementalData)
            {
        ...
        public void InitExportableEntities(IReadOnlyCollection<IExportableEntity> exportableEntities)
        {
            this.ExportableEntities = exportableEntities;
        }
        ...
        protected virtual void InitParams()
        {
            this.LogOperation = new LogOperation
            {
                ...
                Comment = $"Ошибки при экспорте данных со сведениями ЖКХ по формату {this.FormatVersion}"
            };

            ArgumentChecker.NotNull(this.Operator, nameof(this.Operator));
            ...
            this.DataSelectorParams["Info.SenderName"] = this.Operator.User.Name;
            this.DataSelectorParams["Info.Phone"] = this.Operator.Phone;
            this.DataSelectorParams["FormatDataExportProviderType"]
                = this.FormatDataExportRoleService.GetProviderType(this.Operator.Role);

            this.FillOperatorInfo();
        }

        private void FillOperatorInfo()
        {
            var serviceOperatorInspector = this.Container.Resolve<IDomainService<OperatorInspector>>();
            var serviceOperatorContragent = this.Container.Resolve<IDomainService<OperatorContragent>>();
            var serviceOperatorMunicipality = this.Container.Resolve<IDomainService<OperatorMunicipality>>();

            try
            {
                this.DataSelectorParams["OperatorInspectorList"] =
                    serviceOperatorInspector.GetAll()
                        .Where(x => x.Operator.Id == this.Operator.Id)
                        .Select(x => x.Inspector.Id)
                        .ToList();

                this.DataSelectorParams["OperatorContragentList"] =
                    serviceOperatorContragent.GetAll()
                        .Where(x => x.Operator.Id == this.Operator.Id)
                        .Select(x => x.Contragent.Id)
                        .ToList();

                this.DataSelectorParams["OperatorMunicipalityList"] =
            ...
            {
                this.Container.Release(serviceOperatorInspector);
                this.Container.Release(serviceOperatorContragent);
                this.Container.Release(serviceOperatorMunicipality);
            }
        }
        ...
        protected void SaveLogFile(LogOperation logOperation)
        {
            this.Container.UsingForResolved<IFileManager>((container, fileManager) =>
            {
                ...
                using (var writer = new StreamWriter(memoryStream))
                {
                    ...
                    logOperation.LogFile = fileManager.SaveFile(memoryStream, $"{logOperation.OperationType.GetEnumMeta().Display}.json");
                }
            });
