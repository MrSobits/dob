namespace Bars.Gkh.Extensions
{
    ...
    public static class TransactionExtensions
    {
        ...
        {
            ExplicitSessionScope.CallInScope(scope, () => container.InTransaction(action));
        }
        ...
        /// <exception cref="ValidationException"><see cref="IDataResult.Success"/> = false</exception>
        /// <exception cref="DataAccessException"></exception>
        public static TResult InTransactionWithResultInScope<TResult>(this IWindsorContainer container, ILifetimeScope scope, Func<TResult> func) where TResult : IDataResult
        {
            var enterScope = ExplicitSessionScope.EnterScope(scope);
            try
            {
            ...
            {
                ExplicitSessionScope.LeaveScope(enterScope);
            }
        }
        ...
        {
            ExplicitSessionScope.CallInNewScope(() => container.InTransaction(action));
        }
        ...
        {
            using (var newScope = ExplicitSessionScope.CreateNewScope())
            {
        ...
        {
            using (var newScope = ExplicitSessionScope.CreateNewScope(userIdentity))
            {
                return container.InTransactionWithResultInScope(newScope, func);
            }
        }
