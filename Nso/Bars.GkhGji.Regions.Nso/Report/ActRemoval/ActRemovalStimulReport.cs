	...
	public class ActRemovalStimulReport : GkhBaseStimulReport
    {
        ...
        public override void PrepareReport(ReportParams reportParams)
        {
            ...
            try
            {
                ...
                Report["ИдентификаторДокументаГЖИ"] = actRemoval.Id.ToString();
                ...
                Report["ДатаАкта"] = actRemoval.DocumentDate.HasValue
                ...
                Report["НаселенныйПункт"] = violData.Where(x => x.RealityObject != null && x.RealityObject.FiasAddress != null).Select(x => x.RealityObject.FiasAddress.PlaceName).FirstOrDefault();
                ...
                if (roList.Count > 0)
                {
                    Report["Адреса"] = roList.Aggregate(string.Empty, (x, y) => x + (!string.IsNullOrEmpty(x) ? "; " + y : y));    
                }
                ...
                Report["ДатаВремяПроверки"] = actRemovalPeriodDomain.GetAll()
											.Where(x => x.ActRemoval.Id == actRemoval.Id)
                ...
                if (prescription != null)
                {
					...
					{
						Report["УправОрг"] = prescription.Inspection.Contragent.Name;
						Report["ИННУправОрг"] = prescription.Inspection.Contragent.Inn;
						Report["УправОргРП"] = prescription.Inspection.Contragent.Name;
						Report["СокрУправОрг"] = prescription.Inspection.Contragent.ShortName;
						...
						{
							Report["ЮрАдресКонтрагента"] = prescription.Inspection.Contragent.JuridicalAddress;
						}

						if (prescription.Inspection.Contragent.FiasFactAddress != null)
						{
							Report["АдресКонтрагентаФакт"] = prescription.Inspection.Contragent.FiasFactAddress.Return(x => x.AddressName);
						}
					}
                    ...
                    Report.RegData("Нарушения", prescriptionViols);
