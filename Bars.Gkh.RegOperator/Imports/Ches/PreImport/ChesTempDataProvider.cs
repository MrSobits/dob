        ...
        where T : IPeriodImportFileInfo
    {
        ...
        protected virtual void InStatelessImportAction(IDbConnection connection, IDbTransaction transaction)
        {
            ...
            try
            {
                ...
                {
                    var command = new NpgsqlCommand(sqlQuery, connectionToCopy);
                    var copyIn = new NpgsqlCopyIn(command, connectionToCopy, memoryStream);
                    copyIn.Start();
                    copyIn.End();
                }
            ...
            catch (NpgsqlException exception)
            {
                var errorMsg = $"Данные файла {this.FileInfo.FileData.FileName} не прошли валидацию при импорте в таблицу {this.TableName}{Environment.NewLine}"
                    ...
                    + $"Информация об ошибке: {exception.BaseMessage}{Environment.NewLine}"
                    + $"Строка: {exception.Line}{Environment.NewLine}"
                    + $"Детали ошибки: {exception.Where}";

                ApplicationContext.Current.Container.Resolve<ILogManager>().Error(errorMsg, exception);
        ...
        private Stream InStatelessExportAction(IDbConnection connection)
        {
            ...
            {
                var command = new NpgsqlCommand(sqlQuery, connectionToCopy);
                var copyOut = new NpgsqlCopyOut(command, connectionToCopy);
                copyOut.Start();

                var copyOutStream = copyOut.CopyStream;
                var buf = copyOut.Read; // забираем первую строку
                memoryStream = new MemoryStream();

                memoryStream.Write(buf, 0, buf.Length);

                int readLength;
                while ((readLength = copyOutStream.Read(buf, 0, buf.Length)) > 0)
                {
                    memoryStream.Write(buf, 0, readLength);
                }
