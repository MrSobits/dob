...
namespace Bars.Gkh.Controllers
{
    ...
    public class CitizenSuggestionController : FileStorageDataController<CitizenSuggestion>
    {
        ...
        {
            var ros = Container.Resolve<ICitizenSuggestionService>();
        ...
        public ActionResult ListExecutorZonalInspection(BaseParams baseParams) 
        {
            var result = (ListDataResult)this.Resolve<ICitizenSuggestionService>().ListExecutorZonalInspection(baseParams);
            return result.Success ? new JsonListResult((IList)result.Data, result.TotalCount) : JsonNetResult.Failure(result.Message);
        }

        public ActionResult ListInspector(BaseParams baseParams)
        {
            var result = (ListDataResult)this.Resolve<ICitizenSuggestionService>().ListInspector(baseParams);
            return result.Success ? new JsonListResult((IList)result.Data, result.TotalCount) : JsonNetResult.Failure(result.Message);
        }
