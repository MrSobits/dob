    ...
    public class LongProgramService : ILongProgramService
    {
        ...
        public IDataResult CreateDpkrForPublish(BaseParams baseParams)
        {
            ...
            try
            {
                ...
                var oldRoInProgramDict = this.PublishedProgramRecordDomain.GetAll()
                   .Where(x => versions.Any(y => y == x.PublishedProgram.ProgramVersion))
                   .Where(x => x.RealityObject != null)
                   .Select(
                       x => new
                       {
                           x.PublishedProgram.ProgramVersion,
                           RoId = x.RealityObject.Id
                       })
                   .AsEnumerable()
                   .GroupBy(x => x.ProgramVersion)
                   .ToDictionary(x => x.Key, y => y.Select(x => x.RoId).Distinct().ToList());
                ...
                var newRoInProgramDict = new Dictionary<PublishedProgram, List<long>>();
                ...
                foreach (var programVersion in versions)
                {
                    ...
                    newRoInProgramDict.Add(publishProg, new List<long>());
                    ...
                    if (tempCorrection != null)
                    {
                        foreach (var rec in tempCorrection)
                        {
                            ...
                            if (!newRoInProgramDict[publishProg].Contains(rec.RoId))
                            {
                                newRoInProgramDict[publishProg].Add(rec.RoId);
                            }
                ...
                foreach (var newProgram in newRoInProgramDict)
                {
                    newProgram.Key.TotalRoCount = newProgram.Value.Count;

                    if (oldRoInProgramDict.ContainsKey(newProgram.Key.ProgramVersion))
                    {    
                        newProgram.Key.ExcludedRoCount = oldRoInProgramDict[newProgram.Key.ProgramVersion].Except(newProgram.Value).Count();
                        newProgram.Key.IncludedRoCount = newProgram.Value.Except(oldRoInProgramDict[newProgram.Key.ProgramVersion]).Count();
                    }
                }
