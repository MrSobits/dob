namespace Bars.Gkh.Overhaul.Tat.DomainService.Impl
{
    ...
    public class Stage2Service : IStage2Service
    {
        ...
        public IDomainService<CommonEstateObject> CeoDomainService { get; set; }
        ...
        public IEnumerable<RealityObjectStructuralElementInProgramm> GetStage1(int startYear, int endYear, long municipalityId, IQueryable<RealityObjectStructuralElement> roStructElQuery = null)
        {
                ...
                foreach (var objectElement in data)
                {
                    ...
                    foreach (var formulaParam in formulaParams)
                    {
                        ...
                        else if (!string.IsNullOrEmpty(param.ValueResolverCode))
                        {
                            ...
                            var resolver = Container.Resolve<IFormulaParameter>(formulaParam.ValueResolverCode, injections);
                    ...
                    if (jobs.Any())
                    {
                        ...
                        var jobPrices = (dictPrices.ContainsKey(municipalityId)
                                         && dictPrices[municipalityId].ContainsKey(periodStart))
                                            ? dictPrices[municipalityId][periodStart].Where(
                                                x => jobs.ContainsKey(x.Job.Id)).ToList()
                                            : new List<WorkPrice>();
            ...
            finally
            {
                ...
                Container.Release(roStructElService);
            }
