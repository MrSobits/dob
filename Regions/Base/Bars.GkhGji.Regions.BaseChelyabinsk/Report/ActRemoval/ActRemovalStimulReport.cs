	...
	public class ActRemovalStimulReport : GkhBaseStimulReport
    {
        ...
        public override void PrepareReport(ReportParams reportParams)
        {
            ...
            try
            {
                ...
                this.Report["ИдентификаторДокументаГЖИ"] = actRemoval.Id.ToString();
                this.Report["СтрокаПодключениякБД"] = this.Container.Resolve<IDbConfigProvider>().ConnectionString;
                ...
                this.Report["ДатаАкта"] = actRemoval.DocumentDate.HasValue
                    ? actRemoval.DocumentDate.Value.ToString("d MMMM yyyy")
                    : string.Empty;

                this.Report["НомерАкта"] = actRemoval.DocumentNumber;
                this.Report["Описание"] = actRemoval.Description;
                ...
                this.Report["НаселенныйПункт"] = violData.Where(x => x.RealityObject != null && x.RealityObject.FiasAddress != null).Select(x => x.RealityObject.FiasAddress.PlaceName).FirstOrDefault();
                ...
                if (roList.Count > 0)
                {
                    this.Report["Адреса"] = roList.Aggregate(string.Empty, (x, y) => x + (!string.IsNullOrEmpty(x) ? "; " + y : y));    
                }
                ...
                this.Report["НомераНевыполненныхПунктов"] = linksViolation;
                ...
                this.Report["ИнспекторыИКоды"] = inspectorsQuery
                ...
                this.Report["ИнспекторыАкта"] = inspectorsQuery
                ...
                this.Report["Отдел"] = zonalInspDomain.GetAll()
                ...
                if (disposal != null)
                {
                    this.Report["ДатаРаспоряжения"] = disposal.DocumentDate.HasValue
                    ...
                    {
                        this.Report["ДатаИНомерРешенияПрокурора"] = "Не требуется";
                    }
                    
                    var kindCheckName = disposal.KindCheck != null ? disposal.KindCheck.Name : "";
                    this.Report["ВидПроверки"] = kindCheckName;


                    if (disposal.IssuedDisposal != null)
                    {
                        this.Report["ДЛВынесшееРаспоряжение"] = disposal.IssuedDisposal.Position;
                        this.Report["ФИОВынесшееПриказ"] = disposal.IssuedDisposal.Fio;
                        this.Report["РуководительРП"] = (!string.IsNullOrEmpty(disposal.IssuedDisposal.PositionGenitive) ? 
                    ...
                    this.Report["ВидОбследованияРП"] = this.GetTypeCheckAblative(disposal.KindCheck);
                    if (disposal.DateStart.HasValue && disposal.DateEnd.HasValue)
                    {
                     ...
                     //   this.Report["ПродолжительностьПроверки"] = countDays.ToString();
                    }
                ...
                this.Report["Ознакомлен"] = !string.IsNullOrEmpty(witnessIsFamiliar)
                ...
                this.Report.RegData("ДопПредоставленныеДокументы", actOtherProvidedDocs);

                //Собираем дату и время проведения проверки

                this.Report["ДатаВремяПроверки"] = actRemovalPeriodDomain.GetAll()
											.Where(x => x.ActRemoval.Id == actRemoval.Id)
                ...
                if (prescription != null)
                {
					...
					{
						this.Report["УправОрг"] = prescription.Inspection.Contragent.Name;
						this.Report["ИННУправОрг"] = prescription.Inspection.Contragent.Inn;
						this.Report["УправОргРП"] = prescription.Inspection.Contragent.Name;
						this.Report["СокрУправОрг"] = prescription.Inspection.Contragent.ShortName;
						...
						{
							this.Report["РуководительЮЛДолжность"] = string.Format(
								"{0} {1} {2} {3}",
							...
							this.Report["РуководительЮЛ"] = string.Format(
								"{0} {1} {2}",
								headContragent.Surname,
								headContragent.Name,
								headContragent.Patronymic);
						...
						{
							this.Report["ЮрАдресКонтрагента"] = prescription.Inspection.Contragent.JuridicalAddress;
						}

						if (prescription.Inspection.Contragent.FiasFactAddress != null)
						{
							this.Report["АдресКонтрагентаФакт"] = prescription.Inspection.Contragent.FiasFactAddress.Return(x => x.AddressName);
						}
					}
                    ...
                    this.Report.RegData("Нарушения", prescriptionViols);
