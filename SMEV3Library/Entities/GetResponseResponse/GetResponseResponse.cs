...
namespace SMEV3Library.Entities.GetResponseResponse
{
    ...
    public abstract class GetResponseResponse : SMEVResponse
    {
        ...
        public void SetSignatureFileNameSGIO(XElement xElement)
        {
            ...
            foreach (var header in xElement.Elements(SMEVNamespaces12.SGIOBasicNamespace + "AttachmentHeader"))
            {
                string subject = string.Empty;
                var memtype = header.Element(SMEVNamespaces12.SGIOBasicNamespace + "MimeType").Value;
                string ext = GetExtension(memtype);

                var fileGiuid = header.Element(SMEVNamespaces12.SGIOBasicNamespace + "contentId")?.Value;
                var fileName = header.Element(SMEVNamespaces12.SGIOBasicNamespace + "contentId")?.Value + "."+ ext;
                var signatureEl = header.Element(SMEVNamespaces12.SGIOBasicNamespace + "SignaturePKCS7");
        ...
        public void SetFileNameSGIO(XElement xElement)
        {
            ...
            foreach (var attachmentContent in xElement.Elements(SMEVNamespaces12.SGIOBasicNamespace + "AttachmentContent"))
            {
                var fileGuid = attachmentContent.Element(SMEVNamespaces12.SGIOBasicNamespace + "Id")?.Value;
                var href = attachmentContent.Element(SMEVNamespaces12.SGIOBasicNamespace + "Content").Value;
                byte[] newBytes = Convert.FromBase64String(href);
