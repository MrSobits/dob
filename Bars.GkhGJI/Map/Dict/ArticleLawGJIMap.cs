...
namespace Bars.GkhGji.Map
{
    ...
    public class ArticleLawGjiMap : BaseEntityMap<ArticleLawGji>
    {
        ...
        protected override void Map()
        {
            ...
            Property(x => x.OMS, "OMS").Column("OMS").Length(300);
            Property(x => x.OmsRegion, "Принадлежность к ОМС/Регион (енум)").Column("OMS_REGION").NotNull();
            Property(x => x.KBK, "KBK").Column("KBK").Length(300);
            Property(x => x.NameOMS, "NAME_OMS").Column("NAME_OMS").Length(2000);
            Property(x => x.Part, "Часть").Column("PART").Length(50);
            Property(x => x.Article, "Статья").Column("ARTICLE").Length(50);
            Property(x => x.Bank, "Bank").Column("BANK");
        }
    }
}
