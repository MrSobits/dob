namespace Bars.Gkh.RegOperator.Imports.Ches
{
    ...
    public class AccountImporter : BaseChesImporter<AccountFileInfo>
    {
        ...
        public override void InitDicts(AccountFileInfo accountFileInfo)
        {
            ...
            this.realityObjects = EnumerableExtensions.ToHashSet(this.RealityObjectRepo.GetAll().Select(x => x.Id));
            ...
            this.legalOwnerMatchSet = EnumerableExtensions.ToHashSet(
                    this.ChesMatchLegalAccountOwnerRepo.GetAll()
        ...
        public override void ProcessData(AccountFileInfo accountFileInfo)
        {
            ...
            foreach (var rows in accounts)
            {
                ...
                foreach (var row in rowArray)
                {
                    ...
                    string errorMessage;
                    if (!this.BillingFilterService.CheckByRealityObject(account.Room.RealityObject, out errorMessage))
                    {
                        accountFileInfo.AddErrorRow(row, errorMessage, row.LsNum, row.ExtLsNum);
                        continue;
                    }
