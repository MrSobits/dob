    ...
    public class DataCollectorTestController : BaseController
    {
                ...
                () =>
                    {
                        ...
                        var mService = this.Container.ResolveDomain<ManagingOrganization>();
                        var pService = this.Container.ResolveDomain<PeriodDi>();
                        var syncProvider = this.Container.Resolve<ISyncProvider>(new { silentMode = true });
                        try
                        {
                            var moid = baseParams.Params.Get("moid").ToLong();
                            var pid = baseParams.Params.Get("pid").ToLong();
        ...
        public ActionResult CollectRobject988(BaseParams baseParams)
        {
            ...
            try
            {
                var roid = baseParams.Params.Get("roid").ToLong();
                var pid = baseParams.Params.Get("pid").ToLong();
                var result = service.CollectHouseProfile988Data(new HouseProfileData988(), robjectService.Get(roid), periodService.Get(pid));
        ...
        public ActionResult SetHouseProfile988(BaseParams baseParams)
        {
            ...
            ExplicitSessionScope.CallInNewScope(
                () =>
                {
                    ...
                    var provider = this.Container.Resolve<ISyncProvider>(new { silentMode = true });
                    try
                    {
                        ...
                        var result = service.CollectHouseProfile988Data(profile.house_profile_data, robject.RealityObject, period.PeriodDi);
                        ...
                        if (result.Success)
                        {
                            ...
                            response = new ContentResult { Content = sb.ToString(), ContentEncoding = Encoding.UTF8, ContentType = "text/xml" };
                            return;
                        }
                ...
                () =>
                    {
                        ...
                        try
                        {
                            ...
                            result = new ContentResult { Content = JsonNetConvert.SerializeObject(this.Container, robjects), ContentEncoding = Encoding.UTF8, ContentType = "application/json" };
                        }
                        finally
                        {
                            provider.Close();
        ...
        public ActionResult GetRobjects988ByFullAddresses(BaseParams baseParams)
        {
            ...
            ExplicitSessionScope.CallInNewScope(
                ...
                {
                    var service = this.Container.Resolve<IRobjectService>();
                    var provider = this.Container.Resolve<ISyncProvider>(new
                    {
                    ...
                    {                       
                        ...
                        result = new ContentResult { Content = JsonNetConvert.SerializeObject(this.Container, house), ContentEncoding = Encoding.UTF8, ContentType = "application/json" };
                    }
                    finally
                    {
                        provider.Close();
