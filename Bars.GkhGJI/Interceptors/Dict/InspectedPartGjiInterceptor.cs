    ...
    public class InspectedPartGjiInterceptor : EmptyDomainInterceptor<InspectedPartGji>
    {
        public override IDataResult BeforeDeleteAction(IDomainService<InspectedPartGji> service, InspectedPartGji entity)
        {
            var refFuncs = new List<Func<long, string>>
                               {
                                  id => this.Container.Resolve<IDomainService<ActCheckInspectedPart>>().GetAll().Any(x => x.InspectedPart.Id == id) ? "Инспектируемые части в акте проверки" : null,
                                  id => this.Container.Resolve<IDomainService<ActSurveyInspectedPart>>().GetAll().Any(x => x.InspectedPart.Id == id) ? "Инспектируемые части в акте обследования" : null
                               };
