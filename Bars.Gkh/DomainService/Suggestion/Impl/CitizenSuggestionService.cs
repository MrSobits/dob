namespace Bars.Gkh.DomainService
{
    ...
    public class CitizenSuggestionService : ICitizenSuggestionService
    {
        ...
        {
            var bcDomain = this.Container.ResolveDomain<CitizenSuggestion>();
        ...
        public IDataResult ListExecutorZonalInspection(BaseParams baseParams)
        {
            var loadParams = baseParams.GetLoadParam();
            //var executorType = baseParams.Params.GetAs<ZonalInspection>("ExecutorZonalInspection");
            var data = Container.Resolve<IDomainService<ZonalInspection>>().GetAll().Select(x => new { x.Id, x.Name });
        ...
        public IDataResult ListInspector(BaseParams baseParams) 
        {
            var loadParams = baseParams.GetLoadParam();
            //var executorType = baseParams.Params.GetAs<ZonalInspection>("ExecutorZonalInspection");
            var data = Container.Resolve<IDomainService<ZonalInspectionInspector>>().GetAll()
                .Where(x => x.ZonalInspection == x.Inspector.ZonalInspection)
