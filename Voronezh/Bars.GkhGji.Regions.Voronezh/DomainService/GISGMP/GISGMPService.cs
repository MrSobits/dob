...
using Bars.GkhGji.Regions.Voronezh.LicenseInfoRequest;
...
namespace Bars.GkhGji.Regions.Voronezh.DomainService
{
    public class GISGMPService : IGISGMPService
    {
        ...
        /// <summary>
        /// Запрос экспорта начислений
        /// </summary>
        public void SendExportRequest(GisGmp requestData)
        {
            XElement request = GetExportChargesRequestXML(requestData);
            string messageId = GetTimeStampUuid();
            var requestResult = _SMEV3Service.SendRequestAsyncSGIO(request, messageId, null, true);
        }
        ...
        public bool TryProcessResponse(GisGmp requestData, GetResponseResponse response, IProgressIndicator indicator = null)
        {
            try
            {
                ...
                else
                {
                    ...
                    if (response.MessagePrimaryContent.Element(ns0Namespace + "ExportPaymentsResponse")?.Elements() != null)
                    {
        ...
        private XElement GetPayRequestByPayerIdentifierXML(GisGmp requestData)
        {
            ...
            var payReqByIdentifier = new ExportPaymentsProxy.ExportPaymentsRequest
            {
                ...
                originatorId = requestData.Protocol.ComissionMeeting.ZonalInspection.GisGmpId,
            ...
            return ToXElement<ExportPaymentsProxy.ExportPaymentsRequest>(payReqByIdentifier);
        }
        ...
        private XElement GetPayRequestXML(GisGmp requestData)
        {
            ...
            return ToXElement<ExportPaymentsProxy.ExportPaymentsRequest>(payReq);
        }

        private XElement GetExportChargesRequestXML(GisGmp requestData)
        {
            var identifiers = GetIdentifiersFromConfig();

            var exportChReq = new ExportChargesProxy.ExportChargesRequest
            {
                ...
                originatorId = requestData.Protocol.ComissionMeeting.ZonalInspection.GisGmpId,
                external = 0,
                Paging = new ExportChargesProxy.PagingType
                {
                    pageNumber = "1",
                    pageLength = "1000"
                },
                ChargesExportConditions = new ExportChargesProxy.ChargesExportConditions
                {
                    kind = "CHARGESTATUS",
                    ItemElementName = ExportChargesProxy.ItemChoiceType.ChargesConditions,
                    Item = new ExportChargesProxy.ChargesConditionsType
                    {
                        SupplierBillID = new string[] 
                        { 
                            requestData.UIN 
                        }
                    }
            ...
            return ToXElement<ExportChargesProxy.ExportChargesRequest>(exportChReq);
        }
        ...
        private XElement GetReconcileRequestXML(GisGmp requestData)
        {
            ...
            var reconcileReq = ToXElement<ForcedAckmowledgmentProxy.ForcedAcknowledgementRequest>(res);
            ...
            foreach (PayReg payment in paymentList)
            {
                ...
                {
                    reconcileReq.Element(faNamespace + "Reconcile").Add(
        ...
        private XElement GetCalcRequestXML(GisGmp requestData)
        {
            ...
            XElement importChReqXElement = ToXElement<ImportChargesProxy.ImportChargesRequest>(importCHReq);

            return importChReqXElement;
        }
        ...
        private ImportChargesProxy.Payee GetPayee(GisGmp requestData)
        {
            var zonaInsp = requestData.Protocol.ComissionMeeting.ZonalInspection;
            ...
            return new ImportChargesProxy.Payee
            {
                ...
                {
                    accountNumber = zonaInsp.PersonalAcc,
                    Bank = new ImportChargesProxy.BankType
                    {
                        name = zonaInsp.BankName,
                        bik = zonaInsp.bik,
                        correspondentBankAccount = zonaInsp.CorrespAcc
                    }
                }
            };
        }
                        ...
                        document.TypeDocumentGji == TypeDocumentGji.ResolutionRospotrebnadzor)
                    {
                        ...
                        if (docChildrenParent != null)
                        {
                            ...
                            if (resolution != null)
                            {
                                ...
                                {
                                    payerName = resolution.IndividualPerson.Fio;
                                }
