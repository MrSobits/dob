namespace Bars.Gkh.Import.ImportOktmo
{
	...
	public class MunicipalityFiasOktmoImport : GkhImportBase
	{
		...
		private const long minOktmoValue = 10000000000;
		private const long maxOktmoValue = 99999999999;
		...
		private void ProcessData(byte[] fileData, IProgressIndicator indicator)
		{
			...
			try
			{
				using (var excelProvider = this.Container.Resolve<IGkhExcelProvider>("ExcelEngineProvider"))
				{
		...
		private void ProcessRow(GkhExcelCell[] row)
		{
			...
			else
			{
				...
				if (oktmo.Length < minOktmoValue || oktmo.Length > maxOktmoValue)
				{
					return;
				}
		...
		private void ProcessRecordsForSave()
		{
			foreach (var muRecord in muRecords)
			{
				...
				foreach (var placeRecord in muRecord.Places)
				{
					...
					if (!fiasOffNamesDict.ContainsKey(placeName))
					{
						LogImport.Warn(
							placeRecord.Name,
					...
					var aoGuid = fiasOffNamesDict[placeName];
