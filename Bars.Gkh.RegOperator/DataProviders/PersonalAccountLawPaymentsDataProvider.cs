        ...
        protected override IQueryable<PersonalAccountLawPaymentsInfo> GetDataInternal(BaseParams baseParams)
        {
            ...
            try
            {
                ...
                var maxLawsuits = lawCalcDomain.GetAll()
                                    .Where(x => x.PersonalAccountId == persAcc.Id).ToList();
                ...
                if (maxLawsuits.Count > 0)
                {
                    ...
                    lawQuery = lawCalcDomain.GetAll()
                        .Where(x => x.PersonalAccountId == persAcc.Id && x.Lawsuit.Id == maxLawsuit)
                        .Select(x => new
                        {
                            x.Id,
                            x.PaymentDate,
                        ...
                        .Select(x =>
                        {
                            var periodName = periodDomain.GetAll().Where(y => y.Id == x.PeriodId).Select(y => y.Name).FirstOrDefault();
