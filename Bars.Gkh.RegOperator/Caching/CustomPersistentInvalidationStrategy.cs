namespace Bars.Gkh.RegOperator.Caching
{
        ...
        where TValue : class, IEntity, new()
    {
        ...
        public bool Invalidate()
        {
            ...
            this.logger.Info("Кэш:({0}) Прогрев кэша для {1}".FormatUsing(name, fName));
            ...
            try
            {
                ...
                this.logger.Info("Кэш:({0}) К заполнению {1}".FormatUsing(name, objectKeys.Count));
                var counter = 0;
                foreach (var partKeys in objectKeys.Split(this.bulkSize))
                {
                    ...
                    this.logger.Info("Кэш:({0}) Заполнено {1} из {2}".FormatUsing(name, counter, objectKeys.Count));
            ...
            {
                this.logger.ErrorFormat(ex, "Кэш:({0}). Ошибка при обновлении кэша.", name);
                throw;
            }
            finally
            {
                sw.Stop();
                this.logger.Info("Кэш:({0}) Кэш прогрет за {1}".FormatUsing(name, sw.Elapsed));
