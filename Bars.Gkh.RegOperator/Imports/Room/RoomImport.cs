namespace Bars.Gkh.RegOperator.Imports.Room
{
    ...
    {
        private ILogImportManager logImportManager;
        ...
        private List<long> existRobjectIdsList;
        ...
        private void KillCache()
        {
            ...
            //    return; // живи
            //}
            this.Logger.DebugFormat("KillCache(): AppCache._useCount = {0}", this.GetAppCacheInUse());
        ...
        private void TestCache()
        {
            ...
            if (testRo != null)
            {
                this.Logger.DebugFormat("TestCache(): AppCache._useCount = {0}", this.GetAppCacheInUse());
        ...
        private void InitCache(bool force = false)
        {
            ...
            this.Logger.DebugFormat("noCasheRefresh == {0}", this.NoCasheRefresh);

            if (this.NoCasheRefresh == false || force)
            {
                this.KillCache();

                this.Logger.DebugFormat("AppCache.Invalidating = {0}", this.AppCache.Invalidating);
                
                var invalidateResult = this.AppCache.Invalidate();
                this.Logger.DebugFormat("InitCache() var invalidateResult = AppCache.Invalidate();: AppCache._useCount = {0}", this.GetAppCacheInUse());
                ...
                while (!invalidateResult)
                {
                    ...
                    if (this.GetAppCacheInUse() > 0)
                    {
                        ...
                        this.Logger.DebugFormat("InitCache() this.AppCache.Release(): AppCache._useCount = {0}", this.GetAppCacheInUse());
                    }
                    else if (this.GetAppCacheInUse() < 0)
                    {
                        this.AppCache.Use();
                        this.Logger.DebugFormat("InitCache() this.AppCache.Use();: AppCache._useCount = {0}", this.GetAppCacheInUse());
                    }

                    invalidateResult = this.AppCache.Invalidate();
                }

                this.Logger.DebugFormat("var invalidateResult = {0}", invalidateResult);
                this.Logger.DebugFormat("InitCache() AppCache.Invalidate(): AppCache._useCount = {0}", this.GetAppCacheInUse());
            ...
            this.Logger.DebugFormat("InitCache() AppCache.GetClient<Contragent>();: AppCache._useCount = {0}", this.GetAppCacheInUse());
            this.persAccCacheClient = this.AppCache.GetClient<BasePersonalAccount>();
            this.Logger.DebugFormat("InitCache() AppCache.GetClient<BasePersonalAccount>();: AppCache._useCount = {0}", this.GetAppCacheInUse());
            this.roomCacheClient = this.AppCache.GetClient<Room>();
            this.Logger.DebugFormat("InitCache() AppCache.GetClient<Room>();: AppCache._useCount = {0}", this.GetAppCacheInUse());
            this.realityObjectCacheClient = this.AppCache.GetClient<RealityObject>();
            this.Logger.DebugFormat("InitCache() AppCache.GetClient<RealityObject>();: AppCache._useCount = {0}", this.GetAppCacheInUse());
        ...
        public new void InitLog(string fileName)
        {
            ...
            this.logImportManager = this.Container.Resolve<ILogImportManager>();
        ...
        protected override ImportResult Import(BaseParams @params, ExecutionContext ctx, IProgressIndicator indicator, CancellationToken ct)
        {
            ...
            this.NoCasheRefresh = true; // @params.Params.GetAs<bool>("NoRefreshCash");
        ...
        private void InitDictForImport()
        {
            ...
            List<long> roIdsList = records.Select(x => x.RealtyObjectId).Distinct().ToList();
            ...
            var existOwners = this.BasePersonalAccountDomain.GetAll()
              //  .Where(x=> roIdsList.Contains(x.Room.RealityObject.Id))
                .Select(x => new
                {
        ...
        private void InitDictForExtract()
        {
            ...
            this.realtyObjectRoomsDict = this.RoomsDomain.GetAll()
                 .Where(x => roIdsList.Contains(x.RealityObject.Id))
                .Select(x => new
                {
        ...
        private void PrepareData(IProgressIndicator indicator)
        {
            var recDict = this.records.Select(
                ...
                .AsEnumerable().Where(x=> x.Room != null) // попробуем грузить нисмотря на то что помещения в кеше нет, такие просто исключаем из выгрузки
                .GroupBy(
                    x =>
                    new
                        {
        ...
        private void AddLog(int rowNum, string message, bool success = true, Exception ex = null)
        {
            ...
            if (ex.IsNotNull())
            {
                this.Logger.Error("Ошибка в импорте абонентов", ex);
            }
        }
...
#warning костыль от Redis'а
        ...
        public void Dispose()
        {
            if (this.contragentCacheClient != null)
            {
                ...
                this.Logger.DebugFormat("Dispose() ContragentCacheClient.Dispose();: AppCache._useCount = {0}", this.GetAppCacheInUse());
            }

            if (this.persAccCacheClient != null)
            {
                this.persAccCacheClient.Dispose();
                this.Logger.DebugFormat("Dispose() PersAccCacheClient.Dispose();: AppCache._useCount = {0}", this.GetAppCacheInUse());
            }

            if (this.roomCacheClient != null)
            {
                this.roomCacheClient.Dispose();
                this.Logger.DebugFormat("Dispose() RoomCacheClient.Dispose();: AppCache._useCount = {0}", this.GetAppCacheInUse());
            }

            if (this.realityObjectCacheClient != null)
            {
                this.realityObjectCacheClient.Dispose();
                this.Logger.DebugFormat("Dispose() RealityObjectCacheClient.Dispose(): AppCache._useCount = {0}", this.GetAppCacheInUse());
            }
        }
    }
