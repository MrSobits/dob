namespace Bars.GkhGji.Regions.Tomsk.Report
{
    ...
    public class PresentationStimulReport : GjiBaseStimulReport
    {
        ...
        public override void PrepareReport(ReportParams reportParams)
        {
            ...
            Report["АдресПравонарушения"] = InspectionRobjectDomain.GetAll()
            ...
            Report["ДатаПредставления"] = presentation.DocumentDate.ToDateString("«d» MMMM y");
            Report["НомерПредставления"] = presentation.DocumentNumber;

            var parentAdminCase = GetParentDocument(presentation, TypeDocumentGji.AdministrativeCase);

            if (parentAdminCase != null)
            {
                Report["НомерОснования"] = parentAdminCase.DocumentNumber;
            }
            ...
            if (parentProtocol != null)
            {
                ...
                Report["СтатьяЗакона"] = protocolArticles
                ...
                Report["ОписаниеСтатьи"] = protocolArticles
                ...
                {
                    Report["Установил"] = protocolDescription.DescriptionSet.Encode();
                    Report["УстановилПодробнее"] = protocolDescription.Description.Encode();
                }
            }

            Report["ФИОФизЛицаСокр"] = CutFio(presentation.PhysicalPerson);

            if (presentation.Official != null)
            {
                Report["Должность"] = presentation.Official.Position;
                Report["Инспектор"] = presentation.Official.Fio;
                Report["ИнспекторСокр"] = presentation.Official.ShortFio ?? CutFio(presentation.Official.Fio);
            }

            Report["ДолжностьФЛ"] = presentation.ExecutantPost;
            Report["Требование"] = presentation.DescriptionSet;
        }
