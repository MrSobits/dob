namespace Bars.Gkh.FormatDataExport.Domain.Impl
{
    ...
    public class FormatDataExportRoleService : IFormatDataExportRoleService
    {
        ...
        /// <inheritdoc />
        public FormatDataExportProviderFlags GetCustomProviderFlags(Operator gkhOperator)
        {
            var providerFlag = FormatDataExportProviderFlags.None;

            if (gkhOperator?.User != null && gkhOperator.User.Roles.IsNotEmpty())
            {
                foreach (var userRole in gkhOperator.User.Roles)
                {
