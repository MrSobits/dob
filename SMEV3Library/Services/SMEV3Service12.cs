...
namespace SMEV3Library.Services
{
    public class SMEV3Service12 : ISMEV3Service
    {
        ...
        /// <param name="attachments">Прикладываемые файлы</param>
        /// <returns>SendRequestRequest, распарсеный в класс</returns>
        public async Task<SendRequestResponse> SendRequestAsyncSGIO(XElement request, string messageId, List<FileAttachment> attachments = null, bool saveLog = false)
        {
            //-----------AttachmentContentList, AttachmentHeaderList-----------
            XElement AttachmentContentListXElement = null;
            XElement AttachmentHeaderListXElement = null;
            ...
            if (attachments != null && attachments.Count > 0)
            {
                AttachmentContentListXElement = new XElement(SMEVNamespaces12.SGIOBasicNamespace + "AttachmentContentList");
                AttachmentHeaderListXElement = new XElement(SMEVNamespaces12.SGIOBasicNamespace + "AttachmentHeaderList");

                foreach (FileAttachment file in attachments)
                {
                    //AttachmentHeader
                    AttachmentHeaderListXElement.Add(new XElement(SMEVNamespaces12.SGIOBasicNamespace + "AttachmentHeader"
                        , new XElement(SMEVNamespaces12.SGIOBasicNamespace + "contentId", file.FileName)
                        , new XElement(SMEVNamespaces12.SGIOBasicNamespace + "MimeType", MimeTypeMap.GetMimeType(file.GetExtension()))));

                    //AttachmentContent
                    AttachmentContentListXElement.Add(new XElement(SMEVNamespaces12.SGIOBasicNamespace + "AttachmentContent"
                        , new XElement(SMEVNamespaces12.SGIOBasicNamespace + "Id", file.FileName)
                        //, new XElement(SMEVNamespaces12.BasicNamespace + "Content", $"cid:{file.FileName}"))
                        , new XElement(SMEVNamespaces12.SGIOBasicNamespace + "Content",
                            new XElement(SMEVNamespaces12.XopNamespace + "Include",
                                new XAttribute("href", $"cid:{file.FileName}"),
                                new XAttribute(XNamespace.Xmlns + "xop", SMEVNamespaces12.XopNamespace)))));
                }
            ...
            //-----------SenderProvidedRequestData-----------
            var senderProvidedRequestDataXElement = new XElement(SMEVNamespaces12.SGIOTypesNamespace + "SenderProvidedRequestData",
                    new XAttribute("Id", "SIGNED_BY_CONSUMER")
                    , new XElement(SMEVNamespaces12.SGIOTypesNamespace + "MessageID", messageId)
                    , new XElement(SMEVNamespaces12.SGIOBasicNamespace + "MessagePrimaryContent", request)
                );
            ...
            if (_options.TestMode)
                senderProvidedRequestDataXElement.Add(new XElement(SMEVNamespaces12.SGIOTypesNamespace + "TestMessage"));

            //-----------SendRequestRequest-----------
            XElement sendRequestRequestXElement = new XElement(
                    SMEVNamespaces12.SGIOTypesNamespace + "SendRequestRequest"
                    , senderProvidedRequestDataXElement);

            if (AttachmentContentListXElement != null)
                sendRequestRequestXElement.Add(AttachmentContentListXElement);

            sendRequestRequestXElement.Add(new XElement(SMEVNamespaces12.SGIOTypesNamespace + "CallerInformationSystemSignature", SignXElement(senderProvidedRequestDataXElement, @"#SIGNED_BY_CONSUMER")));

            //-----------Применяем namespace-----------
            sendRequestRequestXElement.SetAttributeValue(XNamespace.Xmlns + "ns", SMEVNamespaces12.SGIOTypesNamespace);
            sendRequestRequestXElement.SetAttributeValue(XNamespace.Xmlns + "ns1", SMEVNamespaces12.SGIOBasicNamespace);
        ...
        {
            //-----------MessageTypeSelector-----------
            var messageTypeSelectorXElement = new XElement(SMEVNamespaces12.SGIOBasicNamespace + "MessageTypeSelector",
                    new XAttribute("Id", "SIGNED_BY_CALLER")
                );

            if (namespaceUri != null && rootElementLocalName != null)
            {
                messageTypeSelectorXElement.Add(new XElement(SMEVNamespaces12.SGIOBasicNamespace + "NamespaceURI", namespaceUri));
                messageTypeSelectorXElement.Add(new XElement(SMEVNamespaces12.SGIOBasicNamespace + "RootElementLocalName", rootElementLocalName));
            }

            messageTypeSelectorXElement.Add(new XElement(SMEVNamespaces12.SGIOBasicNamespace + "Timestamp", GetTimeStamp()));
        ...
        {
            //-----------AckTargetMessage-----------
            XElement ackTargetMessageXElement = new XElement(SMEVNamespaces12.SGIOBasicNamespace + "AckTargetMessage",
                    new XAttribute("Id", "SIGNED_BY_CALLER")
                    , new XAttribute("accepted", accepted)
                    , messageId
                );
            //-----------AckRequest-----------
            XElement ackRequestXElement = new XElement(SMEVNamespaces12.SGIOTypesNamespace + "AckRequest",
                    ackTargetMessageXElement,
                    new XElement(SMEVNamespaces12.SGIOTypesNamespace + "CallerInformationSystemSignature", SignXElement(ackTargetMessageXElement, @"#SIGNED_BY_CALLER"))
                );

            //-----------Применяем namespace-----------
            ackRequestXElement.SetAttributeValue(XNamespace.Xmlns + "ns", SMEVNamespaces12.SGIOTypesNamespace);
            ackRequestXElement.SetAttributeValue(XNamespace.Xmlns + "ns1", SMEVNamespaces12.SGIOBasicNamespace);
