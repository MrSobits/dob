...
namespace Bars.GkhGji.Regions.Voronezh.Tasks.GetSMEVAnswers
{
    ...
    public class GetSMEVAnswersTaskExecutor : ITaskExecutor
    {
        ...
        public GetSMEVAnswersTaskExecutor(ISMEV3Service SMEV3Service, IGISERPService GISERPService, ISMEVEGRNService SMEVEGRNService, IFileManager fileManager, 
            ...
            ISMEVDISKVLICService diskvlic, ISMEVPremisesService premises, ISMEVNDFLService ndfl, ISMEVSNILSService snils, ISMEVExploitResolutionService exploit,
            ISMEVChangePremisesStateService changepremises, IGASUService gasu, ISMEVEDOService doServ, IComplaintsService complServ)
        {            
            ...
            _SMEVExploitResolutionService = exploit;
            _GASUService = gasu;
            _SMEVChangePremisesStateService = changepremises;
        ...
        public IDataResult Execute(BaseParams @params, B4.Modules.Tasks.Common.Contracts.ExecutionContext ctx, IProgressIndicator indicator, CancellationToken ct)
        {
            ...
            try
            {
                ...
                do
                {
                    ...
                    else if (TryExploitResolutionProcessed(responseResult, ref processedResult))
                    {
                    ...
                    else if (TryChangePremisesStateProcessed(responseResult, ref processedResult))
                    {
                        processLog.Add($"Сообщение {responseResult.OriginalMessageId} - обработано как ВС смены стасуса у помещения  - {processedResult}");
                    }
        ...
        private bool TryExploitResolutionProcessed(GetResponseResponse responseResult, ref string result)
        {
            SMEVExploitResolution requestData = SMEVExploitResolutionDomain.GetAll().Where(x => x.MessageId == responseResult.OriginalMessageId).FirstOrDefault();

            if (requestData == null)
                return false;

            var success = _SMEVExploitResolutionService.TryProcessResponse(requestData, responseResult, null);
            result = (success ? "успешно" : "ошибка") + ": " + requestData.Answer;

            return true;
        }

        private bool TryChangePremisesStateProcessed(GetResponseResponse responseResult, ref string result)
        {
            SMEVChangePremisesState requestData = SMEVChangePremisesStateDomain.GetAll().Where(x => x.MessageId == responseResult.OriginalMessageId).FirstOrDefault();

            if (requestData == null)
                return false;

            var success = _SMEVChangePremisesStateService.TryProcessResponse(requestData, responseResult, null);
            result = (success ? "успешно" : "ошибка") + ": " + requestData.Answer;
