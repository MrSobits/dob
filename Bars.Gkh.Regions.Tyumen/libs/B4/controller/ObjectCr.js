    ...
    aspects: [
         ...
         */
        {
            ...
            permissions: [
                ...
                { name: 'GkhCr.ObjectCr.Field.AllowReneg.Edit', applyTo: '#btnObjectCrAllowReneg', selector: '#objectCrGrid' },
                {
        ...
        },
        {
            ...
            otherActions: function (actions) {
                actions[this.gridSelector + ' #btnObjectCrAllowReneg'] = { 'click': { fn: this.onAllowReneg, scope: this } };
                actions[this.gridSelector]['select'] = { fn: this.onGridSelect, scope: this };
                actions['#objectCrAddWindow b4selectfield[name=ProgramCr]'] = { change: { fn: this.onProgramChange, scope: this } };
            },
            onGridSelect: function (me, rec, index, eOpts) {
                var button = Ext.ComponentQuery.query(this.gridSelector + ' #btnObjectCrAllowReneg')[0];
                if (button) {
                    button.setText(rec.get('AllowReneg') ? 'Отменить согласование' : 'Разрешить согласование');
                }
            },
            ...
            onAllowReneg: function () {
                var me = this,
                    selection = me.getGrid(me.gridSelector).getSelectionModel().getSelection()[0],
                    model,
                    record;
                if (!selection) return;
                model = this.getModel(selection);
                record = new model({ Id: selection.getId() });
                record.set('AllowReneg', !selection.get('AllowReneg'));
                selection.set('AllowReneg', !selection.get('AllowReneg'));
                record.save()
                    .next(function (resp) {
                        Ext.Msg.alert('Успех', 'Согласование успешно ' + (resp.record.get('AllowReneg') ? 'разрешено' : 'отменено'));
                        var button = Ext.ComponentQuery.query(this.gridSelector + ' #btnObjectCrAllowReneg')[0];
                        if (button) {
                            button.setText(resp.record.get('AllowReneg') ? 'Отменить согласование' : 'Разрешить согласование');
                        }
                    }, this)
                    .error(function (res) {
                        Ext.Msg.alert('Ошибка!', Ext.isString(res.responseData) ? res.responseData : res.responseData.message);
                    });
            },
