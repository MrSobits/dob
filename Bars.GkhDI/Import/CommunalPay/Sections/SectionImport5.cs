namespace Bars.GkhDi.Import.Sections
{
    ...
    public class SectionImport5 : ISectionImport
    {
        public string Name
        {
            get
            {
                return "Импорт из комплат секция #5";
            }
        }
        ...
        public void ImportSection(ImportParams importParams)
        {
            ...
            var disclosureInfo = this.Container.Resolve<IDomainService<DisclosureInfo>>()
               .GetAll()
            ...
            else
            {
                ...
                var adminRespServiceDict = adminRespService
                    .GetAll()
                ...
                foreach (var section5Record in sectionsData.Section5)
                {
                    ...
                    FileInfo fileInfo = FileCreater.Create(importParams.zipName, fileName, fileinfoService, fileManager);

                    var adminResp = adminRespServiceDict.FirstOrDefault(x => x.AmountViolation == section5Record.AmountViolation && x.DateImpositionPenalty == section5Record.DateImpositionPenalty
                        && x.SumPenalty == section5Record.SumPenalty);

                    if (adminResp == null)
                    {
                        logImport.Info(this.Name, string.Format("Добавлена запись в раздел 'Административная ответственность с суммой' для услуги {0}", section5Record.AmountViolation));
