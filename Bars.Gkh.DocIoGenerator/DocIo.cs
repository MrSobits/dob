namespace Bars.Gkh.DocIoGenerator
{
    ...
    public class DocIo : IDocIo
    {
        ...
        public void OpenTemplate(string path)
        {
            ...
            {
                throw new ReportProviderException(string.Format("Шаблон \"{0}\" не существует", path));
            }
        }
        ...
        public void SetPicture(string key, Stream stream, int width, int height)
        {
            try
            {
                ...
                if (textSelection != null)
                {
                    ...
                    var picture = (WPicture)paragraph.AppendPicture(Image.FromStream(stream));
                    if (width > 0 && height > 0)
                    {
                        picture.Width = width;
        ...
        public void SaveAs(string path, string name)
        {
            ...
            {
                ClearValuesDocument();
                wordDocument.Save(Path.Combine(path, name));
            }
            else
            {
                throw new ReportProviderException("Не создан документ");
