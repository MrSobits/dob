namespace Bars.GkhCr.DomainService
{
    ...
    public class ProgramCrService : IProgramCrService
    {
        ...
        {
            var tDerived = new T();
            foreach (var propBase in propertyes.Where(x => x.CanWrite))
            {
        ...
        public virtual IDataResult CopyProgram(BaseParams baseParams)
        {
            ...
            try
            {
                ...
                // Если программа КР была создана на основе ДПКР то нужн озапускат ьметод который реализован в Overhaul.Base модуле
        ...
        private IDataResult BaseCopyProgram(BaseParams baseParams)
        {
            ...
            try
            {
                var journalProperies = typeof(ProgramCrChangeJournal).GetProperties(BindingFlags.Public | BindingFlags.Instance);
                var finProperies = typeof(ProgramCrFinSource).GetProperties(BindingFlags.Public | BindingFlags.Instance);
                var objectCrProperies = typeof(Entities.ObjectCr).GetProperties(BindingFlags.Public | BindingFlags.Instance);
                var qualProperies = typeof(Qualification).GetProperties(BindingFlags.Public | BindingFlags.Instance);
                var buildContractProperies = typeof(BuildContract).GetProperties(BindingFlags.Public | BindingFlags.Instance);
                var contractCrProperies = typeof(ContractCr).GetProperties(BindingFlags.Public | BindingFlags.Instance);
                var defectListProperies = typeof(DefectList).GetProperties(BindingFlags.Public | BindingFlags.Instance);
                var documentWorkCrProperies = typeof(DocumentWorkCr).GetProperties(BindingFlags.Public | BindingFlags.Instance);
                var financeSourceResourceProperies = typeof(FinanceSourceResource).GetProperties(BindingFlags.Public | BindingFlags.Instance);
                var monitoringSmrProperies = typeof(MonitoringSmr).GetProperties(BindingFlags.Public | BindingFlags.Instance);
                var personalAccountProperies = typeof(PersonalAccount).GetProperties(BindingFlags.Public | BindingFlags.Instance);
                var protocolCrProperies = typeof(ProtocolCr).GetProperties(BindingFlags.Public | BindingFlags.Instance);
                var typeWorkCrProperies = typeof(TypeWorkCr).GetProperties(BindingFlags.Public | BindingFlags.Instance);
                var performedWorkActProperies = typeof(PerformedWorkAct).GetProperties(BindingFlags.Public | BindingFlags.Instance);
                var performedWorkActRecProperies = typeof(PerformedWorkActRecord).GetProperties(BindingFlags.Public | BindingFlags.Instance);
                var voiceMemberProperies = typeof(VoiceMember).GetProperties(BindingFlags.Public | BindingFlags.Instance);
                ...
                var defectListDict = defectListService.GetAll().Where(x => x.ObjectCr.ProgramCr.Id == oldProgramCrId).GroupBy(x => x.ObjectCr.Id).ToDictionary(x => x.Key, y => y.ToList());
