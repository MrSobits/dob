Ext.define('B4.controller.protocol197.Edit', {
    ...
    models: [
        ...
        'protocol197.AnotherResolution',
        'protocol197.Petition',
        'protocol197.Annex',
        'comission.ComissionMeetingDocument',
    ...
    views: [
        ...
        'protocol197.PetitionGrid',
        'protocol197.PetitionEditWindow',
        'comission.ComissionsGrid',
        'protocol197.AnotherResolutionGrid',
        'protocol197.ViolationEditWindow',
        'SelectWindow.MultiSelectWindow',
        'SelectWindow.MultiSelectWindowTree',
        'dict.physicalpersondoctype.Grid',
        'protocol197.MassAdditionWindow'
    ],
    ...
    aspects: [
        ...
        {   /*
            Аспект для основной панели Протокола
            */
            ...
            otherActions: function (actions) {
                ...
                actions[this.editPanelSelector + ' #cbIsPlaceResidenceOutState'] = { 'change': { fn: this.onChangeIsPlaceResidence, scope: this } };
                actions[this.editPanelSelector + ' #cbIsActuallyResidenceOutState'] = { 'change': { fn: this.onChangeIsActuallyResidence, scope: this } };   
                ...
                actions[this.editPanelSelector + ' #sfTransportOwner'] = { 'beforeload': { fn: this.onBeforeLoadTransport, scope: this } };
                actions[this.editPanelSelector + ' #sfContragent'] = { 'change': { fn: this.onChangeContragent, scope: this } };
              ...
              //actions[this.editPanelSelector + ' #cbTypeViolator'] = { 'change': { fn: this.onChangeTypeViolator, scope: this } };
                actions[this.editPanelSelector + ' #btnRepeat'] = { 'click': { fn: this.getRepeat, scope: this } };
                actions[this.editPanelSelector + ' #btnCopyButtonFactAddress'] = { 'click': { fn: this.btnClick, scope: this } };
                actions[this.editPanelSelector + ' #dfComissionMeeting'] = { 'change': { fn: this.onChangeComissionMeeting, scope: this } };
                actions[this.editPanelSelector + ' #tfProceedingsPlace'] = { 'change': { fn: this.onChangeProceedingsPlace, scope: this } };
            },
            onChangeIsPlaceResidence: function (field, newValue) {

                var form = field.up('#protocol197EditPanel'),
                tfPlaceResidenceOutState = form.down('#tfPlaceResidenceOutState');
                protocolFiasRegistrationAddressField = form.down('#protocolFiasRegistrationAddressField');
                if (newValue == true) {
                    tfPlaceResidenceOutState.show();
                    protocolFiasRegistrationAddressField.hide();

                }
                else {
                    tfPlaceResidenceOutState.hide();
                    protocolFiasRegistrationAddressField.show();
                }
            },
            onChangeIsActuallyResidence: function (field, newValue) {
                ...
                protocolFiaFactAddressField = form.down('#protocolFiaFactAddressField');
                btnCopyButtonFactAddress = form.down('#btnCopyButtonFactAddress');
                if (newValue == true) {
                    tfActuallyResidenceOutState.show();
                    protocolFiaFactAddressField.hide();
                    btnCopyButtonFactAddress.hide();

                }
                else {
                    tfActuallyResidenceOutState.hide();
                    protocolFiaFactAddressField.show();
                    btnCopyButtonFactAddress.show();
                }
            },
            onBeforeLoadTransport: function (field, options, store) {
                ...
                options.params.personFIO = record.get('Fio');
      
                return true;
            },
            btnClick: function (btn) {
                var form = btn.up('#protocol197EditPanel'),
                    pasteField = form.down('b4fiasselectaddress[name=FiasFactAddress]'),
                    jurAddressField = form.down('b4fiasselectaddress[name=FiasRegistrationAddress]');

                var copy = jurAddressField.getValue();

                if (!copy) {
                    Ext.Msg.alert('Внимание', 'Необходимо заполнить адрес регистрации!');
                    return;
                }
            ...
            getRepeat: function (btn) {               
         
                var cbHasAnotherResolutions = btn.up('#protocol197EditPanel').down('#cbHasAnotherResolutions'), //.down('#protocol197RequisitePanel')
                    me = this,
                    docid = this.controller.params.documentId;
                if (docid) {
                    me.controller.mask('Загрузка', me.controller.getMainComponent());
                    B4.Ajax.request(B4.Url.action('GetRepeatInfo', 'Protocol197', {
                        documentId: docid
                    })).next(function (response) {
                        //десериализуем полученную строку
                        var obj = Ext.JSON.decode(response.responseText);
                        if (obj.hasRepeated) {                           
                            cbHasAnotherResolutions.setValue(obj.hasRepeated);
                            Ext.Msg.alert('Уведомление', 'Обнаружена повторность');
                            me.controller.getStore('protocol197.AnotherResolution').load();
                        }
                        else {
                            Ext.Msg.alert('Уведомление', 'Данных по повторности не обнаружено');
                            me.controller.getStore('protocol197.AnotherResolution').load();
                        }
                        ...
                        me.controller.unmask();
                        Ext.Msg.alert('Ошибка проверки повторности!', e.message || e);
                    });
                }
            },
            onChangeTypeViolator: function (field, newValue) {
            
                var panel = this.getPanel(),
                    position = panel.down('#tfPosition'),
                    uridicalPerson = panel.down('#sfContragentName'),
                    PassportSeries = panel.down('#dfPassportSeries'),
                    PassportNumber = panel.down('#dfPassportNumber'),
                    PassportIssued = panel.down('#dfPassportIssued'),
                    DateIssue = panel.down('#dfDateIssue'),
                    DepartmentCode = panel.down('#dfDepartmentCode'),
                    DateBirth = panel.down('#tfDateBirth'),
                    BirthPlace = panel.down('#tfBirthPlace'),
                    Job = panel.down('#sfJob'),
                    name = panel.down('#sfName'),
                    PlaceResidence = panel.down('#sfPlaceResidence'),
                    ActuallyResidence = panel.down('#sfActuallyResidence');

                    cbIsActuallyResidenceOutState = panel.down('#cbIsActuallyResidenceOutState');
                    tfActuallyResidenceOutState = panel.down('#tfActuallyResidenceOutState');
                    tfPlaceResidenceOutState = panel.down('#tfPlaceResidenceOutState');
                    cbIsPlaceResidenceOutState = panel.down('#cbIsPlaceResidenceOutState');

                if (newValue == B4.enums.TypeViolator.PhisicalPerson) {
                    position.setDisabled(true);
                    uridicalPerson.setDisabled(true);
                    name.setDisabled(false);
                    cbIsPlaceResidenceOutState.setDisabled(false);
                    tfActuallyResidenceOutState.setDisabled(false);
                    tfPlaceResidenceOutState.setDisabled(false);
                    cbIsPlaceResidenceOutState.setDisabled(false);
                    PassportSeries.setDisabled(false);
                    PassportNumber.setDisabled(false);
                    PassportIssued.setDisabled(false);
                    DateIssue.setDisabled(false);
                    DepartmentCode.setDisabled(false);
                    DateBirth.setDisabled(false);
                    BirthPlace.setDisabled(false);
                    Job.setDisabled(false);
                    PlaceResidence.setDisabled(false);
                    ActuallyResidence.setDisabled(false);
                }
                else if (newValue == B4.enums.TypeViolator.OfficialPerson || newValue == B4.enums.TypeViolator.IP) {
                    position.setDisabled(false);
                    uridicalPerson.setDisabled(false);
                    name.setDisabled(false);
                }
                else if (newValue == B4.enums.TypeViolator.UridicalPerson) {
                    position.setDisabled(true);
                    uridicalPerson.setDisabled(false);
                    name.setDisabled(true);

                    PassportSeries.setDisabled(true);
                    PassportNumber.setDisabled(true);
                    PassportIssued.setDisabled(true);
                    DateIssue.setDisabled(true);
                    DepartmentCode.setDisabled(true);
                    DateBirth.setDisabled(true);
                    BirthPlace.setDisabled(true);
                    Job.setDisabled(true);
                    PlaceResidence.setDisabled(true);
                    ActuallyResidence.setDisabled(true);

                    cbIsPlaceResidenceOutState.setDisabled(true);
                    tfActuallyResidenceOutState.setDisabled(true);
                    tfPlaceResidenceOutState.setDisabled(true);
                    cbIsPlaceResidenceOutState.setDisabled(true);
                }

                //position.setDisabled(true);

                // typeviolator.setDisabled(!value || value == 1)

                //после добавления таблиц доделать привязку
            },
            ...
            onAfterSetPanelData: function (asp, rec, panel) {
                ...
                this.controller.getAspect('protocol197ViolationViolationAspect').doInjection();
            ...
            setTypeExecutantPermission: function (typeExec) {
                var me = this,
                    ...
                    tfPhysPerson = panel.down('#sfName'),
                    ...
                    tfDatePlaceOfBirth1 = panel.down('#tfDateBirth'),
                    tfDatePlaceOfBirth2 = panel.down('#tfBirthPlace'),

                    tfRegistrationAddress = panel.down('#protocolFiasRegistrationAddressField'),
                    tfFactAddress = panel.down('#protocolFiaFactAddressField');
                cbIsActuallyResidenceOutState = panel.down('#cbIsActuallyResidenceOutState');
                tfActuallyResidenceOutState = panel.down('#tfActuallyResidenceOutState');
                tfPlaceResidenceOutState = panel.down('#tfPlaceResidenceOutState');
                cbIsPlaceResidenceOutState = panel.down('#cbIsPlaceResidenceOutState');
                btnCopyButtonFactAddress = panel.down('#btnCopyButtonFactAddress');

                sfContragent.setDisabled(true);
                btnCopyButtonFactAddress.setDisabled(true);
                ...
                tfDatePlaceOfBirth1.setDisabled(true);
                tfDatePlaceOfBirth2.setDisabled(true);
                ...
                if (typeExec) {
                    ...
                    sfContragent.setDisabled(!(type === 1 || type === 2));
                    sfContragent.allowBlank = sfContragent.disabled;

                    // только для должностных лиц, граждан и ип
                    cbIsPlaceResidenceOutState.setDisabled(!(type === 1 || type === 3 || type === 4));
                    tfPlaceResidenceOutState.setDisabled(!(type === 1 || type === 3 || type === 4));
                    tfActuallyResidenceOutState.setDisabled(!(type === 1 || type === 3 || type === 4));
                    cbIsActuallyResidenceOutState.setDisabled(!(type === 1 || type === 3 || type === 4));
                    btnCopyButtonFactAddress.setDisabled(!(type === 1 || type === 3 || type === 4));
                    ...
                    // только для должностных лиц, граждан и ип
                    tfDatePlaceOfBirth1.setDisabled(!(type === 1 || type === 3 || type === 4));
                    tfDatePlaceOfBirth2.setDisabled(!(type === 1 || type === 3 || type === 4));
            ...
            onChangeContragent: function (field, newValue) {
                ...
                if (me.controller.afterset) {
                    var panel = this.getPanel(),
                        regAddress = panel.down('#protocolFiasRegistrationAddressField'),
                        factAddress = panel.down('#protocolFiaFactAddressField'),
                        phoneNumber = panel.down('#tfPhoneNumber');
                    if (newValue) {
                        if (newValue.FiasJuridicalAddress > 0 && !regAddress.rawValue) {
                            B4.Ajax.request(B4.Url.action('GetFiasProperty', 'Protocol197', {
                                fiasid: newValue.FiasJuridicalAddress
                            }
                            )).next(function (response) {
                                var data = Ext.decode(response.responseText);
                                if (data.data != null) {
                                    data.data.Id = 0;
                                    regAddress.setValue(data.data);
                                }                               
                            }).error(function () {

                            });                 
                            me.controller.afterset = false;
                        }
                        if (newValue.FiasFactAddress > 0 && !factAddress.rawValue) {
                            B4.Ajax.request(B4.Url.action('GetFiasProperty', 'Protocol197', {
                                fiasid: newValue.FiasFactAddress
                            }
                            )).next(function (response) {
                                var data = Ext.decode(response.responseText);
                                if (data.data != null) {
                                    data.data.Id = 0;
                                    factAddress.setValue(data.data);
                                }
                            }).error(function () {
                        ...
                        if (newValue.Phone != null && !phoneNumber.rawValue)
                        {
                            phoneNumber.setValue(newValue.Phone);
                        }
                    }
                }
            },
            ...
            onChangeComissionMeeting: function (field, newValue) {
                var panel = this.getPanel(),
                    dfComissionMeeting = panel.down('#dfComissionMeeting');
                dfComissionMeeting.setDisabled(!newValue);
            },
            onChangeProceedingsPlace: function (field, newValue) {
                var panel = this.getPanel(),
                    tfProceedingsPlace = panel.down('#tfProceedingsPlace');
                tfProceedingsPlace.setDisabled(!newValue);
            }
        },
        {
            /*
        ...
        },
        {
            ...
            */
            ...
            otherActions: function (actions) {
                actions[this.gridSelector + ' #massAddition'] = { 'click': { fn: this.doMassAddition, scope: this } };
                actions['protocol197massaddwindow b4savebutton'] = { 'click': { fn: this.saveMassAddition, scope: this } };
                actions['protocol197massaddwindow b4closebutton'] = { 'click': { fn: this.closeMassAddition, scope: this } };
            },
            listeners: {
            ...
            closeMassAddition: function (btn) {
                btn.up('protocol197massaddwindow').close();

            },
            saveMassAddition: function (btn) {
                var me = this,
                    form = btn.up('protocol197massaddwindow').down('form'),
                    values = form.getValues();
                    values.protId = me.controller.params.documentId;
             
                form.disable();
                B4.Ajax.request({
                    url: B4.Url.action('MassAddition', 'Protocol197Annex'),
                    params: values,
                ...
                btn.up('protocol197massaddwindow').close();
                me.controller.getStore('protocol197.Annex').load();
            ...
            doMassAddition: function () {
                ...
                if (!me.massAddWindow) {
                    me.massAddWindow = Ext.create('B4.view.protocol197.MassAdditionWindow', {
                        modal: true,
                        closeAction: 'hide'
                    });

                    var b4Combo = me.massAddWindow.down('b4enumcombo[name="TypeAnnex"]');
                    var cbIsProof = me.massAddWindow.down('#cbIsProof');
                    var nfCount = me.massAddWindow.down('#nfCount');

                   
                }
                me.massAddWindow.show();
                var form = me.massAddWindow.down('form');
                form.enable();
            },
        ...
        },
        {
            ...
            */
            xtype: 'grideditwindowaspect',
            name: 'protocol197PetitionAspect',
            gridSelector: 'protocol197petitiongrid',
            editFormSelector: '#protocol197PetitionEditWindow',
            storeName: 'protocol197.Petition',
            modelName: 'protocol197.Petition',
            editWindowView: 'protocol197.PetitionEditWindow',         
            listeners: {
                getdata: function (asp, record) {
                    if (!record.get('Id')) {
                        record.set('Protocol197', this.controller.params.documentId);
        ...
        },
        {
            ...
            */
            xtype: 'gkhinlinegridmultiselectwindowaspect',
            name: 'protocol197AnotherResolutionAspect',
            gridSelector: 'protocol197anotherresolutionGrid',
            saveButtonSelector: 'protocol197ArticleLawGrid #protocolSaveButton',
            storeName: 'protocol197.AnotherResolution',
            modelName: 'protocol197.AnotherResolution',
            multiSelectWindow: 'SelectWindow.MultiSelectWindow',
            multiSelectWindowSelector: '#protocol197AnotherResolutionMultiSelectWindow',
            storeSelect: 'dict.ArticleLawGjiForSelect',
            storeSelected: 'dict.ArticleLawGjiForSelected',
            titleSelectWindow: 'Выбор статей закона',
            titleGridSelect: 'Статьи для отбора',
        ...
        {
            xtype: 'gkhblobtextaspect',
            name: 'protocol197ViolationViolationAspect',
    ...
    init: function () {
        ...
        actions['doccomissiongrid'] = {
            'gotocomussion': { fn: this.rowActionProt, scope: this }
        };
        ...
        me.getStore('comission.ListComissions').on('beforeload', me.onBeforeLoad, me);
        me.getStore('protocol197.AnotherResolution').on('beforeload', me.onBeforeLoad, me);
        me.getStore('protocol197.Annex').on('beforeload', me.onBeforeLoad, me);
        me.getStore('protocol197.Petition').on('beforeload', me.onBeforeLoad, me);
        me.control(actions);
    ...
    rowActionProt: function (record) {
        ...
        var me = this,
            ...
            controllerEditName = 'B4.controller.ComissionMeeting',
            params = {};
    ...
    onLaunch: function () {
        ...
        if (me.params) {
          ...
          //  me.params.documentId = me.params.documentId;
            ...
            me.getStore('comission.ListComissions').load();

            //Обновляем стор приложений
            me.getStore('protocol197.Annex').load();
            //Обновляем стор ходатайств
            me.getStore('protocol197.Petition').load();

            //Обновляем стор статьей закона
            me.getStore('protocol197.ArticleLaw').load();
            me.getStore('protocol197.AnotherResolution').load();
        }
    },
