    ...
    public class DistributionProviderImpl : IDistributionProvider
    {
        ...
        protected virtual IDataResult ApplyInternal(BaseParams baseParams)
        {
            ...
            try
            {
                ...
                if (distributables.IsEmpty())
                {
                    return new BaseDataResult(false, "Не удалось получить распределяемые объект");
                }
                ...
                using (var tr = this.container.Resolve<IDataTransaction>())
                {
                    ...
                    try
                    {
                        foreach (var distributable in distributables)
                        {
                            var args = DistributionProviderImpl.ExtractArgs(distribution, baseParams, distributable);
        ...
        internal static IDistributionArgs ExtractArgs(IDistribution distribution, BaseParams baseParams, IDistributable distributable)
        {
            if (distribution.DistributableAutomatically)
            {
