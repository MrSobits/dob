namespace Bars.Gkh.RegOperator.Domain
{
    ...
    public partial class RefactorConvertor
    {
        ...
        public RefactorConvertor(IWindsorContainer container)
        {
            ...
            this.logger = container.Resolve<ILogManager>();
        ...
        public int Process()
        {
            ...
            this.logger.Debug("Начало миграции данных");
            ...
            this.logger.Debug("Создание трансферов для проведенных начислений");

            ProcessCharges();

            this.logger.Debug("Созданы трансферы для проведенных начислений");

            sessions.CloseCurrentSession();

            // получаем трансферы для ручного изменения пени
            this.logger.Debug("Создание трансферов для изменений пеней");
            container.InTransaction(() => CreateTransferForPenaltyChanges(transfers));
            this.logger.Debug("Созданы трансферы для изменения пеней");

            sessions.CloseCurrentSession();

            // По аренде
            this.logger.Debug("Создание трансферов для проведенных распределений по оплате аренды");
            ...
            this.logger.Debug("Создание трансферов для проведенных распределений по предыдущим работам");
            container.InTransaction(() => data.AddRange(
            ...
            this.logger.Debug("Создание трансферов для проведенных распределений по накопленным средствам");
            container.InTransaction(() => data.AddRange(
            ...
            this.logger.Debug("Создание трансферов для оплат по ЛС");
            ...
            this.logger.Debug("Создание трансферов для оплат по домам");
            container.InTransaction(() => ApplyRealityObjectPayments(transfers));

            sessions.CloseCurrentSession();

            // Проводим оплаты по актам
            this.logger.Debug("Создание трансферов для оплат по актам");
            ...
            this.logger.Debug("Обновление балансов");
            UpdateBalances();
        ...
        private void UpdateBalances()
        {
            ...
            {
                this.logger.DebugFormat("Обновление балансов для домов {0}/{1}", processed, idsCount);
                ...
                {
                    this.logger.Debug("Обновление балансов ЛС");
                    ...
                    this.logger.Debug("Копирование трансферов из ЛС в дома");
...
WHERE
    ID = ch2.acc_id").ExecuteUpdate();
        ...
        {
            this.logger.Debug("Обновление счетов оплат домов");
        ...
        {
            this.logger.Debug("Обновление балансов кошельков");
    ...
    wallet.ID = w2.ID").ExecuteUpdate();
        ...
        private void CreateLoanWalletsAndTransfers()
        {
            ...
            using (var tr = container.Resolve<IDataTransaction>())
            {
                ...
                {
                    tr.Commit();
                    _logger.Debug("Миграция займов завершена");
                }
                catch
                {
                    tr.Rollback();
