namespace Bars.Gkh.RegOperator.DomainModelServices.Impl
{
    ...
    public class BalanceUpdateChecker : IBalanceUpdateChecker
    {
        ...
        public void PerformUpdate()
        {
            ...
            var logger = _container.Resolve<ILogManager>();
            ...
            logger.Info("Старт расчета балансов");
                ...
                () =>
                {
                    if (
                        !mutexManager.TryLock(
                            ...
                            out handle))
                    {
                        logger.Info("Уже идет другой расчет. Выход");
                        return;
                    }
                    ...
                    {
                        logger.Info("Получение информации по изменениям для ЛС");
...
join gkh_reality_object ro on ro.id = acc.ro_id").List<object>().Select(x => x.ToLong());
                        ...
                        logger.Info("Получение информации по изменениям для счетов оплат дома");

                        var allRoIds = rosFromAccs.Union(rosFromPayments).Distinct().ToList();

                        foreach (var roIds in allRoIds.Section(500))
                        {
                            logger.Info("Расчет балансов для {0} домов".FormatUsing(roIds.Count()));
        ...
        {
            ExplicitSessionScope.CallInNewScope(PerformUpdate);
        }
    }
}