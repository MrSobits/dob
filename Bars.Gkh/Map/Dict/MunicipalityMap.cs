...
namespace Bars.Gkh.Map
{
    ...
    public class MunicipalityMap : BaseImportableEntityMap<Municipality>
    {
        ...
        protected override void Map()
        {
            ...
            Property(x => x.UFC, "УФК").Column("UFC");
            Property(x => x.BankName, "Наименование банка").Column("BANKNAME");
            Property(x => x.PersonalAcc, "Расч. счет").Column("PERSACC");
            Property(x => x.CorrespAcc, "Корр. счет").Column("CORRACC");
            Property(x => x.Index, "Индекс").Column("INDEX");
        }
    }
}
