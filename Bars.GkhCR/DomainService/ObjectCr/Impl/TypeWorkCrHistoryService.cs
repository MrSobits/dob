    ...
    public class TypeWorkCrHistoryService : ITypeWorkCrHistoryService
    {
        ...
        public IDataResult Restore(BaseParams baseParams)
        {
            ...
            try
            {
                ...
                {
                    return new BaseDataResult(false, "Долгосрочная программа актуализирована на основе данных текущей Краткосрочной программы. Вид работ или КЭ был удален из Долгосрочной программы. Восстановление записи невозможно.");
                }

                var typeWorkWithSameYear = twDomain.GetAll().FirstOrDefault(x => x.ObjectCr.Id == typeWork.ObjectCr.Id && x.YearRepair == typeWork.YearRepair 
                    && x.Work.Id == typeWork.Work.Id && x.Id != typeWork.Id);

                if (typeWorkWithSameYear != null)
                {
                    dpkrTypeWorkService.MergeStructElementInTypeWork(typeWork, typeWorkWithSameYear);

                    twRemovalDomain.GetAll()
                        .Where(x => x.TypeWorkCr.Id == typeWork.Id)
                        .ToList()
                        .ForEach(x => twRemovalDomain.Delete(x.Id));

                    twHistoryDomain.Delete(history.Id);
                    twDomain.Delete(typeWork.Id);
                }
                else
                {
