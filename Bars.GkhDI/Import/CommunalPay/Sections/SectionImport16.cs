namespace Bars.GkhDi.Import.Sections
{
    ...
    {
        public IDomainService<FinActivityManagRealityObj> FinActivityRealityObjService { get; set; }
        ...
        public string Name
        {
            get { return "Импорт из комплат секция #16"; }
        }
        ...
        public void ImportSection(ImportParams importParams)
        {
            ...
            var finActManagRealObjs = this.FinActivityRealityObjService.GetAll().Where(x => x.DisclosureInfo.Id == disclosureInfo.Id).ToList();

            var disclosureInfoRealityObjs = this.DisclosureInfoRealityObjService.GetAll()
                .Where(x => x.PeriodDi.Id == importParams.PeriodDiId && realityObjects.Contains(x.RealityObject.Id))
                .ToList();

            var realObjs = RealityObjectService.GetAll().Where(x => realityObjects.Contains(x.Id)).ToList();

            var managingOrganization = this.Container.Resolve<IDomainService<ManagingOrganization>>().GetAll().FirstOrDefault(x => x.Contragent.Inn == inn);
            ...
            foreach (var section16Record in sectionsData.Section16)
            {
                ...
                {
                    logImport.Warn(this.Name, string.Format("Общая площадь жилых и нежилых помещений в МКД должна быть не меньше, чем жилых всего {0}", section16Record.CodeErc));
                }
                else
                {
                ...
                if (section16Record.WallMaterial.IsNotEmpty())
                {
                    ...
                    this.PassportProvider.UpdateForm(realityObject.Id, "Form_5_2", values);
                }
                else
                {
                    logImport.Warn(this.Name, string.Format("Не удалось получить Материал несущих стен с кодом {0}", section16Record.WallMaterialCode));
                }
                ...
                if (section16Record.ChuteNum != null)
                {
                    ...
                    this.PassportProvider.UpdateForm(realityObject.Id, "Form_3_7", values);
                }
            ...
            this.InTransaction(resultFinActivity, this.FinActivityRealityObjService);
            this.InTransaction(resultRealObj, this.RealityObjectService);
            this.InTransaction(resultInfoRealObj, this.DisclosureInfoRealityObjService);
        }
