namespace Bars.Gkh.RegOperator.Report
{
	...
	public class FundFormationStimulReport : StimulReport, IGeneratedPrintForm
	{
		...
		public void PrepareReport(ReportParams reportParams)
		{
			...
			this.Report.RegData("Сведения", infoList);

			this.Report["ДатаГенерации"] = DateTime.Now.ToShortDateString();
			this.Report["Квартал"] = ((DateTime.Now.Month / 4) + 1).ToString();
			...
			this.Report["СпособФормированияФонда"] = formationTypeText;
