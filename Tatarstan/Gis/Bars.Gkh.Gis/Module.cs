namespace Bars.Gkh.Gis
{
    ...
    using Bars.B4.Modules.ReportPanel.Domain;
    using Bars.B4.Utils;
    ...
    public partial class Module : AssemblyDefinedModule
    {
        public override void Install()
        {
            ...
            this.Container.RegisterTransient<IBillingReport, ReportIndicator>("Bars.Gkh.Gis.Reports ReportIndicator");
        ...
        public void RegisterRabbitMq()
        {
            ...
            this.Container.RegisterTransient<IProducerService, ProducerService>();
            this.Container.RegisterTransient<IConsumerService, ConsumerService>();

            var appSettings = this.Container.Resolve<IConfigProvider>().GetConfig().AppSettings;
            var rabbitEnable = appSettings[SettingsKeyStore.Enable].To<bool>();
            if (!rabbitEnable)
            {
                return;
            }
            ...
            foreach (var task in (TypeQueueName[]) System.Enum.GetValues(typeof(TypeQueueName)))
            {
                consumerService.StartConsuming(
                    appSettings[SettingsKeyStore.Ip].To<string>(),
                    appSettings[SettingsKeyStore.Port].To<int>(),
                    appSettings[SettingsKeyStore.Login].To<string>(),
                    appSettings[SettingsKeyStore.Password].To<string>(),
                    task.GetEnumMeta().Display);
            }
        }
