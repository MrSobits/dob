    ...
    public class ReformaIntegrationTask : BaseTask
    {
        ...
        public override void Execute(DynamicDictionary @params)
        {
            ...
            try
            {
                ...
                ExplicitSessionScope.CallInNewScope(this.Execute);
            }
            catch (Exception e)
            {
                this.Container.UsingForResolved<ILogManager>((container, manager) => manager.Fatal(e.ToString(), e));
            }
            finally
            {
                lock (ReformaIntegrationTask.syncObject)
        ...
        {
            var provider = this.Container.Resolve<ISyncProvider>(new { this.typeIntegration });
            try
            {
                // синхронизация УО
