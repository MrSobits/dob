namespace Bars.GkhGji.InspectionRules
{
    ...
    /// </summary>
    public class ProtocolToResolutionRule : IDocumentGjiRule
    {
        ...
        public IDataResult CreateDocument(DocumentGji document)
        {
            ...
            var protocol = ProtocolDomain.GetAll()
                                    ...
                                    .Select(x => new { x.Executant, x.Contragent, x.PhysicalPerson, x.PhysicalPersonInfo, x.Description,
                                        x.PhysicalPersonDocType, x.PhysicalPersonDocumentNumber, x.PhysicalPersonDocumentSerial, x.PhysicalPersonIsNotRF, x.PhysicalPersonPosition })
                                    .FirstOrDefault();
            ...
            if (!string.IsNullOrEmpty(protocol.PhysicalPerson))
            {
                resolution = new Resolution()
                {
                    ...
                    Position = protocol.PhysicalPersonPosition,
                    Surname = protocol.PhysicalPerson.Split(' ').Length > 2 ? protocol.PhysicalPerson.Split(' ')[0] : "",
                    FirstName = protocol.PhysicalPerson.Split(' ').Length > 2 ? protocol.PhysicalPerson.Split(' ')[1] : "",
                    Patronymic = protocol.PhysicalPerson.Split(' ').Length > 2 ? protocol.PhysicalPerson.Split(' ')[2] : "",
                    PhysicalPerson = protocol.PhysicalPerson,
                    PhysicalPersonInfo = protocol.PhysicalPersonInfo,
                    PhysicalPersonDocType = protocol.PhysicalPersonDocType,
                    PhysicalPersonDocumentNumber = protocol.PhysicalPersonDocumentNumber,
                    PhysicalPersonDocumentSerial = protocol.PhysicalPersonDocumentSerial,
                    PhysicalPersonIsNotRF = protocol.PhysicalPersonIsNotRF,
        ...
        public virtual void CopyingPhysicalPersons(long protocolId, ref Resolution resolution) { }
    }
}
