    ...
    public partial class DataTransferProvider
    {
            ...
            IList<Entity> entitiesToExport)
        {
            ...
            try
            {
                ...
                {
#if DEBUG
                    this.LogManager.Info($"Старт выгрузки секции {entity.Type.FullName}({processedEntities.Count}/{entitiesToExport.Count})");
                    var sw = Stopwatch.StartNew();
#endif
                    var items = this.GetItemsFromDb(typeInfoDict.Get(entity.Type));
#if DEBUG
                    this.LogManager.Info($"Секция выгружена за {sw.Elapsed}");
#endif
