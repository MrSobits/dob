namespace Bars.GkhDi.Import.Sections
{
    ...
    public class SectionImport14 : ISectionImport
    {
        public string Name
        {
            get
            {
                return "Импорт из комплат секция #14";
            }
        }
        ...
        public void ImportSection(ImportParams importParams)
        {
            ...
            foreach (var section14Record in sectionsData.Section14)
            {
                ...
                {
                    logImport.Warn(this.Name, string.Format("Для дома с кодом ЕРЦ {0} и упр. организацией с инн {1} нет договора управления в данном периоде", section14Record.CodeErc, inn));
                    continue;
                }
                ...
                var nonResidentPlacement = nonResidentPlacementServiceDict.ContainsKey(disclosureInfoRealityObj.Id) ? nonResidentPlacementServiceDict[disclosureInfoRealityObj.Id].FirstOrDefault(x => x.ContragentName == section14Record.ContragentName) : null;

                if (nonResidentPlacement == null)
                {
                    logImport.Info(this.Name, string.Format("Добавлена запись в раздел 'Сведения об использование нежилых помещений' для {0}", section14Record.ContragentName));
