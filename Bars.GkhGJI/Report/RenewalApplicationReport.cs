        ...
        public override void PrepareReport(ReportParams reportParams)
        {
            ...
            try
            {
               ...
               .Where(x => x.Id == _renewalApplicationReportId).FirstOrDefault();
                ...
                Report["НачальникДолжность"] = parentDisposal.IssuedDisposal.Position + (zonal != null ? " " + zonal.ZonalInspection.NameGenetive : "");
                Report["НачальникФИО"] = parentDisposal.IssuedDisposal.Fio;
                Report["ДатаРешения"] = prescription.RenewalApplicationDate.HasValue ? prescription.RenewalApplicationDate.Value.ToString("dd:MM:yyyy") : DateTime.Now.ToString("dd:MM:yyyy");
                Report["Контрагент"] = prescription.Contragent != null ? prescription.Contragent.Name : "";
                if (renewalAnnex != null)
                {
                    Report["Ходатайство"] = renewalAnnex.DocumentDate.HasValue ? renewalAnnex.DocumentDate.Value.ToString("dd:MM:yyyy") + " №" + renewalAnnex.Number : "№ " + renewalAnnex.Number;
                }
                Report["НомерПредписания"] = prescription.DocumentNumber;
                Report["ДатаПредписания"] = prescription.DocumentDate.HasValue ? prescription.DocumentDate.Value.ToString("dd:MM:yyyy") : "";

                var maxDate = violationsList.Max(x => x.DatePlanExtension);

                Report["ДатаПродления"] = maxDate.HasValue ? maxDate.Value.ToString("dd:MM:yyyy") : "";
