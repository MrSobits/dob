        ...
        protected override IDictionary<long, DuProxy> GetCache()
        {
            ...
            using (this.Container.Using(contractOwnersRepository, contractTransferRepository))
            {
                ...
                var manOrgContractOwnersList = this.FilterService
                    .FilterByContragent(contractOwnersRepository.GetAll(), x => x.ManagingOrganization.Contragent)
                ...
                var manOrgContractTransferList = this.FilterService
                    .FilterByContragent(contractTransferRepository.GetAll(), x => x.ManagingOrganization.Contragent)
                    .Where(x => !x.EndDate.HasValue || x.EndDate > DateTime.Today)
                    .Select(x => new
                    {
                        x.Id,
