namespace Bars.Gkh.Decisions.Nso.Domain.Impl
{
    ...
    public class RealityObjectDecisionService : IRealityObjectDecisionsService
    {
        ...
        public Dictionary<long, IGrouping<long, T>> GetActualDecisions<T>(List<long> realityObjectIds) where T : UltimateDecision
        {
            var protocols = this.roDecisionProtocolDomainService.GetAll()
                .Where(x => realityObjectIds.Contains(x.RealityObject.Id)).Select(x => x.Id);

            var domain = this.container.ResolveDomain<T>();
            using (this.container.Using(domain))
            {
                var decisions = domain.GetAll()
                    .Where(x => protocols.Any(p => p == x.Protocol.Id));
        ...
        /// <returns></returns>
        public Dictionary<long, IEnumerable<Tuple<DateTime, CrFundFormationDecisionType>>> GetRobjectsFundFormation(ICollection<long> roIds = null)
        {
        ...
        /// <returns></returns>
        public Dictionary<long, IEnumerable<Tuple<DateTime, CrFundFormationDecisionType>>> GetRobjectsFundFormation(IQueryable<long> roIdsQuery = null)
        {
            var session = this.container.Resolve<ISessionProvider>().GetCurrentSession();
            var oldFlushMode = session.FlushMode;
            session.FlushMode = FlushMode.Never;
