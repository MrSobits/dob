namespace Bars.Gkh.FormatDataExport.ExportableEntities
{
    ...
    public abstract partial class BaseExportableEntity : IExportableEntity
    {
        ...
        /// Параметры экспорта
        /// </summary>
        protected FormatDataExportConfig Config { get; private set; }

        /// <inheritdoc />
        public abstract IList<string> GetHeader();

        /// <inheritdoc />
        public virtual IList<List<string>> GetData(DynamicDictionary baseParams)
        {
            this.SelectParams = baseParams ?? new DynamicDictionary();
            this.Config = this.Container.GetGkhConfig<AdministrationConfig>().FormatDataExport;
            ...
            if (this.Config.FormatDataExportGeneral.NoEmptyMandatoryFields && this.EmptyMandatoryFields.IsNotEmpty())
            {
        ...
        protected IEnumerable<T> AddFilesToExport<T>(IEnumerable<T> entityData, Func<T, FileInfo> fileSelector)
        {
            ...
            if (this.Config.FormatDataExportGeneral.OnlyExistsFiles)
            {
                return entityData.Where(x => existsFiles.Contains(fileSelector.Invoke(x).Id));
            }
        ...
        where T : IEntity
    {
        ...
        /// <summary>
        /// Отфильтрованный по жилым домам запрос получения сущностей типа <typeparamref name="T"/>
        /// </summary>
        /// <param name="realityObjectSelector">Селектор жилого дома</param>
        protected IQueryable<T> GetFiltred(Expression<Func<T, RealityObject>> realityObjectSelector)
        {
            return this.FilterService.FilterByRealityObject(this.EntityRepository.GetAll(), realityObjectSelector);
        }
    }
