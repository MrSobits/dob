namespace Bars.GkhGji.Report
{
    ...
    public abstract class GjiBaseStimulReport : GkhBaseStimulReport
    {
        ...
        protected void FillCommonFields(DocumentGji doc)
        {
            ...
            if (firstRo != null)
            {
                ...
                if (zonalInspection != null)
                {
                    ...
                    Report["Email"] = zonalInspection.Email;
                    Report["ЗональноеНаименование1ГосЯзык(ТворП)"] = zonalInspection.NameAblative;
                    Report["ЗональноеНаименование1ГосЯзык(РП)"] = zonalInspection.NameGenetive;
                    Report["ЗональноеНаименование1ГосЯзык(ВП)"] = zonalInspection.NameAccusative;
                }
                ...
                if (doc.Inspection != null)
                {
                    ...
                    {
                        Report["УправОрг(РП)"] = doc.Inspection.Contragent.NameGenitive;
                        Report["УправОрг"] = doc.Inspection.Contragent.Name;
                        Report["АдресКонтрагента"] = doc.Inspection.Contragent.FiasJuridicalAddress != null
                    ...
                    if (doc.Inspection.TypeBase == TypeBase.CitizenStatement)
                    {
                        ...
                        if (appeals.Count > 0)
                        {
                            ...
                            Report["АдресКорреспондента"] = String.Join(",",
        ...
        {
            Report.RegData(dataSourceName, inspectiors.ToList().Select(x => new Инспектор(x)));
        }

        protected void RegViolDataSource(IEnumerable<InspectionGjiViol> violations, string dataSourceName = "Нарушения")
        {
            Report.RegData(dataSourceName, violations.ToList().Select(x => new Нарушение(x)));
        }
