namespace Bars.Gkh.RegOperator.DomainService.BankDocumentImport.Impl
{
    ...
    #region Properties
            ...
            IProgressIndicator progressIndicator = null)
        {
            ...
            catch (Exception exception)
            {
                ...
                this.Container.InTransactionInNewScope(() =>
                {
                    ...
                    this.LogManager.Error("Ошибка отмены оплат", exception);
                });
            ...
            foreach (var bankDocumentImport in bankDocumentImports)
            {
               ...
               var isSkipDocument = this.ImportedPaymentDomain.GetAll()
                ...
                if (isSkipDocument)
                {
                    ...
                    this.LogManager.Error($"Ошибка отмены реестра({errorDoc})");
                ...
                catch (Exception exception)
                {
                    ...
                    this.Container.InTransactionInNewScope(() =>
                    {
                        ...
                        errorDocuments.Add(errorDoc);
                        this.LogManager.Error($"Ошибка отмены реестра({errorDoc})", exception);
                    });
                }
                finally
                {
            ...
            foreach (var bankDocumentImport in bankDocumentImports)
            {
                ...
                try
                {
                    ...
                    catch (Exception exception)
                    {
                        ...
                        errorDocuments.Add(errorDoc);
                        this.LogManager.Error($"Ошибка подтверждения реестра({errorDoc})", exception);
                    }
                ...
                catch (Exception exception)
                {
                    ...
                    ExplicitSessionScope.CallInNewScope(() => this.Container.InTransaction(() =>
                    {
                        progressLogger.SetError(exception.ToString());
        ...
        public IDataResult SetPaymentsNotDistributed(BankDocumentImport document)
        {
            ...
            {
                this.LogManager.Error("Ошибка актуализации статусов после неуспешного подтвреждения реестра", exception);
                return BaseDataResult.Error(exception.Message);
            }
                ...
                if (log != null)
                {
                    ...
                    var report =
                        ...
                        this.Container.Resolve<IBaseReport>(
                            ...
                            new {docDate, docNum, docSum, results });
