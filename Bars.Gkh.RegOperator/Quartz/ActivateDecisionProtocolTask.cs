    ...
    public class ActivateDecisionProtocolTask : BaseTask
    {
        ...
        public override void Execute(DynamicDictionary @params)
        {
            ExplicitSessionScope.CallInNewScope(() =>
            {
                ...
                logger.Info("Задание {0} успешно запущено в {1}".FormatUsing("ActivateDecisionProtocolTask", DateTime.Now));

                try
                {
                    this.ExecuteInternal(logger);
                    logger.Info("Задание {0} успешно завершено в {1}, затрачено {2:g}".FormatUsing("ActivateDecisionProtocolTask", DateTime.Now, stopWatch.Elapsed));
                }
                catch (Exception exception)
                {
                    logger.Error("Ошибка во время выполнения задания ActivateDecisionProtocolTask", exception);
                }
        ...
        private void ExecuteInternal(ILogManager logger)
        {
