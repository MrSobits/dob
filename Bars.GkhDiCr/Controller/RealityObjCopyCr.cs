...
#warning Убрать из контроллера логику в сервисы и вью-модели
    public class RealityObjCopyCrController : DataController<ManOrgContractRealityObject>
    {
        ...
        {
            var disclosureInfoId = Request["disclosureInfoId"].ToLong();
            var programCrId = Request["programCrId"].ToLong();
        ...
        public ActionResult LoadWorkCr()
        {
            try
            {
                var realityObjIds = !string.IsNullOrEmpty(Request["realityObjIds"].ToStr()) ? Request["realityObjIds"].ToStr().Split(',').Select(x => x.ToLong()).ToArray() : new long[0];
