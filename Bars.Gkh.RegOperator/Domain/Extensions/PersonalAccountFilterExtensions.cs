    ...
    public static class PersonalAccountFilterExtensions
    {
        ...
        public static IQueryable<PersonalAccountDto> ToDtoInCurrentPeriod(this IQueryable<BasePersonalAccount> query, bool joinPeriodSummary)
        {
            ...
            using (container.Using(periodSummaryDomain, chargeDomain, personalAccountPrivilegedCategies, chargePeriodRepository))
            {
                ...
                {
                    var summary = periodSummaryDomain.GetAll()
                       .Where(sum => sum.Period.Id == periodId)
                       .Join(query,
                           sum => sum.PersonalAccount.Id,
                           x => x.Id,
        ...
        public static IQueryable<BillingPersonalAccountDto> ToBillingDto(this IQueryable<BasePersonalAccount> query, BaseParams baseParams)
        {
            ...
            try
            {
                ...
                //запрос абонента, актуального для лс на конец выбранного периода
                var persAccHistoryQuery = ownershipHistoryDomain.GetAll()
                    .Where(x => x.Date <= (period.EndDate ?? DateTime.Now))
                ...
                return query
                    .Where(x => persAccHistoryQuery.Any(y => y.Id == x.Id))
                    .Select(
                        x =>
                            new BillingPersonalAccountDto
                            {
                                Id = x.Id,
                                OwnerId = persAccHistoryQuery.Where(y => y.Id == x.Id).Select(y => y.AccountOwner.Id).FirstOrDefault(),
                                OwnerType = persAccHistoryQuery.Where(y => y.Id == x.Id).Select(y => y.AccountOwner.OwnerType).FirstOrDefault(),
                                AccountOwner = persAccHistoryQuery.Where(y => y.Id == x.Id)
                                ...
                                PrivilegedCategoryId = persAccHistoryQuery.Where(y => y.Id == x.Id).Select(y => y.AccountOwner.PrivilegedCategory.Id).FirstOrDefault(),
                                HasOnlyOneRoomWithOpenState = x.State.StartState
                                    && persAccHistoryQuery.Where(y => y.Id == x.Id).Select(y => y.AccountOwner.ActiveAccountsCount).FirstOrDefault() == 1,

                                Email = persAccHistoryQuery.Where(y => y.Id == x.Id)
                                ...
                                AddressOutsideSubject = persAccHistoryQuery.Where(y => y.Id == x.Id)
                                    .Select(y => y.AccountOwner is IndividualAccountOwner
                                ...
                                LegalFactAddress = persAccHistoryQuery.Where(y => y.Id == x.Id)
        ...
        public static IQueryable<PersonalAccountDto> GetDtoQuery(this IQueryable<BasePersonalAccount> query, long periodId, bool joinQuery = false, bool joinPeriodSummary = false)
        {
            ...
            using (container.Using(dtoDomain, chargeDomain, privilegedCategiesDomain, periodSummaryDomain))
            {
                if (joinPeriodSummary)
                {
                    try
                    {
                        var ps = periodSummaryDomain.GetAll()
                            .Where(sum => sum.Period.Id == periodId)
