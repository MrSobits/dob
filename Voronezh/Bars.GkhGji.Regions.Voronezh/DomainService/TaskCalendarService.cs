namespace Bars.GkhGji.Regions.Voronezh.DomainService
{
    ...
    public class TaskCalendarService : ITaskCalendarService
    {
            ...
            var predata = serviceViewProtocol.GetAll()
              .Where(x => protCnt.Contains(x.Id))
                .Select(x => new ProtocolProxy
                {
                    ...
                    CaseNumber = GetInspNumber(x.InspectionId),
                    ViolatorFio = GetVolator(x.Id)
                }).ToList();
        ...
        public IDataResult GetListProtocolsInCommission(BaseParams baseParams)
        {
            var loadParam = baseParams.GetLoadParam();
            var comissionId = baseParams.Params.GetAs<long>("comissionmeetId");
            ...
            var docIds = ComissionMeetingDocumentDomain.GetAll()
                .Where(x => x.ComissionMeeting.Id == comissionId)
                .Select(x => x.DocumentGji.Id).Distinct().ToList();


            var protCnt = ProtocolDomain.GetAll()
                .Where(x=> docIds.Contains(x.Id))
       //   .Where(x => x.ComissionMeeting != null && x.ComissionMeeting.Id == comissionId)
          .Select(x => x.Id).Distinct().ToList();
            var prot197Cnt = Protocol197Domain.GetAll()
                .Where(x => docIds.Contains(x.Id))
        //  .Where(x => x.ComissionMeeting != null && x.ComissionMeeting.Id == comissionId)
                ...
                var predata = serviceViewProtocol.GetAll()
                  .Where(x => protCnt.Contains(x.Id))
                    ...
                    .Select(x => new ProtocolProxy
                    {
                        ...
                        ControlType = x.ControlType,
                        TypeDocumentGji = x.TypeDocumentGji,
                        ComissionDocumentDecision = GetCurrentDecision(x.Id, comprx),
                        ArticleLaw = x.ArticleLaw,
                        CaseNumber = GetInspNumber(x.InspectionId),
                        ViolatorFio = GetVolator(x.Id),
                        DateViolation = x.DateViolation,
                        Penalty = x.Penalty
                    }).ToList();
        ...
        public IDataResult GetListResolutionsInCommission(BaseParams baseParams)
        {
            var loadParam = baseParams.GetLoadParam();
            var comissionId = baseParams.Params.GetAs<long>("comissionmeetId");
            ...
            var ProtocolDomain = this.Container.Resolve<IDomainService<DocumentGjiChildren>>();
            var Protocol197Domain = this.Container.Resolve<IDomainService<DocumentGjiChildren>>();

            var ResolutionDomain = this.Container.Resolve<IDomainService<Resolution>>();

            var docIds = ComissionMeetingDocumentDomain.GetAll()
                .Where(x => x.ComissionMeeting.Id == comissionId)
                .Where(x=> x.ComissionDocumentDecision == ComissionDocumentDecision.Resolution)
                .Select(x => x.DocumentGji.Id).Distinct().ToList();

            var protIds = ProtocolDomain.GetAll()
                .Where(x => docIds.Contains(x.Parent.Id))
                .Select(x => x.Children.Id).Distinct().ToList();

            var prot197Ids = Protocol197Domain.GetAll()
                .Where(x => docIds.Contains(x.Parent.Id))
                .Select(x => x.Children.Id).Distinct().ToList();

            var resolCnt = ResolutionDomain.GetAll()
                .Where(x => protIds.Contains(x.Id))
                .Select(x => x.Id).Distinct().ToList();

            var resol197Cnt = ResolutionDomain.GetAll()
                .Where(x => prot197Ids.Contains(x.Id))
                .Select(x => x.Id).Distinct().ToList();

            var viewResolutionService = Container.Resolve<IDomainService<ViewResolution>>();

            try
            {
                var data = viewResolutionService.GetAll()
                .Where(x => resolCnt.Contains(x.Id) || resol197Cnt.Contains(x.Id))
                .AsEnumerable()
                .Select(x => new
                {
                    x.Id,
        ...
        public IDataResult GetListResolutionDefinitionsInCommission(BaseParams baseParams)
        {
            var loadParam = baseParams.GetLoadParam();
            var comissionId = baseParams.Params.GetAs<long>("comissionmeetId");  
          
            var ResolutionDefinitonDomain = this.Container.Resolve<IDomainService<ResolutionDefinition>>(); 
            try
            {
                var predata = ResolutionDefinitonDomain.GetAll()
                  .Where(x => x.ComissionMeeting != null && x.ComissionMeeting.Id == comissionId)
                    .AsEnumerable()
                    .Select(x => new ResolutionDefinitionProxy
                    {
                        Id = x.Id,
                        ...
                        FileInfo = x.FileInfo,
                        IssuedDefinition = x.IssuedDefinition,
        ...
        public IDataResult GetListListComissionsInDocument(BaseParams baseParams)
        {
            var loadParam = baseParams.GetLoadParam();
            ...
            var data = ComissionMeetingDocumentDomain.GetAll()
                .Where(x => x.DocumentGji.Id == documentId)
                .Select(x => new
                {
                    x.ComissionMeeting.Id,
                    x.ComissionDocumentDecision,
                    x.ComissionMeeting.CommissionDate,
                    x.ComissionMeeting.ComissionName,
                    x.ComissionMeeting.State,
                    ZonalInspection = x.ComissionMeeting.ZonalInspection.Name,
                    x.Description
                }).Filter(loadParam, Container);
        ...
        private ComissionDocumentDecision GetCurrentDecision(long docId, long comprx)
        {
            var ComissionMeetingDocumentDomain = this.Container.Resolve<IDomainService<ComissionMeetingDocument>>();
            var comdoc = ComissionMeetingDocumentDomain.GetAll()
                .FirstOrDefault(x => x.DocumentGji.Id == docId && x.ComissionMeeting.Id == comprx);
            if (comdoc != null)
            {
                return comdoc.ComissionDocumentDecision;
            }
            return ComissionDocumentDecision.NotSet;
        }

        private string GetInspNumber(long? inspId)
        {
            if (!inspId.HasValue)
            {
                return "";
            ...
            var comdoc = InspectionGjiDomain.GetAll()
                .FirstOrDefault(x => x.Id == inspId.Value);
            if (comdoc != null)
            {
                return comdoc.InspectionNumber;
            }
            return "";
        }

        private string GetVolator(long doc)
        {
            var ProtocolDomain = this.Container.Resolve<IDomainService<Protocol>>();
            var Protocol197Domain = this.Container.Resolve<IDomainService<Protocol197>>();
            try
            {
                var prot = ProtocolDomain.GetAll().Where(x => x.Id == doc).FirstOrDefault();
                if (prot != null && prot.IndividualPerson != null)
                {
                    return prot.IndividualPerson.Fio;
                }
                var prot197 = Protocol197Domain.GetAll().Where(x => x.Id == doc).FirstOrDefault();
                if (prot197 != null && prot197.IndividualPerson != null)
                {
                    return prot197.IndividualPerson.Fio;
                }
            }
            catch
            {
            ...
            finally
            {
                ...
                this.Container.Release(Protocol197Domain);
            }
            return "";
        }
        ...
        private int GetTasksCurrentDay()
        {
            ...
            var prot197Cnt = Protocol197Domain.GetAll()
                .Where(x => x.State.FinalState && x.DateOfProceedings.HasValue && x.DateOfProceedings.Value.Date == currentDate.Date)
                .Count();
            return cntCP + cntDisposal + protCnt + prot197Cnt;
        }
    ...
    internal class ProtocolProxy
    {
        ...
        public ComissionDocumentDecision ComissionDocumentDecision;
        public string ArticleLaw;
        public string ViolatorFio;
        public string CaseNumber;
        public DateTime? DateViolation;
        public decimal? Penalty;
    }

    internal class ResolutionProxy
    {
        public long Id;
        public long? InspectionId;
        public decimal? SumPays;
    ...
    internal class ResolutionDefinitionProxy
    {
        public long Id;
        ...
        public Inspector IssuedDefinition;
        public TypeDefinitionResolution TypeDefinition;
    }
}