namespace Bars.Gkh.DomainService
{
    ...
    public class RealityObjectTpSyncService : IRealityObjectTpSyncService
    {
        ...
        private readonly Dictionary<string, string> tpCellDisplayNames = new Dictionary<string, string>
        {
            ...
            {"Form_1#1:1", "1 Серия тип проекта"},
            ...
            {"Form_1#28:1", "1 Адрес многоквартирного дома"},
        ...
        public IDataResult Validate(TehPassportValue value)
        {
            ...
            if (!this.map.ContainsValue(code))
            {
                return new BaseDataResult();
            }

            var robject = value.TehPassport.RealityObject;
            var propertyName = this.map.FirstOrDefault(x => x.Value == code).Key;
        ...
        {
            var roValues = this.GetRoValues(this.map.Keys, robject);
            var tpValues = this.GetTpValues(this.map.Values, robject);

            var differences =
                this.map.Select(pair => new { pair, roValue = roValues.Get(pair.Key), tpValue = tpValues.Get(pair.Value) })
        ...
        public IDataResult Sync(TehPassportValue value)
        {
            try
            {
                HttpContext.Current.Session["noSync"] = true;

                var propertyName = this.map.FirstOrDefault(x => x.Value == value.FormCode + "#" + value.CellCode).Return(x => x.Key);
            ...
            {
                HttpContext.Current.Session["noSync"] = false;
            }
        }
        ...
        public IDataResult Sync(RealityObject robject)
        {
            try
            {
                ...
                var provider = this.Container.ResolveAll<IPassportProvider>().FirstOrDefault(x => x.Name == "Техпаспорт" && x.TypeDataSource == "xml");
                if (provider == null)
                {
                ...
                var roValues = this.GetRoValues(this.map.Keys, robject);
                var cellsLookup =
                    roValues.Select(x => new { CellCode = this.map[x.Key], x.Value })
                ...
                cellsLookup.ForEach(cells => provider.UpdateForm(robject.Id, cells.Key, cells.ToList(), fromSync: true ));
        ...
        private string GetRoValue(string propertyName, RealityObject robject)
        {
            ...
            if (value != null)
            {
                if (property.PropertyType.Namespace == "Bars.Gkh.Entities")
                {
                ...
                else if (property.PropertyType == typeof(ConditionHouse))
                {
                    return ((ConditionHouse) value) == ConditionHouse.Emergency ? "Да" : "Нет";
                }
