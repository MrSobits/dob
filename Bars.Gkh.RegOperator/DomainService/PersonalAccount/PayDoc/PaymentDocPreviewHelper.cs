    ...
    public class PaymentDocPreviewHelper
    {
        ...
        internal TreeNode GetPreview(IEnumerable<long> accountIds, BaseParams baseParams, out int count)
        {
            ...
            using (container.Using(accountSvc, accountDomain))
            {
                ...
                var bpfilter = new BaseParamsAccountFilter(baseParams);

                var accountsQuery = accountDomain.GetAll().WhereIf(accountIds.Any(), x => accountIds.Contains(x.Id));
                if (bpfilter != null && bpfilter.CrOwnerTypeList != null && bpfilter.CrOwnerTypeList.Count > 0)
                {
                    List<PersonalAccountOwnerType> filterbyOwner = new List<PersonalAccountOwnerType>();
                    foreach (CrOwnerFilterType typeOwnrt in bpfilter.CrOwnerTypeList)
                    {
                        switch (typeOwnrt)
                        {
                            case CrOwnerFilterType.LegalPerson:
                                filterbyOwner.Add(PersonalAccountOwnerType.Legal);
                                break;
                            ...
                            case CrOwnerFilterType.PrysicalPerson:
                                filterbyOwner.Add(PersonalAccountOwnerType.Individual);
                                break;


                        }
                    }
                    accountsQuery = accountsQuery.Where(x => filterbyOwner.Contains(x.AccountOwner.OwnerType));
                }
                ...
                if (bpfilter == null || bpfilter.CrOwnerTypeList == null  || (bpfilter != null && bpfilter.CrOwnerTypeList != null && bpfilter.CrOwnerTypeList.Count == 0) || (bpfilter != null && bpfilter.CrOwnerTypeList != null && bpfilter.CrOwnerTypeList.Contains(CrOwnerFilterType.PrysicalPerson)) || (bpfilter != null && bpfilter.CrOwnerTypeList != null && bpfilter.CrOwnerTypeList.Contains(CrOwnerFilterType.LegalPersonWithOneRoom)))
        ...
        ToDictionary(
                    ...
                    filtered.ForEach(
                          x =>
                          {
                              x.IndividualFactAddressRo = persAccOwnerDict2.ContainsKey(x.Id) && roDict.ContainsKey(persAccOwnerDict2[x.Id].RealityObjectID) ? roDict[persAccOwnerDict2[x.Id].RealityObjectID] : null;
                              x.IndividualFactAddress = persAccOwnerDict2.ContainsKey(x.Id) && fiasFA.ContainsKey(persAccOwnerDict2[x.Id].FiasFactAddressId) ? fiasFA[persAccOwnerDict2[x.Id].FiasFactAddressId] : null;
