namespace Bars.Gkh.Interceptors
{
    ...
    public class NormativeDocInterceptor : EmptyDomainInterceptor<NormativeDoc>
    {
        ...
        public override IDataResult BeforeDeleteAction(IDomainService<NormativeDoc> service, NormativeDoc entity)
        {
            ...
            using (Container.Using(npdDomain))
            {
                var listNpd = npdDomain.GetAll()
                        .Where(x => x.NormativeDoc.Id == entity.Id)
                        .Select(x => x.Id)
                        .ToList();

                if (listNpd.Any())
                {
                    return Failure("Данный нормативно - правовой документ содержит пункты ");
                }
            }
