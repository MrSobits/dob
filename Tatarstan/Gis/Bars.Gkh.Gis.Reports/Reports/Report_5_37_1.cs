namespace Bars.Gkh.Gis.Reports.Reports
{
    ...
    public class Report_5_37_1 : StimulReportDynamicExcel
    {
        ...
        {
            const string sqlMaster = @"select raj, a.nzp_raj, headbank, a.nzp_headbank, sum(count_ls) as count_ls,
                sum(sum_oplat) as sum_oplat
                from t1 a, public.s_head_bank s, public.subs_rajon_gor b
                where a.nzp_headbank=s.nzp_headbank
                and a.nzp_raj=b.nzp_raj
                group by 1,2,3,4
                order by 1,3";

            const string sqlDetail = @"select nzp_headbank, nzp_raj, (case when (paysource < 0) or (paysource > 7) then 99
                   ...
                   sum(sum_oplat) as sum_oplat
                   from t1 group by 1,2,3,4 order by 1,2,3";

            const string sqlSummary = @"select  nzp_raj, (case when (paysource < 0) or (paysource > 7) then 99
                                when  paysource = 0  then 1 else paysource end)
                                      as sort_plat,
                                      (case
                                           when paysource in (0,1) then 'Касса по счету'
...
//            var session = Container.Resolve<ISessionProvider>().GetCurrentSession(Container.Resolve<IDbConfigProvider>("GisDb").DbName);
            ...
            using (var transaction = session.BeginTransaction())
            {
                ...
                session.CreateSQLQuery("SELECT report_5_37(:startmonth, :startyear, :endmonth, :endyear);")
                ...
                master.Columns.AddRange(new[]
                {
                    ...
                    new DataColumn("headbank", typeof(string)),
                    new DataColumn("nzp_headbank", typeof(int)),
                ...
                var relationDetail = new StiDataRelation("masterdetail", "masterdetail", "masterdetail", Report.Dictionary.DataSources["master"],
                    Report.Dictionary.DataSources["detail"], new[] { "nzp_raj", "nzp_headbank" }, new[] { "nzp_raj", "nzp_headbank" });
                var relationSummary = new StiDataRelation("mastersummary", "mastersummary", "mastersummary", Report.Dictionary.DataSources["master"],
                    Report.Dictionary.DataSources["summary"], new[] { "nzp_raj" }, new[] { "nzp_raj" });
                Report.Dictionary.Relations.Add(relationDetail);
                Report.Dictionary.Relations.Add(relationSummary);
        ...
        {
            get { return "Статистика по платежам за период (полная)"; }
        }
        ...
        {
            get { return "B4.controller.report.Report_5_37_1"; }
        }
        ...
        {
            get { return Resources.Report_5_37_1; }
        }

        public override Stream GetTemplate()
        {
            return new MemoryStream(Resources.Report_5_37_1);
        }
