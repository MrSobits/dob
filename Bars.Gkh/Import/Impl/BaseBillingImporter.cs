namespace Bars.Gkh.Import
{
        ...
        where T : BaseEntity
    {
        ...
        /// </summary>
        public void Import(Stream fileStream, string archiveName, ILogImport logger = null, IProgressIndicator indicator = null, long userId = 0)
        {
            using (fileStream)
            {
                using (var streamReader = new StreamReader(fileStream, Encoding.GetEncoding(1251)))
                {
                    ...
                    this.File = fileManager.SaveFile(fileStream, archiveName);
                }
            }
        ...
        private IDictionary<string, FiasAddress> fiasBillingIds;
        ...
        /// Словарь ФИАС
        /// </summary>
        public IDictionary<string, FiasAddress> FiasBillingIds
        {
            get
            {
        ...
        {
            return this.GetRealityObject(addresUid).Return(x => x.Municipality);
        }
        ...
        /// <returns></returns>
        protected RealityObject GetRealityObject(string addressUid, string bilingId = "")
        {
            ...
            if (address == null && addressUid.Length == 20)
            {
                address = this.FiasShortuids.ContainsKey(addressUid) ? this.FiasShortuids[addressUid] : null;
            }
            else if (address == null && addressUid.Length == 36)
            {
                address = this.Fiasuids.ContainsKey(addressUid) ? this.Fiasuids[addressUid] : null;
            }

            if (address == null && !string.IsNullOrEmpty(bilingId))
            {
                address = this.FiasBillingIds.ContainsKey(bilingId) ? this.FiasBillingIds[bilingId] : null;
            }
